...
async function cap(url, slug, n=5){
  ...
  for(let i=0;i<n;i++){await page.evaluate(y=>scrollTo(0,y),Math.round(maxY*(i/(n-1))));await page.waitForTimeout(260);await page.screenshot({path:`design-pilot/review/lt-${slug}-${i}.png`});}
  console.log(`light ${slug}: ${h}px`);
}
await cap('http://localhost:3100/','home');
await cap('http://localhost:3100/insights','ins');
await cap('http://localhost:3100/product','prod');
await cap('http://localhost:3100/pricing','price');
