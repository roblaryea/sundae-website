...
| CLM-002 | `179 restaurant data models` / `500+ governed restaurant data models` | Product, technology, SQC and legacy redesign copy | **REVOKED — NOT PUBLIC** | No current architecture inventory substantiates either count | Eng | No | Do not publish either figure. The closest located inventory is 340 lazy visual components across the 12 module registries, which is a different quantity. Restore a model count only after Engineering defines the counted object, derives the inventory from code, and updates every locale atomically. |
...
| CLM-013 | ~~`5-10x richer than competitors who only see POS`~~ | /crew BYO-HR block (was live, 4 locales) | **REVOKED 2026-08-19** | None | Robert | n/a | A quantified competitive comparison with no defined methodology for "richer" and no supporting measurement - the class CLM-903 blocks. Replaced with the factual version: labor intelligence gets materially richer when scheduling, attendance, payroll and POS share one operating context. |
| CLM-014 | `36+ countries` (payroll calculation and statutory export) | /crew, /crew/payroll, Crew nav and mobile locales | **APPROVED PUBLIC** | `crew_payroll_country_packs` registry - `SELECT COUNT(DISTINCT country_code)` | Eng + Robert | No | The site carried 36 on /crew and 39+ on /crew/payroll. Both are defensible under different conventions, which is why they drifted: 36 distinct ISO codes, or 39 counting the UK's four constituent nations separately (one ISO code, two tax regimes). Standardised on 36+ with the convention stated inline, matching the registry and the approved Crew document. Do not raise this per-page as packs are authored - change it here first. |
| CLM-015 | `Crew Pay calculates gross to net across 36+ countries` | /crew/payroll | **APPROVED PUBLIC** | `app/services/crew/payroll/payroll_calculation_engine.ts` + payslip, gratuity, termination and year-end services | Eng | No | Verified in code before publishing: a gross-to-net engine with per-employee settings, salary components, approved adjustments, FICA and income-tax handlers, and immutable calculation snapshots. Supersedes the previous readiness-and-export framing, which understated the product to the point a finance buyer could conclude Sundae does not run payroll. |
| CLM-016 | `Every rule comes from a versioned country pack, so nothing is hardcoded` | /crew/payroll | **APPROVED PUBLIC** | Phase 0 lock (`docs/crew-phase-0-spec.md`) - no hardcoded statutory rates, all from versioned pack rules | Eng | No | This is the substance of the retired "not a tax engine" line. Kept deliberately: Sundae calculates payroll but is not the authority on statutory rates, and stating that is a strength rather than a hedge. Do not remove it when strengthening payroll copy. |
| CLM-017 | `Core refreshes through the trading day - typically every 2-4 hours, and faster where a source supports it` | /core | **APPROVED PUBLIC** | Tier specification + connector cadence | Eng | No | Replaces "Real-Time Operations" paired with a 2-4 hour refresh in the same sentence. Real-time language is retained only for Pulse, which genuinely is intraday (CLM-005/006); five Insights module blurbs carrying the same overstatement were moved to same-shift. |
| CLM-018 | ~~`Full Core coverage, included` / `Every package covers the same ground` / `Core covers all four`~~ | /core package cards, solves section, tiers section, FAQ (was live, 4 locales + English fallback for 18) | **REVOKED 2026-08-19** | Contradicted by `sundae-backend/config/pricing_master.ts` `CORE_PACKAGE_MODULES`, which grants 4 / 6 / 8 / 11 domain modules to Foundation / Margin / Growth / Performance respectively | Eng | Replaced by per-package area coverage (`Covers Revenue & Profit` … `Covers all four areas, incl. Foresight & Action`). Foundation gets labor + profit + revenue_assurance + pulse only; Growth does NOT include inventory or purchasing; only Performance carries `foresight`. |
| CLM-019 | `Covers Revenue & Profit` / `+ Food & Supply` / `+ Guest & Market` / `all four areas, incl. Foresight & Action` (per Core package) | /core package cards | **APPROVED PUBLIC** | `CORE_PACKAGE_MODULES` in `sundae-backend/config/pricing_master.ts`; the four areas map 1:1 onto the four packages (Foundation anchor / Margin adds / Growth adds / Performance adds) | Eng | Derived directly from the pricing authority. Re-verify if `CORE_PACKAGE_MODULES` changes. Foresight & Action remains a separately sellable expansion (`FORESIGHT_AND_ACTION`, $495 first unit) as well as a Performance component. |
...
| CLM-103 | `See pacing, labor, leakage, comps, voids, and service exceptions while the shift is still running — not after the weekly recap.` | Section 5 Layer 1 (Pulse) | **APPROVED PUBLIC** | Restaurant intelligence domain skill, Pulse design | Eng | r7+1 update: reframed from architectural domain language ("speaks covers, voids…") to buyer-facing capability statement. |
| CLM-104 | `Add the market around each restaurant: weather, events, competitor pricing, footfall signals, and daily briefings your BI stack does not naturally know.` | Section 5 Layer 2 (Watchtower) | **APPROVED PUBLIC** | Watchtower feature spec | Eng | r7+1 update: tighter buyer phrasing. All capabilities listed are documented. |
...
| CLM-304 | ~~`Start with Report Lite` (CTA action)~~ | (withdrawn) | **REVOKED 2026-08-10** | — | Robert | Report Lite is retired under price book v1.7 sections 2.6 and 13, so this acquisition CTA pointed at an entry point that no longer exists. The site CTAs were already migrated; this row is revoked so nothing is rebuilt from it. There is no free entry — Core Foundation at $1,195/mo first unit is the paid entry point. |
