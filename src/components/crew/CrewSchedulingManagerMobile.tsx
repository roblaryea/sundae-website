...
const EN = {
  ...
  ofShiftsFilled: 'of 20 shifts filled',
  gaps: 'gaps',
  legendFilled: 'filled',
  legendOpen: 'open',
  legendNoShowRisk: 'no-show risk',
  today: 'Today',
  week: 'Week',
  group: 'Group',
  byShift: 'By shift',
  byPerson: 'By person',
  byRole: 'By role',
  openShiftsActNow: 'Open shifts · act now',
  open: 'Open',
  roleServer: 'Server',
    ...
    <div style={{ background: T.bg, color: T.tx, fontFamily: FONT, padding: '8px 14px 16px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 7, background: T.surf, border: `1px solid ${T.bd}`, borderRadius: 999, padding: '5px 10px 5px 5px' }}>
          ...
          <span style={{ font: `600 12px ${FONT}`, color: T.tx }}>{t.orgName}</span>
      ...
      <div style={{ font: `700 19px ${FONT}`, color: T.tx, margin: '14px 2px 0' }}>{t.title}</div>
      ...
      <div style={{ marginTop: 13, background: `linear-gradient(180deg, ${T.warnt}, transparent)`, border: `1px solid ${T.bd}`, borderRadius: 18, padding: 15 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{ font: `700 10px ${FONT}`, letterSpacing: '.09em', textTransform: 'uppercase', color: T.tx3 }}>{t.todaysCoverage}</span>
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, font: `700 10px ${FONT}`, letterSpacing: '.04em', textTransform: 'uppercase', color: T.warnk }}>
            <span style={{ width: 8, height: 8, borderRadius: '50%', background: T.warn }} />{t.needsAttention}
          </span>
        </div>
        <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginTop: 10 }}>
          <span style={{ font: `700 24px ${FONT}`, color: T.tx }}>18 <span style={{ font: `600 14px ${FONT}`, color: T.tx2 }}>{t.ofShiftsFilled}</span></span>
          <span style={{ font: `700 13px ${FONT}`, color: T.warnk }}>2 {t.gaps}</span>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 13, marginTop: 9, flexWrap: 'wrap' }}>
          <LegendDot c={T.pos}>18 {t.legendFilled}</LegendDot>
          <LegendDot c={T.warn}>2 {t.legendOpen}</LegendDot>
          <LegendDot c={T.neg}>1 {t.legendNoShowRisk}</LegendDot>
        </div>
      </div>
      ...
      <div style={{ marginTop: 15 }}>
        <div style={{ display: 'flex', gap: 4, background: T.surf2, border: `1px solid ${T.bd}`, borderRadius: 12, padding: 4 }}>
          <span style={{ flex: 1, textAlign: 'center', minHeight: 32, lineHeight: '32px', borderRadius: 9, font: `700 12px ${FONT}`, background: T.acc, color: T.acck }}>{t.today}</span>
          <span style={{ flex: 1, textAlign: 'center', minHeight: 32, lineHeight: '32px', borderRadius: 9, font: `600 12px ${FONT}`, color: T.tx2 }}>{t.week}</span>
        </div>
        <div style={{ display: 'flex', gap: 7, marginTop: 10, alignItems: 'center' }}>
          <span style={{ font: `600 10px ${FONT}`, color: T.tx3 }}>{t.group}</span>
          <span style={{ minHeight: 28, display: 'inline-flex', alignItems: 'center', padding: '0 11px', borderRadius: 99, background: T.infot, border: `1px solid ${T.info}`, font: `600 11px ${FONT}`, color: T.infok }}>{t.byShift}</span>
          <span style={{ minHeight: 28, display: 'inline-flex', alignItems: 'center', padding: '0 11px', borderRadius: 99, background: T.surf, border: `1px solid ${T.bd}`, font: `600 11px ${FONT}`, color: T.tx2 }}>{t.byPerson}</span>
          <span style={{ minHeight: 28, display: 'inline-flex', alignItems: 'center', padding: '0 11px', borderRadius: 99, background: T.surf, border: `1px solid ${T.bd}`, font: `600 11px ${FONT}`, color: T.tx2 }}>{t.byRole}</span>
        </div>
      </div>

      {/* open shifts */}
      <div style={{ margin: '18px 2px 9px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <span style={{ font: `700 10px ${FONT}`, letterSpacing: '.09em', textTransform: 'uppercase', color: T.tx3 }}>{t.openShiftsActNow}</span>
      ...
      <div style={{ display: 'flex', background: T.surf, border: `1px dashed ${T.warn}`, borderRadius: 15, overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', padding: '0 11px' }}>
          <span style={{ minHeight: 30, display: 'inline-flex', alignItems: 'center', borderRadius: 9, background: T.acc, color: T.acck, font: `700 11px ${FONT}`, padding: '0 12px' }}>{t.fill}</span>
        </div>
      </div>
    </div>
  );
