...
export const payrollCopy: Record<'en', CrewModuleCopy> = {
  en: {
    ...
    secondaryCta: 'See Crew in action',
    heroProof: [
      { value: '36+ countries', label: 'US, Canada, UK, EU & GCC' },
    ...
    featuresTitle: 'A real pay run, in every country you operate.',
    featuresDescription:
      'Crew calculates the run, produces the payslips and files, and keeps the statutory rules in versioned country packs rather than in code - so what it applies is current, and auditable, without your team maintaining rates.',    features: [
      { title: '36+ countries, one way of working', body: 'The US (every state and major city), Canada (every province), the UK (all four nations), the 27 EU countries and the six GCC states - the same run-review-pay flow everywhere. Counted as ISO country codes; the UK is one code covering four nations.', chips: ['US · Canada · UK', 'EU · GCC', '36+ countries'] },
      { title: 'Gross to net, then the files', body: 'Crew calculates the run from each country pack, then produces the payslips, statutory filings and bank-payment files in the exact format the authorities expect - your team reviews and sends.', chips: ['Gross to net', 'Payslips', 'Statutory files'] },
      ...
      { title: 'Or keep your provider', body: "Already committed to a payroll provider? Crew still calculates and exports to them, so you get one workforce record and one set of numbers without ripping anything out.", chips: ['Clean hand-off', 'Works alongside', 'Your choice'] },
    ...
    howEyebrow: 'How it works',
    howTitle: 'Hours in, pay run out - checked before payday.',
    howSteps: [
      { title: 'Hours come in', body: 'Approved hours arrive from Time & Attendance with overtime already calculated.' },
      { title: 'Crew calculates the run', body: "Each country's versioned pack drives gross to net, then the payslips, bank payments and year-end forms - nothing hardcoded, always current." },
      { title: 'You check, then pay', body: 'Your team reviews the plain-English preview and the audit trail behind it, then pays directly or exports to your provider.' },
    ],
    loopLine: 'Pay-run status and overtime trends flow into Labor Intelligence - so cost is never a month behind.',
    creamEyebrow: 'Your rules, versioned - not hardcoded',
    creamStatement: 'Close the month in every country.',
    creamLede:
      'The same run-review-pay flow across 36+ countries - calculated from versioned country packs, with payslips, statutory filings and year-end forms, all reviewed by your team before payday.',
    relatedEyebrow: 'The rest of Crew',
    relatedTitle: 'Payroll is one piece of the operational layer.',
    ctaEyebrow: 'End the country-by-country scramble',
    ctaTitle: 'See a pay run, country by country.',
    ctaDescription:
      '30 minutes, your countries. We walk a calculated run end to end - gross to net, payslips, statutory filings and year-end forms - and the export path if you keep a provider.',
    ctaPrimary: 'Book a Crew walkthrough',
    ctaSecondary: 'Explore Sundae Crew',
  },
};
