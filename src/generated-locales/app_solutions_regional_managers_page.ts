// Generated website local copy overrides for src/app/solutions/regional-managers/page.tsx.
// Structural fields and protected glossary terms normalized from English source; review prose translations before release.
export const generatedLocalCopy = {
  "localizedCopy": {
    "fr": {
      "badge": "Pour les directeurs régionaux et de zone",
      "titleLine1": "Gérez douze points de vente.",
      "titleLine2": "Comme si vous étiez dans chacun d'eux.",
      "description": "Visibilité des services en direct sur chaque site. Le rythme signale le point de vente qui a besoin de vous en ce moment. Plus besoin d'attendre le récapitulatif du vendredi.",
      "primaryCta": "Réservez une visite régionale",
      "secondaryCta": "Voir la démo des opérations",
      "problemsEyebrow": "CE QUI VOUS RALENTIT",
      "problemsTitle": "Ce qui ralentit un directeur régional",
      "problemsDescription": "Trop de sites, pas assez de temps, trop de données après coup.",
      "challenges": [
        {
          "title": "Vous ne pouvez pas être dans tous les restaurants à la fois",
          "description": "Le temps que le mauvais service se termine, vous l'apprenez par le récapitulatif - et non sur le terrain.",
          "icon": "operators"
        },
        {
          "title": "Les performances du serveur varient de plus de 30 %",
          "description": "Vous connaissez vos meilleurs éléments et vos retardataires. Vous ne le savez pas à temps pour les jumeler.",
          "icon": "performance"
        },
        {
          "title": "La fuite se cache dans le bruit",
          "description": "Annulations, gestes commerciaux, remises, sur-effectifs - petits par service, coûteux sur un mois.",
          "icon": "balance"
        },
        {
          "title": "Chaque site demande de l'aide en même temps",
          "description": "Sans visibilité sur le rythme, vous triez à l'instinct, et non selon le site qui décroche réellement.",
          "icon": "support"
        }
      ],
      "howTitle": "Comment Sundae fonctionne pour les directeurs régionaux",
      "howDescription": "Une vue en direct de chaque service, de chaque serveur, de chaque point de vente.",
      "howSundaeHelps": [
        {
          "title": "Rythme en direct dans toute la région",
          "description": "Les revenus, les couverts et la main-d'œuvre de chaque site - mis à jour tout au long du service. Le site qui décroche est signalé dans votre brief du matin, pas dans la revue du vendredi.",
          "product": "Pulse",
          "icon": "chart"
        },
        {
          "title": "Intelligence au niveau du serveur",
          "description": "Taux de vente additionnelle, rotation des tables, addition moyenne par serveur. Associez votre serveur en retard à votre meilleur élément avant la fin du service.",
          "product": "Pulse · Server view",
          "icon": "operators"
        },
        {
          "title": "Fuite repérée pendant le service",
          "description": "Tickets annulés, gestes commerciaux groupés, remises hors normes - repérés sur place pendant qu'il est encore temps de parler au directeur.",
          "product": "Insights · Revenue Assurance",
          "icon": "owners"
        },
        {
          "title": "Sundae Coach donne le prochain coup",
          "description": "Lorsqu'un site est en retard de 14 %, Coach recommande un jeu spécifique : travail ajusté, campagne de fidélisation, échange de menus - basé sur ce qui a fonctionné auparavant.",
          "product": "Sundae Intelligence",
          "icon": "intelligence"
        }
      ],
      "outcomesTitle": "Quels changements pour la région",
      "outcomesDescription": "Intervention plus rapide, chantiers plus pointus, moins de surprises du vendredi.",
      "outcomes": [
        {
          "title": "Intervenir pendant le service, pas après",
          "description": "Le site qui a besoin de vous envoie le signal avant même que la salle ne le sache. L'action se dirige vers le bon point de vente au bon moment.",
          "icon": "speed"
        },
        {
          "title": "Coach au lieu d’audit",
          "description": "Arrêtez de passer votre visite à courir après ce que le rapport a manqué. Profitez de la visite pour coacher l’équipe sur ce qu’elle doit faire ensuite.",
          "icon": "support"
        },
        {
          "title": "Même playbook, chaque point de vente",
          "description": "Ce qui fonctionne sur le site 4 se propage au site 7. Les normes à l'échelle de la région restent strictes sans la moindre note de service.",
          "icon": "performance"
        },
        {
          "title": "Récupérez les fuites dont vous ignoriez l'existence",
          "description": "1 à 3 % des revenus se cachent généralement dans les annulations, les gestes commerciaux et les surclassements. Sundae les fait remonter service après service.",
          "icon": "owners"
        }
      ],
      "ctaTitle": "Visualisez votre région en une seule vue.",
      "ctaDescription": "30 minutes. Vos sites. Le service que vous auriez géré différemment.",
      "ctaButton": "Réservez une visite régionale"
    },
    "ar": {
      "badge": "للمديرين الإقليميين ومديري المناطق",
      "titleLine1": "أدِر اثني عشر فرعًا.",
      "titleLine2": "وكأنك واقف داخل أحدها.",
      "description": "رؤية مباشرة للورديات عبر كل موقع. يحدد الإيقاع الفرع الذي يحتاجك الآن. لا انتظار لملخص يوم الجمعة.",
      "primaryCta": "احجز جولة إقليمية",
      "secondaryCta": "شاهد عرض العمليات",
      "problemsEyebrow": "ما يبطئك",
      "problemsTitle": "ما يبطئ المدير الإقليمي",
      "problemsDescription": "هناك عدد كبير جدًا من المواقع، وليس الوقت الكافي، والكثير من البيانات بعد وقوعها.",
      "challenges": [
        {
          "title": "لا يمكنك أن تكون في كل مطعم في وقت واحد",
          "description": "بحلول وقت انتهاء الوردية السيئة، تكتشف ذلك من الملخص - وليس من أرضية المطعم.",
          "icon": "operators"
        },
        {
          "title": "يختلف أداء النُّدُل بنسبة 30%+",
          "description": "أنت تعرف الأفضل أداءً والمتأخرين. لكنك لا تعرف ذلك في الوقت المناسب لإقرانهم.",
          "icon": "performance"
        },
        {
          "title": "يختبئ التسرب في الضوضاء",
          "description": "عمليات الإلغاء، والمجاملات، والخصومات، والعمالة الزائدة - صغيرة في كل وردية، ومكلفة على مدار شهر.",
          "icon": "balance"
        },
        {
          "title": "كل موقع يطلب المساعدة في وقت واحد",
          "description": "بدون رؤية للإيقاع، فإنك تفرز الأولويات بناءً على الحدس، وليس على الموقع الذي يتراجع فعليًا.",
          "icon": "support"
        }
      ],
      "howTitle": "كيف يعمل Sundae للمديرين الإقليميين",
      "howDescription": "عرض مباشر واحد لكل وردية، وكل نادل، وكل فرع.",
      "howSundaeHelps": [
        {
          "title": "إيقاع حي في جميع أنحاء المنطقة",
          "description": "إيرادات كل موقع وعدد ضيوفه وعمالته - يتم تحديثها خلال الوردية. يُوضع على الموقع المتراجع علامة في ملخصك الصباحي، وليس في مراجعة يوم الجمعة.",
          "product": "Pulse",
          "icon": "chart"
        },
        {
          "title": "ذكاء على مستوى النادل",
          "description": "معدل البيع الإضافي، ودوران الطاولات، ومتوسط الفاتورة لكل نادل. اقرِن النادل المتأخر بأقوى نادل لديك قبل انتهاء الوردية.",
          "product": "Pulse · Server view",
          "icon": "operators"
        },
        {
          "title": "تسرب يُرصد أثناء الوردية",
          "description": "عمليات الإلغاء، وتجمعات المجاملات، والخصومات الخارجة عن النمط - تظهر في الموقع بينما لا يزال هناك وقت للتحدث مع المدير العام.",
          "product": "Insights · Revenue Assurance",
          "icon": "owners"
        },
        {
          "title": "مدرب Sundae يعطي الخطوة التالية",
          "description": "عندما يتأخر موقع ما بنسبة 14%، يوصي Coach بإجراء محدد - تعديل العمالة، دفعة ولاء، تبديل قائمة - بناءً على ما نجح من قبل.",
          "product": "Sundae Intelligence",
          "icon": "intelligence"
        }
      ],
      "outcomesTitle": "ما الذي يتغير بالنسبة للمنطقة",
      "outcomesDescription": "تدخل أسرع، ومواقع أكثر وضوحًا، ومفاجآت يوم الجمعة أقل.",
      "outcomes": [
        {
          "title": "تدخّل أثناء الوردية، وليس بعدها",
          "description": "الموقع الذي يحتاجك يرسل الإشارة قبل أن تعرفها أرضية المطعم. ينتقل الإجراء إلى الفرع الصحيح في الوقت المناسب.",
          "icon": "speed"
        },
        {
          "title": "تدريب بدلاً من تدقيق",
          "description": "توقف عن قضاء الزيارة في مطاردة ما فاته التقرير. استخدم الزيارة لتدريب الفريق على ما يجب فعله بعد ذلك.",
          "icon": "support"
        },
        {
          "title": "نفس دليل العمل، في كل فرع",
          "description": "ما ينجح في الموقع 4 ينتشر إلى الموقع 7. وتظل المعايير على مستوى المنطقة صارمة دون مذكرة.",
          "icon": "performance"
        },
        {
          "title": "استرجع التسرب الذي لم تكن تعلم بوجوده",
          "description": "عادةً ما يختبئ ما بين 1 إلى 3% من الإيرادات في عمليات الإلغاء والمجاملات والتجاوزات. تكشفها Sundae وردية تلو الأخرى.",
          "icon": "owners"
        }
      ],
      "ctaTitle": "شاهد منطقتك في عرض واحد.",
      "ctaDescription": "30 دقيقة. مواقعك. الوردية التي كنت ستتعامل معها بشكل مختلف.",
      "ctaButton": "احجز جولة إقليمية"
    },
    "es": {
      "badge": "Para gerentes regionales y de área",
      "titleLine1": "Gestione doce locales.",
      "titleLine2": "Como si estuviera dentro de uno.",
      "description": "Visibilidad de turnos en vivo en todos los locales. El ritmo señala el local que te necesita en este momento. Sin esperar al resumen del viernes.",
      "primaryCta": "Agenda una sesión regional",
      "secondaryCta": "Vea la demostración de operaciones",
      "problemsEyebrow": "LO QUE TE DETIENE",
      "problemsTitle": "¿Qué frena a un gerente regional?",
      "problemsDescription": "Demasiados sitios, poco tiempo, demasiados datos posteriores al hecho.",
      "challenges": [
        {
          "title": "No puedes estar en todos los restaurantes a la vez.",
          "description": "Cuando termina el mal turno, lo descubres en el resumen, no en el piso.",
          "icon": "operators"
        },
        {
          "title": "El rendimiento de los camareros varía en un 30%+",
          "description": "Conoces a los de mejor desempeño y a los rezagados. No lo sabes a tiempo para emparejarlos.",
          "icon": "performance"
        },
        {
          "title": "La fuga se esconde en el ruido.",
          "description": "Anulaciones, invitaciones, descuentos, exceso de personal: pequeños por turno, caros en un mes.",
          "icon": "balance"
        },
        {
          "title": "Cada local pide ayuda a la vez",
          "description": "Sin visibilidad del ritmo, priorizas por instinto, no por el local que realmente se está cayendo.",
          "icon": "support"
        }
      ],
      "howTitle": "Cómo funciona Sundae para los gerentes regionales",
      "howDescription": "Una vista en vivo de cada turno, cada camarero, cada local.",
      "howSundaeHelps": [
        {
          "title": "Ritmo en vivo en toda la región",
          "description": "Los ingresos, los comensales y la mano de obra de cada local, actualizados durante el turno. El local que se está cayendo se marca en el informe de la mañana, no en la revisión del viernes.",
          "product": "Pulse",
          "icon": "chart"
        },
        {
          "title": "Inteligencia a nivel de camarero",
          "description": "Tasa de venta adicional, rotación de mesas, ticket medio por camarero. Empareja a tu camarero rezagado con el más fuerte antes de que termine el turno.",
          "product": "Pulse · Server view",
          "icon": "operators"
        },
        {
          "title": "Fuga detectada en el turno.",
          "description": "Anulaciones, grupos de invitaciones, descuentos fuera de patrón... surgieron en el local mientras todavía había tiempo para hablar con el gerente general.",
          "product": "Insights · Revenue Assurance",
          "icon": "owners"
        },
        {
          "title": "Sundae Coach da el siguiente paso",
          "description": "Cuando un sitio tiene un ritmo de retraso del 14%, Coach recomienda una jugada específica (mano de obra ajustada, impulso de lealtad, cambio de menú) en función de lo que funcionó antes.",
          "product": "Sundae Intelligence",
          "icon": "intelligence"
        }
      ],
      "outcomesTitle": "¿Qué cambios para la región?",
      "outcomesDescription": "Intervención más rápida, sitios más precisos, menos sorpresas los viernes.",
      "outcomes": [
        {
          "title": "Intervenir en el turno, no después",
          "description": "El local que te necesita envía la señal antes de que la sala lo sepa. La acción llega al local correcto en el momento correcto.",
          "icon": "speed"
        },
        {
          "title": "Entrenar en lugar de auditar",
          "description": "Deja de pasar la visita persiguiendo lo que el informe omitió. Usa la visita para orientar al equipo sobre qué hacer a continuación.",
          "icon": "support"
        },
        {
          "title": "El mismo manual de jugadas, en cada local",
          "description": "Lo que funciona en el Local 4 se propaga al Local 7. Los estándares de toda la región se mantienen estrictos sin necesidad de una nota.",
          "icon": "performance"
        },
        {
          "title": "Recupera fugas que no sabías que tenías",
          "description": "Por lo general, entre el 1% y el 3% de los ingresos se esconden en anulaciones, invitaciones y sobrescrituras. Sundae las saca a la luz turno a turno.",
          "icon": "owners"
        }
      ],
      "ctaTitle": "Vea su región en una sola vista.",
      "ctaDescription": "30 minutos. Tus locales. El turno que habrías gestionado de otra manera.",
      "ctaButton": "Agenda una sesión regional"
    },
    "nl": {
      "badge": "Voor regio- en gebiedsmanagers",
      "titleLine1": "Beheer twaalf vestigingen.",
      "titleLine2": "Alsof je er middenin staat.",
      "description": "Live zicht op elke dienst op elke locatie. Pacing markeert de vestiging die je nu nodig heeft. Geen wachten op de samenvatting van vrijdag.",
      "primaryCta": "Boek een Regionale Walk-through",
      "secondaryCta": "Bekijk de Ops-demo",
      "problemsEyebrow": "WAT VERTRAAGT JE",
      "problemsTitle": "Wat een regiomanager afremt",
      "problemsDescription": "Te veel sites, niet genoeg tijd, te veel gegevens achteraf.",
      "challenges": [
        {
          "title": "Je kunt niet in elk restaurant tegelijk zijn",
          "description": "Tegen de tijd dat de slechte dienst voorbij is, kom je erachter via de samenvatting - niet op de vloer.",
          "icon": "operators"
        },
        {
          "title": "Prestaties van bedienend personeel verschillen met 30%+",
          "description": "Je kent de toppresteerders en de achterblijvers. Je weet het niet op tijd om ze te koppelen.",
          "icon": "performance"
        },
        {
          "title": "Lekkage verbergt zich in de ruis",
          "description": "Annuleringen, weggeefacties, kortingen, overbezetting: klein per dienst, duur over een maand.",
          "icon": "balance"
        },
        {
          "title": "Elke vestiging vraagt tegelijk om hulp",
          "description": "Zonder zicht op het tempo prioriteer je op instinct, niet op welke locatie daadwerkelijk wegglijdt.",
          "icon": "support"
        }
      ],
      "howTitle": "Hoe Sundae werkt voor regiomanagers",
      "howDescription": "Eén liveweergave van elke dienst, elke medewerker, elke vestiging.",
      "howSundaeHelps": [
        {
          "title": "Live-pacing door de hele regio",
          "description": "De omzet, gasten en arbeid van elke locatie worden gedurende de dienst bijgewerkt. De vestiging die wegglijdt, wordt gemarkeerd in je ochtendbriefing, niet in de review van vrijdag.",
          "product": "Pulse",
          "icon": "chart"
        },
        {
          "title": "Intelligentie op medewerkerniveau",
          "description": "Upsell-percentage, tafelrotatie, gemiddelde rekening per medewerker. Koppel je achterblijvende medewerker aan je sterkste voordat de dienst eindigt.",
          "product": "Pulse · Server view",
          "icon": "operators"
        },
        {
          "title": "Lekkage opgemerkt tijdens de dienst",
          "description": "Annuleringen, clusters van weggeefacties, kortingen buiten het patroon: ze doken op in de vestiging terwijl er nog tijd was om met de GM te praten.",
          "product": "Insights · Revenue Assurance",
          "icon": "owners"
        },
        {
          "title": "Sundae Coach geeft de volgende zet",
          "description": "Wanneer een site 14% achterloopt, beveelt Coach een specifieke actie aan - aangepaste arbeid, een loyaliteitsactie, een menuwissel - op basis van wat eerder heeft gewerkt.",
          "product": "Sundae Intelligence",
          "icon": "intelligence"
        }
      ],
      "outcomesTitle": "Wat verandert er voor de regio",
      "outcomesDescription": "Snellere interventie, scherpere sites, minder verrassingen op vrijdag.",
      "outcomes": [
        {
          "title": "Grijp in tijdens de dienst, niet erna",
          "description": "De locatie die je nodig heeft, stuurt het signaal voordat de vloer het weet. Actie gaat op het juiste moment naar de juiste vestiging.",
          "icon": "speed"
        },
        {
          "title": "Coachen in plaats van auditeren",
          "description": "Stop met je bezoek te besteden aan het najagen van wat het rapport heeft gemist. Gebruik het bezoek om het team te coachen over wat het vervolgens moet doen.",
          "icon": "support"
        },
        {
          "title": "Hetzelfde draaiboek, elke vestiging",
          "description": "Wat op locatie 4 werkt, verspreidt zich naar locatie 7. De normen voor de hele regio blijven streng zonder een memo.",
          "icon": "performance"
        },
        {
          "title": "Herstel lekkage waarvan je niet wist dat je die had",
          "description": "1 à 3% van de omzet verbergt zich doorgaans in annuleringen, weggeefacties en overschrijvingen. Sundae brengt het dienst voor dienst naar boven.",
          "icon": "owners"
        }
      ],
      "ctaTitle": "Bekijk uw regio in één weergave.",
      "ctaDescription": "30 minuten. Jouw locaties. De dienst die je anders zou hebben aangepakt.",
      "ctaButton": "Boek een Regionale Walk-through"
    },
    "pt": {
      "badge": "Para gerentes regionais e de área",
      "titleLine1": "Gerencie doze unidades.",
      "titleLine2": "Como se você estivesse dentro de uma.",
      "description": "Visibilidade do turno ao vivo em todos os locais. O ritmo sinaliza a unidade que precisa de você agora. Sem esperar pela recapitulação de sexta-feira.",
      "primaryCta": "Agende um passeio regional",
      "secondaryCta": "Veja a demonstração de operações",
      "problemsEyebrow": "O QUE TE ATRASA",
      "problemsTitle": "O que retarda um gerente regional",
      "problemsDescription": "Muitos sites, pouco tempo, muitos dados após o fato.",
      "challenges": [
        {
          "title": "Você não pode estar em todos os restaurantes ao mesmo tempo",
          "description": "Quando o turno ruim termina, você descobre pela recapitulação - não pelo salão.",
          "icon": "operators"
        },
        {
          "title": "O desempenho dos garçons varia em 30%+",
          "description": "Você conhece os melhores e os retardatários. Você não sabe a tempo de emparelhá-los.",
          "icon": "performance"
        },
        {
          "title": "O vazamento se esconde no barulho",
          "description": "Cancelamentos, cortesias, descontos, excesso de pessoal - pequenos por turno, caros em um mês.",
          "icon": "balance"
        },
        {
          "title": "Toda unidade pede ajuda de uma vez",
          "description": "Sem visibilidade de ritmo, você faz a triagem por instinto, e não por qual local está realmente escorregando.",
          "icon": "support"
        }
      ],
      "howTitle": "Como funciona o Sundae para gerentes regionais",
      "howDescription": "Uma visualização ao vivo de cada turno, cada garçom, cada unidade.",
      "howSundaeHelps": [
        {
          "title": "Ritmo ao vivo em toda a região",
          "description": "Receita, clientes e mão de obra de cada local - atualizados durante o turno. A unidade que está escorregando é sinalizada em seu briefing matinal, não na revisão de sexta-feira.",
          "product": "Pulse",
          "icon": "chart"
        },
        {
          "title": "Inteligência em nível de garçom",
          "description": "Taxa de upsell, giro de mesa, ticket médio por garçom. Emparelhe seu garçom atrasado com o mais forte antes que o turno termine.",
          "product": "Pulse · Server view",
          "icon": "operators"
        },
        {
          "title": "Vazamento detectado no turno",
          "description": "Cancelamentos, grupos de cortesias, descontos fora do padrão - surgiram no local enquanto ainda há tempo para falar com o GM.",
          "product": "Insights · Revenue Assurance",
          "icon": "owners"
        },
        {
          "title": "Sundae Coach dá o próximo passo",
          "description": "Quando um site está 14% atrasado, o Coach recomenda uma jogada específica - mão de obra ajustada, um impulso de fidelidade, uma troca de menu - com base no que funcionou antes.",
          "product": "Sundae Intelligence",
          "icon": "intelligence"
        }
      ],
      "outcomesTitle": "O que muda para a região",
      "outcomesDescription": "Intervenção mais rápida, sites mais nítidos, menos surpresas de sexta-feira.",
      "outcomes": [
        {
          "title": "Intervir no turno, não depois",
          "description": "O local que precisa de você envia o sinal antes que o salão saiba. A ação se move para a unidade certa na hora certa.",
          "icon": "speed"
        },
        {
          "title": "Treinar em vez de auditar",
          "description": "Pare de gastar a visita perseguindo o que o relatório deixou passar. Use a visita para orientar a equipe sobre o que fazer a seguir.",
          "icon": "support"
        },
        {
          "title": "O mesmo manual, em todas as unidades",
          "description": "O que funciona na Unidade 4 se propaga para a Unidade 7. Os padrões regionais permanecem rígidos sem um memorando.",
          "icon": "performance"
        },
        {
          "title": "Recupere vazamentos que você não sabia que tinha",
          "description": "1-3% da receita normalmente fica oculta em cancelamentos, cortesias e substituições. O Sundae traz isso à tona turno por turno.",
          "icon": "owners"
        }
      ],
      "ctaTitle": "Veja sua região em uma visualização.",
      "ctaDescription": "30 minutos. Seus locais. O turno que você teria tratado de forma diferente.",
      "ctaButton": "Agende um passeio regional"
    },
    "de": {
      "badge": "Für Regional- und Gebietsmanager",
      "titleLine1": "Betreiben Sie zwölf Filialen.",
      "titleLine2": "Als stünden Sie in einer davon.",
      "description": "Live-Einblick in jede Schicht an jedem Standort. Pacing markiert die Filiale, die Sie gerade braucht. Kein Warten auf die Zusammenfassung vom Freitag.",
      "primaryCta": "Buchen Sie einen regionalen Rundgang",
      "secondaryCta": "Sehen Sie sich die Ops-Demo an",
      "problemsEyebrow": "Was dich verlangsamt",
      "problemsTitle": "Was einen Regionalmanager ausbremst",
      "problemsDescription": "Zu viele Websites, zu wenig Zeit, zu viele Daten im Nachhinein.",
      "challenges": [
        {
          "title": "Man kann nicht in jedem Restaurant gleichzeitig sein",
          "description": "Wenn die schlechte Schicht vorbei ist, erfahren Sie es aus der Zusammenfassung - nicht auf der Fläche.",
          "icon": "operators"
        },
        {
          "title": "Die Leistung der Servicekräfte variiert um mehr als 30 %",
          "description": "Sie kennen die Leistungsträger und die Nachzügler. Sie wissen es nicht rechtzeitig, um sie zu paaren.",
          "icon": "performance"
        },
        {
          "title": "Schwund verbirgt sich im Rauschen",
          "description": "Stornos, Kulanzbuchungen, Rabatte, Überbesetzung - klein pro Schicht, teuer über einen Monat hinweg.",
          "icon": "balance"
        },
        {
          "title": "Jede Filiale bittet gleichzeitig um Hilfe",
          "description": "Ohne Einblick in das Tempo priorisieren Sie nach Instinkt und nicht nach der Filiale, die tatsächlich abrutscht.",
          "icon": "support"
        }
      ],
      "howTitle": "So funktioniert Sundae für Regionalmanager",
      "howDescription": "Eine Live-Ansicht jeder Schicht, jeder Servicekraft, jeder Filiale.",
      "howSundaeHelps": [
        {
          "title": "Live-Pacing in der gesamten Region",
          "description": "Die Einnahmen, Gäste und Arbeitskräfte jedes Standorts - aktualisiert während der Schicht. Die abrutschende Filiale wird in Ihrem morgendlichen Briefing markiert, nicht in der Auswertung vom Freitag.",
          "product": "Pulse",
          "icon": "chart"
        },
        {
          "title": "Intelligenz auf Ebene der Servicekraft",
          "description": "Upsell-Rate, Tischrotation, durchschnittlicher Bon pro Servicekraft. Koppeln Sie Ihre schwächere Servicekraft mit Ihrer stärksten, bevor die Schicht endet.",
          "product": "Pulse · Server view",
          "icon": "operators"
        },
        {
          "title": "Schwund während der Schicht entdeckt",
          "description": "Stornierte Vorgänge, Kulanz-Cluster, Rabatte außerhalb des Musters - tauchen vor Ort auf, solange noch Zeit ist, mit dem GM zu sprechen.",
          "product": "Insights · Revenue Assurance",
          "icon": "owners"
        },
        {
          "title": "Sundae Coach gibt den nächsten Schritt",
          "description": "Wenn eine Website 14 % hinterherhinkt, empfiehlt Coach einen bestimmten Ansatz - angepasste Arbeitskräfte, einen Treueschub, einen Menüwechsel - basierend auf dem, was zuvor funktioniert hat.",
          "product": "Sundae Intelligence",
          "icon": "intelligence"
        }
      ],
      "outcomesTitle": "Was sich für die Region ändert",
      "outcomesDescription": "Schnelleres Eingreifen, schärfere Websites, weniger Freitagsüberraschungen.",
      "outcomes": [
        {
          "title": "Greifen Sie während der Schicht ein, nicht danach",
          "description": "Der Standort, der Sie benötigt, sendet das Signal, bevor die Fläche es weiß. Die Aktion wird zur richtigen Zeit an die richtige Filiale weitergeleitet.",
          "icon": "speed"
        },
        {
          "title": "Coachen statt auditieren",
          "description": "Hören Sie auf, den Besuch damit zu verbringen, dem hinterherzujagen, was der Bericht übersehen hat. Nutzen Sie den Besuch, um dem Team beizubringen, was als nächstes zu tun ist.",
          "icon": "support"
        },
        {
          "title": "Gleiches Playbook, jede Filiale",
          "description": "Was an Standort 4 funktioniert, überträgt sich auf Standort 7. Die regionsweiten Standards bleiben ohne Memo streng.",
          "icon": "performance"
        },
        {
          "title": "Beseitigen Sie Schwund, von dem Sie nicht wussten, dass Sie ihn haben",
          "description": "1-3 % des Umsatzes verbergen sich typischerweise in Stornos, Kulanzbuchungen und Außerkraftsetzungen. Sundae bringt es Schicht für Schicht zum Vorschein.",
          "icon": "owners"
        }
      ],
      "ctaTitle": "Sehen Sie Ihre Region auf einen Blick.",
      "ctaDescription": "30 Minuten. Ihre Standorte. Die Schicht, die Sie anders gehandhabt hätten.",
      "ctaButton": "Buchen Sie einen regionalen Rundgang"
    },
    "ur": {
      "badge": "علاقائی اور ایریا مینیجرز کے لیے",
      "titleLine1": "بارہ آؤٹ لیٹس چلائیں۔",
      "titleLine2": "جیسے آپ ان میں سے کسی ایک میں کھڑے ہوں۔",
      "description": "ہر مقام پر لائیو شفٹ کی مرئیت۔ پیسنگ اس آؤٹ لیٹ کو نشان زد کرتی ہے جس کی ابھی آپ کو ضرورت ہے۔ جمعہ کے خلاصے کا انتظار نہیں۔",
      "primaryCta": "ریجنل واک تھرو بک کریں۔",
      "secondaryCta": "اوپس ڈیمو دیکھیں",
      "problemsEyebrow": "جو آپ کو سست کرتا ہے۔",
      "problemsTitle": "علاقائی مینیجر کو کیا سست کرتا ہے۔",
      "problemsDescription": "بہت ساری سائٹیں، کافی وقت نہیں، حقیقت کے بعد بہت زیادہ ڈیٹا۔",
      "challenges": [
        {
          "title": "آپ ایک ساتھ ہر ریستوراں میں نہیں ہو سکتے",
          "description": "جب تک بری شفٹ ختم ہوتی ہے، آپ کو خلاصے سے پتہ چلتا ہے - فلور سے نہیں۔",
          "icon": "operators"
        },
        {
          "title": "ویٹرز کی کارکردگی 30%+ تک مختلف ہوتی ہے۔",
          "description": "آپ بہترین کارکردگی دکھانے والوں اور پیچھے رہ جانے والوں کو جانتے ہیں۔ لیکن آپ انہیں جوڑنے کے لیے بروقت نہیں جان پاتے۔",
          "icon": "performance"
        },
        {
          "title": "رساو شور میں چھپ جاتا ہے۔",
          "description": "منسوخیاں، مفت پیشکشیں، ڈسکاؤنٹس، اضافی عملہ - فی شفٹ چھوٹی، ایک ماہ میں مہنگی۔",
          "icon": "balance"
        },
        {
          "title": "ہر آؤٹ لیٹ ایک ہی وقت میں مدد طلب کرتا ہے۔",
          "description": "پیسنگ مرئیت کے بغیر، آپ جبلت پر ترجیح دیتے ہیں، نہ کہ اس بنیاد پر کہ اصل میں کون سا آؤٹ لیٹ پھسل رہا ہے۔",
          "icon": "support"
        }
      ],
      "howTitle": "Sundae علاقائی مینیجرز کے لیے کیسے کام کرتا ہے۔",
      "howDescription": "ہر شفٹ، ہر ویٹر، ہر آؤٹ لیٹ کا ایک لائیو منظر۔",
      "howSundaeHelps": [
        {
          "title": "پورے خطے میں لائیو پیسنگ",
          "description": "ہر مقام کی آمدنی، مہمان، اور لیبر - شفٹ کے دوران اپ ڈیٹ ہوتے ہیں۔ جو آؤٹ لیٹ پھسل رہا ہے اسے آپ کے صبح کے بریف میں نشان زد کیا جاتا ہے، جمعہ کے جائزے میں نہیں۔",
          "product": "Pulse",
          "icon": "chart"
        },
        {
          "title": "ویٹر کی سطح کی ذہانت",
          "description": "اپ سیل ریٹ، ٹیبل ٹرن، فی ویٹر اوسط بل۔ شفٹ ختم ہونے سے پہلے اپنے پیچھے رہنے والے ویٹر کو اپنے سب سے مضبوط ویٹر کے ساتھ جوڑیں۔",
          "product": "Pulse · Server view",
          "icon": "operators"
        },
        {
          "title": "شفٹ میں رساو دیکھا گیا۔",
          "description": "منسوخ لین دین، مفت پیشکشوں کے کلسٹرز، پیٹرن سے ہٹ کر ڈسکاؤنٹس - اس مقام پر سامنے آئے جب کہ GM سے بات کرنے کا ابھی بھی وقت ہے۔",
          "product": "Insights · Revenue Assurance",
          "icon": "owners"
        },
        {
          "title": "Sundae کوچ اگلا اقدام دیتا ہے۔",
          "description": "جب کوئی آؤٹ لیٹ 14% پیچھے ہوتا ہے، تو Coach ایک مخصوص اقدام تجویز کرتا ہے - ایڈجسٹ شدہ عملہ، ایک لائلٹی پش، ایک مینو سویپ - اس کی بنیاد پر جو پہلے کام کر چکا ہے۔",
          "product": "Sundae Intelligence",
          "icon": "intelligence"
        }
      ],
      "outcomesTitle": "خطے کے لیے کیا تبدیلیاں ہیں۔",
      "outcomesDescription": "تیز تر مداخلت، تیز سائٹس، کم جمعہ کی حیرت۔",
      "outcomes": [
        {
          "title": "شفٹ میں مداخلت کریں، بعد میں نہیں۔",
          "description": "جس آؤٹ لیٹ کی آپ کو ضرورت ہے وہ فلور کو معلوم ہونے سے پہلے ہی سگنل بھیجتا ہے۔ ایکشن صحیح وقت پر صحیح آؤٹ لیٹ پر جاتا ہے۔",
          "icon": "speed"
        },
        {
          "title": "آڈٹ کے بجائے کوچنگ",
          "description": "رپورٹ میں جو چیز چھوٹ گئی اس کا پیچھا کرنے میں دورہ ضائع کرنا بند کریں۔ اس دورے کو ٹیم کی کوچنگ کے لیے استعمال کریں کہ آگے کیا کرنا ہے۔",
          "icon": "support"
        },
        {
          "title": "ایک ہی پلے بک، ہر آؤٹ لیٹ",
          "description": "سائٹ 4 پر جو کام کرتا ہے وہ سائٹ 7 پر پھیلتا ہے۔ پورے خطے کے معیار میمو کے بغیر سخت رہتے ہیں۔",
          "icon": "performance"
        },
        {
          "title": "وہ رساو بازیافت کریں جس کے بارے میں آپ کو معلوم ہی نہیں تھا۔",
          "description": "آمدنی کا 1-3% عام طور پر منسوخیوں، مفت پیشکشوں، اور اوور رائیڈز میں چھپا ہوتا ہے۔ Sundae اسے شفٹ بہ شفٹ سامنے لاتا ہے۔",
          "icon": "owners"
        }
      ],
      "ctaTitle": "اپنے علاقے کو ایک نظر میں دیکھیں۔",
      "ctaDescription": "30 منٹ۔ آپ کے مقامات۔ جس شفٹ کو آپ مختلف طریقے سے سنبھالتے۔",
      "ctaButton": "ریجنل واک تھرو بک کریں۔"
    },
    "hi": {
      "badge": "क्षेत्रीय एवं क्षेत्रीय प्रबंधकों के लिए",
      "titleLine1": "बारह आउटलेट चलाएँ।",
      "titleLine2": "जैसे आप उनमें से किसी एक में खड़े हों।",
      "description": "हर स्थान पर लाइव शिफ्ट दृश्यता। पेसिंग उस आउटलेट को चिह्नित करता है जिसे अभी आपकी ज़रूरत है। शुक्रवार के सारांश का इंतज़ार नहीं।",
      "primaryCta": "एक क्षेत्रीय वॉक-थ्रू बुक करें",
      "secondaryCta": "ऑप्स डेमो देखें",
      "problemsEyebrow": "आपको क्या धीमा करता है",
      "problemsTitle": "एक क्षेत्रीय प्रबंधक की गति क्या धीमी होती है?",
      "problemsDescription": "तथ्य के बाद बहुत सारी साइटें, पर्याप्त समय नहीं, बहुत अधिक डेटा।",
      "challenges": [
        {
          "title": "आप हर रेस्तरां में एक साथ नहीं हो सकते",
          "description": "जब तक ख़राब शिफ्ट ख़त्म होती है, आपको सारांश से पता चलता है - फ़्लोर से नहीं।",
          "icon": "operators"
        },
        {
          "title": "वेटर का प्रदर्शन 30%+ तक भिन्न होता है",
          "description": "आप शीर्ष प्रदर्शन करने वालों और पिछड़ने वालों को जानते हैं। पर उन्हें जोड़ने के लिए आप समय रहते नहीं जान पाते।",
          "icon": "performance"
        },
        {
          "title": "शोर में रिसाव छिप जाता है",
          "description": "रद्दीकरण, मुफ़्त सेवाएँ, छूट, अतिरिक्त स्टाफ़ - प्रति शिफ्ट छोटा, एक महीने में महंगा।",
          "icon": "balance"
        },
        {
          "title": "हर आउटलेट एक साथ मदद मांगता है",
          "description": "पेसिंग दृश्यता के बिना, आप अंतर्ज्ञान के आधार पर प्राथमिकता तय करते हैं, न कि इस आधार पर कि असल में कौन सा आउटलेट फिसल रहा है।",
          "icon": "support"
        }
      ],
      "howTitle": "Sundae क्षेत्रीय प्रबंधकों के लिए कैसे काम करता है",
      "howDescription": "प्रत्येक शिफ्ट, प्रत्येक वेटर, प्रत्येक आउटलेट का एक लाइव दृश्य।",
      "howSundaeHelps": [
        {
          "title": "पूरे क्षेत्र में लाइव पेसिंग",
          "description": "प्रत्येक स्थान का राजस्व, ग्राहक और श्रम - शिफ्ट के दौरान अद्यतन किए जाते हैं। जो आउटलेट फिसल रहा है उसे आपके सुबह के संक्षिप्त विवरण में चिह्नित किया जाता है, शुक्रवार की समीक्षा में नहीं।",
          "product": "Pulse",
          "icon": "chart"
        },
        {
          "title": "वेटर-स्तरीय बुद्धिमत्ता",
          "description": "अपसेल दर, टेबल टर्न, प्रति वेटर औसत बिल। शिफ्ट समाप्त होने से पहले अपने पिछड़ते वेटर को अपने सबसे मजबूत वेटर के साथ जोड़ें।",
          "product": "Pulse · Server view",
          "icon": "operators"
        },
        {
          "title": "शिफ्ट में लीकेज देखा गया",
          "description": "रद्द लेन-देन, मुफ़्त सेवाओं के समूह, पैटर्न से हटकर छूट - जीएम से बात करने का समय रहते ही स्थान पर सामने आ जाते हैं।",
          "product": "Insights · Revenue Assurance",
          "icon": "owners"
        },
        {
          "title": "Sundae कोच अगली चाल देता है",
          "description": "जब कोई आउटलेट 14% पीछे चल रहा होता है, तो Coach एक विशिष्ट कदम की सिफारिश करता है - समायोजित स्टाफ़, लॉयल्टी पुश, मेनू स्वैप - जो पहले काम कर चुका है उसके आधार पर।",
          "product": "Sundae Intelligence",
          "icon": "intelligence"
        }
      ],
      "outcomesTitle": "क्षेत्र के लिए क्या परिवर्तन",
      "outcomesDescription": "तेज़ हस्तक्षेप, तेज़ साइटें, शुक्रवार को कम आश्चर्य।",
      "outcomes": [
        {
          "title": "शिफ्ट में हस्तक्षेप करें, बाद में नहीं",
          "description": "जिस आउटलेट को आपकी आवश्यकता है वह फ़्लोर को पता चलने से पहले ही संकेत भेज देता है। कार्रवाई सही समय पर सही आउटलेट की ओर बढ़ती है।",
          "icon": "speed"
        },
        {
          "title": "ऑडिट के बजाय कोचिंग",
          "description": "रिपोर्ट में जो छूट गया, उसके पीछे दौरे को बर्बाद करना बंद करें। इस दौरे का उपयोग टीम को यह सिखाने के लिए करें कि आगे क्या करना है।",
          "icon": "support"
        },
        {
          "title": "वही प्लेबुक, हर आउटलेट",
          "description": "साइट 4 पर जो काम करता है वह साइट 7 पर फैलता है। बिना किसी मेमो के क्षेत्र-व्यापी मानक सख्त बने रहते हैं।",
          "icon": "performance"
        },
        {
          "title": "उस रिसाव को वापस पाएं जिसके बारे में आपको पता नहीं था",
          "description": "राजस्व का 1-3% आम तौर पर रद्दीकरण, मुफ़्त सेवाओं और ओवरराइड्स में छिपा होता है। Sundae इसे शिफ्ट-दर-शिफ्ट सामने लाता है।",
          "icon": "owners"
        }
      ],
      "ctaTitle": "अपने क्षेत्र को एक दृश्य में देखें.",
      "ctaDescription": "30 मिनट. आपके स्थान. शिफ्ट को आपने अलग तरीके से संभाला होगा।",
      "ctaButton": "एक क्षेत्रीय वॉक-थ्रू बुक करें"
    },
    "it": {
      "badge": "Per manager regionali e di area",
      "titleLine1": "Gestisci dodici punti vendita.",
      "titleLine2": "Come se fossi in uno di essi.",
      "description": "Visibilità dei turni in tempo reale in ogni sede. Il ritmo segnala il locale che ha bisogno di te in questo momento. Nessuna attesa per il riepilogo di venerdì.",
      "primaryCta": "Prenota una visita regionale",
      "secondaryCta": "Guarda la demo delle operazioni",
      "problemsEyebrow": "COSA TI RALLENTA",
      "problemsTitle": "Ciò che rallenta un manager regionale",
      "problemsDescription": "Troppi siti, poco tempo, troppi dati a posteriori.",
      "challenges": [
        {
          "title": "Non puoi essere in tutti i ristoranti contemporaneamente",
          "description": "Quando il brutto turno finisce, lo scopri dal riepilogo, non dal pavimento.",
          "icon": "operators"
        },
        {
          "title": "Le prestazioni dei camerieri variano del 30%+",
          "description": "Conosci i migliori e i ritardatari. Non lo sai in tempo per accoppiarli.",
          "icon": "performance"
        },
        {
          "title": "La perdita si nasconde nel rumore",
          "description": "Storni, omaggi, sconti, personale in eccesso: piccoli per turno, costosi nell'arco di un mese.",
          "icon": "balance"
        },
        {
          "title": "Ogni locale chiede aiuto contemporaneamente",
          "description": "Senza visibilità sul ritmo, dai le priorità per istinto, non in base a quale locale sta effettivamente scivolando.",
          "icon": "support"
        }
      ],
      "howTitle": "Come funziona Sundae per i manager regionali",
      "howDescription": "Una visione dal vivo di ogni turno, ogni cameriere, ogni locale.",
      "howSundaeHelps": [
        {
          "title": "Ritmo in tempo reale in tutta la regione",
          "description": "Ricavi, coperti e manodopera di ciascuna sede: aggiornati durante il turno. Il locale che sta scivolando viene segnalato nel briefing mattutino, non nella revisione di venerdì.",
          "product": "Pulse",
          "icon": "chart"
        },
        {
          "title": "Intelligenza a livello di cameriere",
          "description": "Tasso di upsell, rotazione dei tavoli, scontrino medio per cameriere. Accoppia il tuo cameriere in ritardo con il più forte prima della fine del turno.",
          "product": "Pulse · Server view",
          "icon": "operators"
        },
        {
          "title": "Perdita individuata nel turno",
          "description": "Storni, gruppi di omaggi, sconti fuori schema: sono emersi sul posto mentre c'era ancora tempo per parlare con il GM.",
          "product": "Insights · Revenue Assurance",
          "icon": "owners"
        },
        {
          "title": "Sundae Coach dà la mossa successiva",
          "description": "Quando il ritmo di un sito è indietro del 14%, il Coach consiglia un'azione specifica (manodopera modificata, spinta alla fidelizzazione, scambio di menu) in base a ciò che ha funzionato prima.",
          "product": "Sundae Intelligence",
          "icon": "intelligence"
        }
      ],
      "outcomesTitle": "Cosa cambia per la Regione",
      "outcomesDescription": "Interventi più rapidi, siti più nitidi, meno sorprese del venerdì.",
      "outcomes": [
        {
          "title": "Intervenire nel turno, non dopo",
          "description": "Il locale che ha bisogno di te invia il segnale prima che la sala lo sappia. L'azione si sposta verso il locale giusto al momento giusto.",
          "icon": "speed"
        },
        {
          "title": "Coaching invece di audit",
          "description": "Smetti di trascorrere la visita rincorrendo ciò che il rapporto ha mancato. Usa la visita per istruire la squadra su cosa fare dopo.",
          "icon": "support"
        },
        {
          "title": "Stesso playbook, ogni locale",
          "description": "Ciò che funziona nel Sito 4 si propaga al Sito 7. Gli standard a livello regionale rimangono rigorosi senza alcun promemoria.",
          "icon": "performance"
        },
        {
          "title": "Recupera le perdite che non sapevi di avere",
          "description": "L'1-3% delle entrate in genere si nasconde in storni, omaggi e forzature. Sundae le fa emergere turno dopo turno.",
          "icon": "owners"
        }
      ],
      "ctaTitle": "Visualizza la tua regione in un'unica visualizzazione.",
      "ctaDescription": "30 minuti. Le tue posizioni. Il turno che avresti gestito diversamente.",
      "ctaButton": "Prenota una visita regionale"
    },
    "zh-Hans": {
      "badge": "对于区域和区域经理",
      "titleLine1": "经营十二家门店。",
      "titleLine2": "就像你正站在其中一家。",
      "description": "每家门店的实时班次可见性。节奏会标记出此刻需要你的门店。无需等到周五的回顾。",
      "primaryCta": "预约区域巡查",
      "secondaryCta": "查看操作演示",
      "problemsEyebrow": "是什么让你慢下来",
      "problemsTitle": "是什么让区域经理放慢脚步",
      "problemsDescription": "网站太多，时间不够，事后数据太多。",
      "challenges": [
        {
          "title": "你无法同时身处每一家餐厅",
          "description": "等到糟糕的班次结束，你只能从回顾中发现问题——而不是在现场。",
          "icon": "operators"
        },
        {
          "title": "服务员的表现相差 30% 以上",
          "description": "你知道谁表现出色，谁落在后面。但你来不及在合适的时机把他们搭配起来。",
          "icon": "performance"
        },
        {
          "title": "流失隐藏在噪音中",
          "description": "作废、赠送、折扣、人手过剩——单个班次微不足道，但一个月下来代价高昂。",
          "icon": "balance"
        },
        {
          "title": "每家门店都同时求助",
          "description": "没有节奏的可见性，你只能凭本能分轻重缓急，而不是看哪家门店真正在下滑。",
          "icon": "support"
        }
      ],
      "howTitle": "Sundae如何为区域经理提供帮助",
      "howDescription": "每个班次、每位服务员、每家门店的统一实时视图。",
      "howSundaeHelps": [
        {
          "title": "整个地区的实时节奏",
          "description": "每家门店的营收、客流和人工——随班次实时更新。下滑的门店会在你的早间简报中标记出来，而不是周五的回顾中。",
          "product": "Pulse",
          "icon": "chart"
        },
        {
          "title": "服务员层级的智能洞察",
          "description": "追加销售率、翻台率、按服务员统计的人均消费。在班次结束前，把落后的服务员与最强的服务员搭配起来。",
          "product": "Pulse · Server view",
          "icon": "operators"
        },
        {
          "title": "班次中发现流失",
          "description": "作废、赠送集中、异常折扣——在仍有时间与总经理沟通时，就在该门店浮现出来。",
          "product": "Insights · Revenue Assurance",
          "icon": "owners"
        },
        {
          "title": "Sundae教练给出了下一步行动",
          "description": "当一个网站的速度落后 14% 时，Coach 会根据之前的效果推荐特定的策略--调整劳动力、忠诚度推动、菜单交换。",
          "product": "Sundae Intelligence",
          "icon": "intelligence"
        }
      ],
      "outcomesTitle": "该地区有何变化",
      "outcomesDescription": "更快的干预、更敏锐的站点、更少的周五意外。",
      "outcomes": [
        {
          "title": "在班次中干预，而不是事后",
          "description": "需要你的门店会在现场察觉之前发出信号。行动在正确的时间送达正确的门店。",
          "icon": "speed"
        },
        {
          "title": "用辅导取代审查",
          "description": "别再把巡店时间花在追查报告遗漏的内容上。利用这次巡店来指导团队下一步该做什么。",
          "icon": "support"
        },
        {
          "title": "同一套打法，每家门店",
          "description": "在门店 4 有效的做法会传播到门店 7。无需一纸通知，全区域标准依然严格统一。",
          "icon": "performance"
        },
        {
          "title": "找回你不知道存在的流失",
          "description": "1-3% 的营收通常隐藏在作废、赠送和强制改价中。Sundae 会逐个班次地把它呈现出来。",
          "icon": "owners"
        }
      ],
      "ctaTitle": "在一个视图中查看你所在的地区。",
      "ctaDescription": "30分钟。你的门店。那个你本会以不同方式处理的班次。",
      "ctaButton": "预约区域巡查"
    },
    "pl": {
      "badge": "Dla menedżerów regionalnych i lokalnych",
      "titleLine1": "Zarządzaj dwunastoma lokalami.",
      "titleLine2": "Tak, jakbyś stał w jednym z nich.",
      "description": "Widoczność zmian na żywo w każdej lokalizacji. Pacing oznacza lokal, który potrzebuje Cię teraz. Bez czekania na piątkowe podsumowanie.",
      "primaryCta": "Zarezerwuj wycieczkę regionalną",
      "secondaryCta": "Zobacz demonstrację operacji",
      "problemsEyebrow": "CO CIĘ SPOWOLNI",
      "problemsTitle": "Co spowalnia menedżera regionalnego",
      "problemsDescription": "Za dużo witryn, za mało czasu, za dużo danych po fakcie.",
      "challenges": [
        {
          "title": "Nie możesz być w każdej restauracji na raz",
          "description": "Zanim zła zmiana się skończy, dowiesz się z podsumowania, a nie z podłogi.",
          "icon": "operators"
        },
        {
          "title": "Wydajność kelnerów waha się o ponad 30%.",
          "description": "Znasz najlepszych i maruderów. Nie wiesz tego na czas, aby ich połączyć.",
          "icon": "performance"
        },
        {
          "title": "Wyciek ukrywa się w szumie",
          "description": "Anulacje, gratisy, rabaty, nadmiar personelu - małe w przeliczeniu na zmianę, drogie w ciągu miesiąca.",
          "icon": "balance"
        },
        {
          "title": "Każdy lokal prosi o pomoc naraz",
          "description": "Bez widoczności tempa ustalasz priorytety instynktownie, a nie według tego, który lokal faktycznie się osuwa.",
          "icon": "support"
        }
      ],
      "howTitle": "Jak Sundae działa na menedżerów regionalnych",
      "howDescription": "Jeden podgląd na żywo każdej zmiany, każdego kelnera i każdego lokalu.",
      "howSundaeHelps": [
        {
          "title": "Pacing na żywo w całym regionie",
          "description": "Przychody, goście i siła robocza każdej lokalizacji - aktualizowane w trakcie zmiany. Lokal, który się osuwa, zostanie oznaczony w porannym briefie, a nie w piątkowym przeglądzie.",
          "product": "Pulse",
          "icon": "chart"
        },
        {
          "title": "Inteligencja na poziomie kelnera",
          "description": "Wskaźnik sprzedaży dodatkowej, rotacja stolików, średni rachunek na kelnera. Połącz swojego opóźnionego kelnera z najsilniejszym przed końcem zmiany.",
          "product": "Pulse · Server view",
          "icon": "operators"
        },
        {
          "title": "Wyciek zauważony podczas zmiany",
          "description": "Anulowane transakcje, skupiska gratisów, nietypowe rabaty - wypłynęły na powierzchnię, póki jest jeszcze czas na rozmowę z GM.",
          "product": "Insights · Revenue Assurance",
          "icon": "owners"
        },
        {
          "title": "Sundae Coach wykonuje kolejny ruch",
          "description": "Kiedy lokal jest w tyle o 14%, Coach zaleca konkretne zagranie - skorygowaną obsadę, zachętę lojalnościową, zmianę menu - w oparciu o to, co sprawdziło się wcześniej.",
          "product": "Sundae Intelligence",
          "icon": "intelligence"
        }
      ],
      "outcomesTitle": "Co się zmienia dla regionu",
      "outcomesDescription": "Szybsza interwencja, ostrzejsze strony, mniej piątkowych niespodzianek.",
      "outcomes": [
        {
          "title": "Interweniuj na zmianie, a nie później",
          "description": "Lokal, który Cię potrzebuje, wysyła sygnał, zanim dowie się o tym sala. Działanie trafia do właściwego lokalu we właściwym czasie.",
          "icon": "speed"
        },
        {
          "title": "Coaching zamiast audytu",
          "description": "Przestań spędzać wizytę na gonieniu za tym, co zostało pominięte w raporcie. Wykorzystaj tę wizytę, aby poinstruować zespół, co robić dalej.",
          "icon": "support"
        },
        {
          "title": "Ten sam playbook, każdy lokal",
          "description": "To, co sprawdza się w Lokalu 4, przenosi się do Lokalu 7. Standardy obowiązujące w całym regionie pozostają rygorystyczne bez notatki.",
          "icon": "performance"
        },
        {
          "title": "Odzyskaj wyciek, o którym nie miałeś pojęcia",
          "description": "1-3% przychodów zazwyczaj kryje się w anulacjach, gratisach i nadpisaniach. Sundae wydobywa to zmiana po zmianie.",
          "icon": "owners"
        }
      ],
      "ctaTitle": "Zobacz swój region w jednym widoku.",
      "ctaDescription": "30 minut. Twoje lokalizacje. Zmiana, z którą poradziłbyś sobie inaczej.",
      "ctaButton": "Zarezerwuj wycieczkę regionalną"
    },
    "tr": {
      "badge": "Bölge ve Bölge Yöneticileri için",
      "titleLine1": "On iki şube yönetin.",
      "titleLine2": "Sanki içlerinden birinde duruyormuşsunuz gibi.",
      "description": "Her lokasyonda canlı vardiya görünürlüğü. Pacing, şu anda size ihtiyacı olan şubeyi işaretler. Cuma özetini beklemenize gerek yok.",
      "primaryCta": "Bölgesel İnceleme Rezervasyonu Yapın",
      "secondaryCta": "Ops Demosunu görün",
      "problemsEyebrow": "SİZİ NE YAVAŞLATIR",
      "problemsTitle": "Bir bölge müdürünü yavaşlatan şey nedir?",
      "problemsDescription": "Çok fazla site, yeterli zaman yok, olaydan sonra çok fazla veri var.",
      "challenges": [
        {
          "title": "Aynı anda her restorana giremezsiniz",
          "description": "Kötü vardiya sona erdiğinde, bunu sahadan değil, özetten öğrenirsiniz.",
          "icon": "operators"
        },
        {
          "title": "Garson performansı %30+ oranında değişir",
          "description": "En iyi performans gösterenleri ve geride kalanları bilirsiniz. Ama onları eşleştirecek vakti zamanında bilemezsiniz.",
          "icon": "performance"
        },
        {
          "title": "Sızıntı gürültünün içinde gizleniyor",
          "description": "İptaller, ikramlar, indirimler, fazla personel - vardiya başına küçük, bir ay boyunca pahalı.",
          "icon": "balance"
        },
        {
          "title": "Her şube aynı anda yardım ister",
          "description": "Pacing görünürlüğü olmadan, hangi şubenin gerçekten kaydığına göre değil, içgüdüsel olarak önceliklendirme yaparsınız.",
          "icon": "support"
        }
      ],
      "howTitle": "Sundae bölge yöneticileri için nasıl çalışır?",
      "howDescription": "Her vardiyanın, her garsonun, her şubenin canlı görüntüsü.",
      "howSundaeHelps": [
        {
          "title": "Bölge genelinde canlı pacing",
          "description": "Her konumun geliri, misafir sayısı ve işçiliği vardiya boyunca güncellenir. Kayan şube Cuma günkü incelemede değil, sabah brifinginizde işaretlenir.",
          "product": "Pulse",
          "icon": "chart"
        },
        {
          "title": "Garson düzeyinde zeka",
          "description": "Üst satış oranı, masa dönüşü, garson başına ortalama adisyon. Vardiya sona ermeden geride kalan garsonunuzu en güçlü garsonunuzla eşleştirin.",
          "product": "Pulse · Server view",
          "icon": "operators"
        },
        {
          "title": "Vardiyada sızıntı tespit edildi",
          "description": "İptal işlemleri, ikram kümeleri, kalıp dışı indirimler - GM ile konuşmak için hâlâ zaman varken sahada ortaya çıktı.",
          "product": "Insights · Revenue Assurance",
          "icon": "owners"
        },
        {
          "title": "Sundae Coach bir sonraki hamleyi veriyor",
          "description": "Bir site %14 geride kaldığında Coach, daha önce işe yarayanları temel alarak belirli bir oyun (ayarlanmış iş gücü, bağlılık çabası, menü değişimi) öneriyor.",
          "product": "Sundae Intelligence",
          "icon": "intelligence"
        }
      ],
      "outcomesTitle": "Bölge için neler değişiyor?",
      "outcomesDescription": "Daha hızlı müdahale, daha keskin alanlar, daha az Cuma sürprizi.",
      "outcomes": [
        {
          "title": "Vardiyada müdahale edin, sonrasında değil",
          "description": "Size ihtiyaç duyan şube, saha fark etmeden sinyali gönderir. Eylem doğru zamanda doğru şubeye doğru hareket eder.",
          "icon": "speed"
        },
        {
          "title": "Denetim yerine koçluk",
          "description": "Ziyaretinizi raporun kaçırdığı şeyleri kovalayarak harcamayı bırakın. Bu ziyareti takıma bundan sonra ne yapılacağı konusunda koçluk yapmak için kullanın.",
          "icon": "support"
        },
        {
          "title": "Her şubede aynı oyun kitabı",
          "description": "Site 4'te işe yarayan şey, Site 7'ye de yayılır. Bölge çapındaki standartlar, bir not olmadan da sıkı kalır.",
          "icon": "performance"
        },
        {
          "title": "Sahip olduğunuzu bilmediğiniz sızıntıyı kurtarın",
          "description": "Gelirin %1-3'ü genellikle iptallerde, ikramlarda ve geçersiz kılmalarda gizlenir. Sundae bunu her vardiyada yüzeye çıkarır.",
          "icon": "owners"
        }
      ],
      "ctaTitle": "Bölgenizi tek görünümde görün.",
      "ctaDescription": "30 dakika. Konumlarınız. Farklı şekilde ele alacağınız vardiya.",
      "ctaButton": "Bölgesel İnceleme Rezervasyonu Yapın"
    },
    "ko": {
      "badge": "지역 및 지역 관리자용",
      "titleLine1": "12개의 매장을 운영하세요.",
      "titleLine2": "마치 그 안에 서 있는 것처럼.",
      "description": "모든 매장의 실시간 교대 가시성을 제공합니다. 페이싱은 지금 당장 당신이 필요한 매장을 표시합니다. 금요일 요약을 기다릴 필요가 없습니다.",
      "primaryCta": "지역별 체험 예약",
      "secondaryCta": "운영 데모 보기",
      "problemsEyebrow": "당신을 느리게 만드는 것",
      "problemsTitle": "지역 관리자의 속도를 늦추는 요인",
      "problemsDescription": "사이트가 너무 많고, 시간이 충분하지 않으며, 사후 데이터가 너무 많습니다.",
      "challenges": [
        {
          "title": "모든 레스토랑에 동시에 있을 수는 없습니다.",
          "description": "나쁜 교대가 끝날 때쯤이면 현장이 아니라 요약을 통해 알게 됩니다.",
          "icon": "operators"
        },
        {
          "title": "서빙 직원의 성과는 30% 이상 차이가 납니다.",
          "description": "누가 잘하고 누가 뒤처지는지는 알고 있습니다. 하지만 그들을 짝지을 제때를 알지 못합니다.",
          "icon": "performance"
        },
        {
          "title": "누수는 소음 속에 숨어 있습니다.",
          "description": "취소, 무료 제공, 할인, 인력 과잉 - 교대당으로는 작지만 한 달이면 비용이 많이 듭니다.",
          "icon": "balance"
        },
        {
          "title": "모든 매장에서 한꺼번에 도움을 요청합니다",
          "description": "페이싱 가시성이 없으면 실제로 어느 매장이 무너지고 있는지가 아니라 본능에 따라 우선순위를 정하게 됩니다.",
          "icon": "support"
        }
      ],
      "howTitle": "Sundae가 지역 관리자를 위해 작동하는 방식",
      "howDescription": "모든 교대, 모든 서빙 직원, 모든 매장에 대한 하나의 실시간 보기를 제공합니다.",
      "howSundaeHelps": [
        {
          "title": "지역 전체의 실시간 페이싱",
          "description": "각 매장의 매출, 내방객 수, 인건비가 교대 내내 업데이트됩니다. 무너지는 매장은 금요일 리뷰가 아니라 아침 브리핑에 표시됩니다.",
          "product": "Pulse",
          "icon": "chart"
        },
        {
          "title": "서빙 직원 단위의 인텔리전스",
          "description": "업셀 비율, 테이블 회전, 서빙 직원별 객단가. 교대가 끝나기 전에 뒤처지는 직원을 가장 강한 직원과 짝지으세요.",
          "product": "Pulse · Server view",
          "icon": "operators"
        },
        {
          "title": "교대 중에 발견된 누수",
          "description": "취소 처리, 무료 제공 집중, 패턴을 벗어난 할인 등이 아직 GM과 대화할 시간이 있을 때 해당 매장에서 드러납니다.",
          "product": "Insights · Revenue Assurance",
          "icon": "owners"
        },
        {
          "title": "Sundae Coach가 다음 수를 제시합니다",
          "description": "매장 페이싱이 14% 뒤처지면 Coach는 이전에 효과가 있었던 것을 바탕으로 인력 조정, 로열티 강화, 메뉴 교체 등 특정 플레이를 권장합니다.",
          "product": "Sundae Intelligence",
          "icon": "intelligence"
        }
      ],
      "outcomesTitle": "지역에 어떤 변화가 있나요?",
      "outcomesDescription": "더 빠른 개입, 더 날카로운 사이트, 금요일의 놀라움은 더 적습니다.",
      "outcomes": [
        {
          "title": "교대가 끝난 뒤가 아니라 교대 중에 개입하세요",
          "description": "당신이 필요한 매장은 현장이 알아차리기 전에 신호를 보냅니다. 행동은 적시에 적절한 매장으로 이동합니다.",
          "icon": "speed"
        },
        {
          "title": "감사가 아니라 코칭",
          "description": "보고서가 놓친 것을 쫓는 데 방문 시간을 낭비하지 마세요. 방문을 활용해 팀에게 다음에 무엇을 해야 할지 코칭하세요.",
          "icon": "support"
        },
        {
          "title": "동일한 플레이북, 모든 매장",
          "description": "매장 4에서 효과가 있는 것은 매장 7로 전파됩니다. 메모 없이도 지역 전체 표준이 엄격하게 유지됩니다.",
          "icon": "performance"
        },
        {
          "title": "있는 줄 몰랐던 누수 회수",
          "description": "매출의 1~3%는 일반적으로 취소, 무료 제공, 강제 변경에 숨어 있습니다. Sundae는 이를 교대마다 드러냅니다.",
          "icon": "owners"
        }
      ],
      "ctaTitle": "한 번에 해당 지역을 확인하세요.",
      "ctaDescription": "30분. 당신의 매장. 당신이 다르게 처리했을 교대.",
      "ctaButton": "지역별 체험 예약"
    },
    "id": {
      "badge": "Untuk Manajer Regional & Area",
      "titleLine1": "Jalankan dua belas outlet.",
      "titleLine2": "Seperti Anda berdiri di salah satunya.",
      "description": "Visibilitas shift langsung di setiap lokasi. Pacing menandai gerai yang membutuhkan Anda saat ini. Tidak perlu menunggu rekap hari Jumat.",
      "primaryCta": "Pesan Walk-through Regional",
      "secondaryCta": "Lihat Demo Operasi",
      "problemsEyebrow": "APA YANG MEMPERLAMBAT ANDA",
      "problemsTitle": "Apa yang memperlambat manajer regional",
      "problemsDescription": "Terlalu banyak situs, tidak cukup waktu, terlalu banyak data setelahnya.",
      "challenges": [
        {
          "title": "Anda tidak bisa berada di setiap restoran sekaligus",
          "description": "Pada saat shift yang buruk selesai, Anda mengetahuinya dari rekap - bukan dari lantai.",
          "icon": "operators"
        },
        {
          "title": "Kinerja pramusaji bervariasi sebesar 30%+",
          "description": "Anda tahu siapa yang berkinerja terbaik dan yang lamban. Tetapi Anda tidak tahu tepat waktu untuk memasangkannya.",
          "icon": "performance"
        },
        {
          "title": "Kebocoran bersembunyi di balik kebisingan",
          "description": "Pembatalan, traktiran, diskon, kelebihan staf - kecil per shift, mahal dalam sebulan.",
          "icon": "balance"
        },
        {
          "title": "Setiap gerai meminta bantuan sekaligus",
          "description": "Tanpa visibilitas pacing, Anda menentukan prioritas berdasarkan insting, bukan berdasarkan gerai mana yang sebenarnya merosot.",
          "icon": "support"
        }
      ],
      "howTitle": "Bagaimana Sundae bekerja untuk manajer regional",
      "howDescription": "Satu tampilan langsung setiap shift, setiap pramusaji, setiap gerai.",
      "howSundaeHelps": [
        {
          "title": "Pacing langsung di seluruh wilayah",
          "description": "Pendapatan, jumlah tamu, dan tenaga kerja setiap lokasi - diperbarui sepanjang shift. Gerai yang merosot akan ditandai dalam ringkasan pagi Anda, bukan ulasan hari Jumat.",
          "product": "Pulse",
          "icon": "chart"
        },
        {
          "title": "Kecerdasan tingkat pramusaji",
          "description": "Tingkat penjualan tambahan, perputaran meja, rata-rata tagihan per pramusaji. Pasangkan pramusaji Anda yang tertinggal dengan yang terkuat sebelum shift berakhir.",
          "product": "Pulse · Server view",
          "icon": "operators"
        },
        {
          "title": "Kebocoran terlihat di shift",
          "description": "Transaksi batal, kelompok traktiran, diskon di luar pola - muncul di lokasi selagi masih ada waktu untuk berbicara dengan GM.",
          "product": "Insights · Revenue Assurance",
          "icon": "owners"
        },
        {
          "title": "Pelatih Sundae memberikan langkah selanjutnya",
          "description": "Ketika sebuah situs tertinggal 14%, Coach merekomendasikan permainan tertentu - penyesuaian tenaga kerja, dorongan loyalitas, pertukaran menu - berdasarkan apa yang telah berhasil sebelumnya.",
          "product": "Sundae Intelligence",
          "icon": "intelligence"
        }
      ],
      "outcomesTitle": "Perubahan apa yang terjadi di wilayah ini",
      "outcomesDescription": "Intervensi lebih cepat, lokasi lebih tajam, lebih sedikit kejutan di hari Jumat.",
      "outcomes": [
        {
          "title": "Lakukan intervensi pada shift, bukan setelahnya",
          "description": "Lokasi yang membutuhkan Anda mengirimkan sinyal sebelum lantai mengetahuinya. Tindakan berpindah ke gerai yang tepat pada waktu yang tepat.",
          "icon": "speed"
        },
        {
          "title": "Melatih, bukan mengaudit",
          "description": "Berhentilah menghabiskan kunjungan untuk mengejar hal-hal yang terlewatkan dalam laporan. Gunakan kunjungan ini untuk melatih tim tentang apa yang harus dilakukan selanjutnya.",
          "icon": "support"
        },
        {
          "title": "Pedoman yang sama, di setiap gerai",
          "description": "Apa yang berhasil di Situs 4 menyebar ke Situs 7. Standar-standar di seluruh wilayah tetap ketat tanpa adanya memo.",
          "icon": "performance"
        },
        {
          "title": "Pulihkan kebocoran yang tidak Anda sadari",
          "description": "1-3% dari pendapatan biasanya tersembunyi dalam pembatalan, traktiran, dan penggantian. Sundae memunculkannya shift demi shift.",
          "icon": "owners"
        }
      ],
      "ctaTitle": "Lihat wilayah Anda dalam satu tampilan.",
      "ctaDescription": "30 menit. Lokasi Anda. Shift yang akan Anda tangani secara berbeda.",
      "ctaButton": "Pesan Walk-through Regional"
    },
    "ja": {
      "badge": "地域およびエリアマネージャー向け",
      "titleLine1": "12店舗を運営。",
      "titleLine2": "まるでその一つに立っているかのように。",
      "description": "すべての店舗のライブなシフト可視性。ペーシングが、今あなたを必要としている店舗にフラグを立てます。金曜日の総括を待つ必要はありません。",
      "primaryCta": "地域ウォークスルーを予約する",
      "secondaryCta": "運用デモを見る",
      "problemsEyebrow": "何があなたの速度を遅らせているのか",
      "problemsTitle": "地域マネージャーの仕事を遅らせるもの",
      "problemsDescription": "サイトが多すぎる、時間が足りない、事後のデータが多すぎる。",
      "challenges": [
        {
          "title": "一度にすべてのレストランに行くことはできません",
          "description": "悪いシフトが終わる頃には、フロアではなく要約からそれが分かります。",
          "icon": "operators"
        },
        {
          "title": "接客スタッフのパフォーマンスは 30% 以上ばらつきます",
          "description": "誰が優秀で誰が出遅れているかは分かっています。でも、組ませるのに間に合うタイミングでは分からないのです。",
          "icon": "performance"
        },
        {
          "title": "漏れはノイズに隠れる",
          "description": "取消、サービス提供、割引、過剰人員 - シフトごとには少額ですが、1 か月にすると高額になります。",
          "icon": "balance"
        },
        {
          "title": "すべての店舗が同時に助けを求めます",
          "description": "ペーシングの可視性がなければ、実際にどの店舗が崩れているかではなく、勘に基づいて優先順位を付けることになります。",
          "icon": "support"
        }
      ],
      "howTitle": "地域マネージャーにとってSundaeがどのように機能するか",
      "howDescription": "すべてのシフト、すべての接客スタッフ、すべての店舗を1つにまとめたライブビュー。",
      "howSundaeHelps": [
        {
          "title": "地域全体でのライブペーシング",
          "description": "各拠点の収益、客数、人件費がシフトを通じて更新されます。崩れている店舗は金曜日のレビューではなく、朝のブリーフで報告されます。",
          "product": "Pulse",
          "icon": "chart"
        },
        {
          "title": "接客スタッフ単位のインテリジェンス",
          "description": "アップセル率、テーブル回転、接客スタッフごとの客単価を確認します。シフトが終わる前に、遅れているスタッフを最も強いスタッフと組ませてください。",
          "product": "Pulse · Server view",
          "icon": "operators"
        },
        {
          "title": "シフト中に漏れが発見されました",
          "description": "ボイドラン、コンプクラスター、オフパターン割引 - GM と話す時間がまだあるうちに、その場所で表面化しました。",
          "product": "Insights · Revenue Assurance",
          "icon": "owners"
        },
        {
          "title": "Sundaeコーチが次の一手を出す",
          "description": "サイトのペースが 14% 遅れている場合、コーチは以前に機能していたものに基づいて、労働力の調整、ロイヤルティの推進、メニューの交換などの特定の戦略を推奨します。",
          "product": "Sundae Intelligence",
          "icon": "intelligence"
        }
      ],
      "outcomesTitle": "地域にとって何が変わるのか",
      "outcomesDescription": "より迅速な介入、より鋭い現場、金曜日のサプライズの減少。",
      "outcomes": [
        {
          "title": "シフト後ではなく、シフト中に介入する",
          "description": "あなたを必要としている店舗は、フロアが気づく前に信号を送ります。アクションは適切なタイミングで適切な店舗に届きます。",
          "icon": "speed"
        },
        {
          "title": "監査ではなくコーチ",
          "description": "報告書が見逃していたことを追いかけて訪問に時間を費やすのはやめましょう。この訪問を利用して、次に何をすべきかをチームに指導します。",
          "icon": "support"
        },
        {
          "title": "すべての店舗で同じプレイブック",
          "description": "サイト 4 で機能するものはサイト 7 に伝播します。地域全体の標準はメモなしでも厳格なままです。",
          "icon": "performance"
        },
        {
          "title": "気づかなかった漏れを取り戻す",
          "description": "通常、収益の 1 ～ 3% は取消、サービス提供、上書きに隠れています。Sundaeはそれをシフトごとに浮かび上がらせます。",
          "icon": "owners"
        }
      ],
      "ctaTitle": "お住まいの地域を 1 つのビューで確認できます。",
      "ctaDescription": "30分。あなたの場所。別の方法で処理したであろうシフト。",
      "ctaButton": "地域ウォークスルーを予約する"
    },
    "vi": {
      "badge": "Dành cho quản lý vùng và quản lý khu vực",
      "titleLine1": "Vận hành mười hai cửa hàng.",
      "titleLine2": "Như thể bạn đang đứng trong một cửa hàng.",
      "description": "Khả năng hiển thị ca trực tiếp trên mọi địa điểm. Nhịp độ đánh dấu cửa hàng cần bạn ngay bây giờ. Không phải chờ đến bản tóm tắt thứ Sáu.",
      "primaryCta": "Đặt chuyến tham quan khu vực",
      "secondaryCta": "Xem bản demo hoạt động",
      "problemsEyebrow": "ĐIỀU GÌ LÀM CHẬM BẠN",
      "problemsTitle": "Điều gì khiến người quản lý khu vực chậm lại",
      "problemsDescription": "Quá nhiều trang web, không đủ thời gian, quá nhiều dữ liệu sau thực tế.",
      "challenges": [
        {
          "title": "Bạn không thể có mặt ở mọi nhà hàng cùng một lúc",
          "description": "Vào thời điểm ca làm việc tồi tệ kết thúc, bạn sẽ phát hiện ra điều đó từ bản tóm tắt - không phải từ sàn nhà.",
          "icon": "operators"
        },
        {
          "title": "Hiệu suất nhân viên phục vụ chênh lệch từ 30% trở lên",
          "description": "Bạn biết ai làm tốt nhất và ai tụt lại. Nhưng bạn không biết kịp lúc để ghép họ với nhau.",
          "icon": "performance"
        },
        {
          "title": "Rò rỉ ẩn trong tiếng ồn",
          "description": "Hủy đơn, suất mời, giảm giá, thừa nhân sự - nhỏ mỗi ca, đắt đỏ trong một tháng.",
          "icon": "balance"
        },
        {
          "title": "Mọi cửa hàng đều cần trợ giúp cùng một lúc",
          "description": "Nếu không có khả năng hiển thị nhịp độ, bạn sẽ ưu tiên theo bản năng chứ không phải theo cửa hàng thực sự đang sa sút.",
          "icon": "support"
        }
      ],
      "howTitle": "Sundae hoạt động như thế nào đối với các nhà quản lý khu vực",
      "howDescription": "Một chế độ xem trực tiếp về mọi ca, mọi nhân viên phục vụ, mọi cửa hàng.",
      "howSundaeHelps": [
        {
          "title": "Nhịp độ trực tiếp trên toàn khu vực",
          "description": "Doanh thu, số khách và nhân công của mỗi địa điểm - được cập nhật suốt ca làm việc. Cửa hàng đang sa sút sẽ được gắn cờ trong bản tóm tắt buổi sáng của bạn chứ không phải bài đánh giá vào thứ Sáu.",
          "product": "Pulse",
          "icon": "chart"
        },
        {
          "title": "Thông tin cấp nhân viên phục vụ",
          "description": "Tỷ lệ bán thêm, xoay bàn, hóa đơn trung bình theo nhân viên phục vụ. Ghép nhân viên phục vụ đang tụt lại với người mạnh nhất của bạn trước khi ca làm việc kết thúc.",
          "product": "Pulse · Server view",
          "icon": "operators"
        },
        {
          "title": "Phát hiện rò rỉ trong ca làm việc",
          "description": "Các giao dịch hủy, cụm suất mời, giảm giá không theo mẫu - xuất hiện tại địa điểm trong khi vẫn còn thời gian để nói chuyện với GM.",
          "product": "Insights · Revenue Assurance",
          "icon": "owners"
        },
        {
          "title": "Huấn luyện viên Sundae đưa ra động thái tiếp theo",
          "description": "Khi một trang web có tốc độ chậm 14%, Huấn luyện viên đề xuất một cách chơi cụ thể - lao động được điều chỉnh, thúc đẩy lòng trung thành, hoán đổi menu - dựa trên những gì đã hoạt động trước đó.",
          "product": "Sundae Intelligence",
          "icon": "intelligence"
        }
      ],
      "outcomesTitle": "Những thay đổi nào đối với khu vực",
      "outcomesDescription": "Can thiệp nhanh hơn, trang web sắc nét hơn, ít bất ngờ hơn vào thứ Sáu.",
      "outcomes": [
        {
          "title": "Can thiệp trong ca, không phải sau ca",
          "description": "Cửa hàng cần bạn gửi tín hiệu trước khi sàn nhận ra. Hành động chuyển đến đúng cửa hàng vào đúng thời điểm.",
          "icon": "speed"
        },
        {
          "title": "Huấn luyện thay vì kiểm toán",
          "description": "Hãy ngừng dành chuyến thăm để theo đuổi những gì báo cáo đã bỏ lỡ. Hãy dùng chuyến thăm để huấn luyện nhóm về những việc cần làm tiếp theo.",
          "icon": "support"
        },
        {
          "title": "Cùng một playbook, mọi cửa hàng",
          "description": "Những gì hiệu quả ở Địa điểm 4 sẽ lan đến Địa điểm 7. Các tiêu chuẩn toàn khu vực được giữ chặt chẽ mà không cần bản ghi nhớ.",
          "icon": "performance"
        },
        {
          "title": "Khôi phục khoản rò rỉ mà bạn không biết mình đang có",
          "description": "1-3% doanh thu thường ẩn trong các giao dịch hủy, suất mời và phần ghi đè. Sundae đưa nó ra ánh sáng theo từng ca.",
          "icon": "owners"
        }
      ],
      "ctaTitle": "Xem khu vực của bạn trong một chế độ xem.",
      "ctaDescription": "30 phút. Các địa điểm của bạn. Ca làm việc mà bạn sẽ xử lý theo cách khác.",
      "ctaButton": "Đặt chuyến tham quan khu vực"
    },
    "bn": {
      "badge": "আঞ্চলিক ও এরিয়া ম্যানেজারদের জন্য",
      "titleLine1": "বারোটি আউটলেট চালান।",
      "titleLine2": "যেন আপনি তাদেরই একটিতে দাঁড়িয়ে আছেন।",
      "description": "প্রতিটি অবস্থান জুড়ে লাইভ শিফট দৃশ্যমানতা। পেসিং সেই আউটলেটটিকে চিহ্নিত করে যেটির এই মুহূর্তে আপনাকে প্রয়োজন। শুক্রবারের সারাংশের জন্য অপেক্ষা নেই।",
      "primaryCta": "একটি আঞ্চলিক ওয়াক-থ্রু বুক করুন",
      "secondaryCta": "অপস ডেমো দেখুন",
      "problemsEyebrow": "যা আপনাকে ধীর করে দেয়",
      "problemsTitle": "কোন আঞ্চলিক ব্যবস্থাপককে ধীর করে দেয়",
      "problemsDescription": "অনেক বেশি সাইট, পর্যাপ্ত সময় নেই, ঘটনার পরে অনেক বেশি ডেটা।",
      "challenges": [
        {
          "title": "আপনি একবারে প্রতিটি রেস্টুরেন্টে থাকতে পারবেন না",
          "description": "খারাপ শিফট শেষ হওয়ার মধ্যে, আপনি সারাংশ থেকে জানতে পারেন - ফ্লোর থেকে নয়।",
          "icon": "operators"
        },
        {
          "title": "ওয়েটারের পারফরম্যান্স 30%+ পর্যন্ত ভিন্ন হয়",
          "description": "আপনি সেরা পারফর্মার এবং পিছিয়ে পড়াদের জানেন। কিন্তু তাদের জোড়া বাঁধার জন্য সময়মতো তা জানেন না।",
          "icon": "performance"
        },
        {
          "title": "ফুটো আওয়াজে লুকিয়ে থাকে",
          "description": "বাতিল, ফ্রি পরিবেশন, ডিসকাউন্ট, অতিরিক্ত কর্মী - প্রতি শিফটে ছোট, এক মাসে ব্যয়বহুল।",
          "icon": "balance"
        },
        {
          "title": "প্রতিটি আউটলেট একসাথে সাহায্য চায়",
          "description": "পেসিং দৃশ্যমানতা ছাড়া, আপনি প্রবৃত্তির উপর অগ্রাধিকার দেন, আসলে কোন আউটলেট পিছলে যাচ্ছে তার উপর নয়।",
          "icon": "support"
        }
      ],
      "howTitle": "কিভাবে Sundae আঞ্চলিক পরিচালকদের জন্য কাজ করে",
      "howDescription": "প্রতিটি শিফট, প্রতিটি ওয়েটার, প্রতিটি আউটলেটের একটি লাইভ ভিউ।",
      "howSundaeHelps": [
        {
          "title": "অঞ্চল জুড়ে লাইভ পেসিং",
          "description": "প্রতিটি অবস্থানের আয়, অতিথি এবং শ্রম - শিফট জুড়ে আপডেট করা হয়। যে আউটলেটটি পিছলে যাচ্ছে সেটি আপনার সকালের সংক্ষিপ্ত বিবরণে চিহ্নিত হবে, শুক্রবারের পর্যালোচনায় নয়।",
          "product": "Pulse",
          "icon": "chart"
        },
        {
          "title": "ওয়েটার-স্তরের বুদ্ধিমত্তা",
          "description": "আপসেল রেট, টেবিল টার্ন, প্রতি ওয়েটার গড় বিল। শিফট শেষ হওয়ার আগে আপনার পিছিয়ে পড়া ওয়েটারকে আপনার সবচেয়ে শক্তিশালী ওয়েটারের সাথে জোড়া বাঁধুন।",
          "product": "Pulse · Server view",
          "icon": "operators"
        },
        {
          "title": "শিফটে ফুটো দেখা গেছে",
          "description": "বাতিল লেনদেন, ফ্রি পরিবেশনের ক্লাস্টার, প্যাটার্নের বাইরের ডিসকাউন্ট - জিএম-এর সাথে কথা বলার সময় থাকতেই লোকেশনে দেখা গেছে।",
          "product": "Insights · Revenue Assurance",
          "icon": "owners"
        },
        {
          "title": "Sundae কোচ পরবর্তী পদক্ষেপ দেয়",
          "description": "যখন একটি আউটলেট 14% পিছিয়ে যায়, তখন Coach একটি নির্দিষ্ট পদক্ষেপের সুপারিশ করে - সমন্বিত কর্মী, একটি লয়্যালটি পুশ, একটি মেনু অদলবদল - আগে যা কাজ করেছে তার উপর ভিত্তি করে।",
          "product": "Sundae Intelligence",
          "icon": "intelligence"
        }
      ],
      "outcomesTitle": "অঞ্চলের জন্য কি পরিবর্তন",
      "outcomesDescription": "দ্রুত হস্তক্ষেপ, তীক্ষ্ণ সাইট, কম শুক্রবারের চমক।",
      "outcomes": [
        {
          "title": "শিফটে হস্তক্ষেপ করুন, পরে নয়",
          "description": "যে আউটলেটের আপনাকে প্রয়োজন সেটি ফ্লোর জানার আগেই সংকেত পাঠায়। অ্যাকশন সঠিক সময়ে সঠিক আউটলেটে চলে যায়।",
          "icon": "speed"
        },
        {
          "title": "অডিটের বদলে কোচিং",
          "description": "রিপোর্ট যা মিস করেছে তার পেছনে ছুটে সফর ব্যয় করা বন্ধ করুন। পরবর্তীতে কী করতে হবে তা দলকে শেখানোর জন্য সফরটি ব্যবহার করুন।",
          "icon": "support"
        },
        {
          "title": "একই প্লেবুক, প্রতিটি আউটলেট",
          "description": "সাইট 4-এ যা কাজ করে তা সাইট 7-এ ছড়িয়ে পড়ে। কোনো মেমো ছাড়াই অঞ্চল-ব্যাপী মানগুলি কঠোর থাকে।",
          "icon": "performance"
        },
        {
          "title": "যে লিকেজের কথা আপনি জানতেনই না তা পুনরুদ্ধার করুন",
          "description": "রাজস্বের 1-3% সাধারণত বাতিল, ফ্রি পরিবেশন এবং ওভাররাইডে লুকিয়ে থাকে। Sundae একে শিফট-বাই-শিফট সামনে আনে।",
          "icon": "owners"
        }
      ],
      "ctaTitle": "এক দৃষ্টিতে আপনার অঞ্চল দেখুন।",
      "ctaDescription": "30 মিনিট। আপনার অবস্থানগুলি। যে শিফটটি আপনি অন্যভাবে পরিচালনা করতেন।",
      "ctaButton": "একটি আঞ্চলিক ওয়াক-থ্রু বুক করুন"
    },
    "ro": {
      "badge": "Pentru managerii regionali și de zonă",
      "titleLine1": "Conduceți douăsprezece locații.",
      "titleLine2": "De parcă ai sta într-una dintre ele.",
      "description": "Vizibilitate live a turelor în fiecare locație. Pacing semnalează locația care are nevoie de tine chiar acum. Fără așteptări pentru recapitularea de vineri.",
      "primaryCta": "Rezervați o prezentare regională",
      "secondaryCta": "Vedeți demonstrația operațiunilor",
      "problemsEyebrow": "CE TE ÎNCETINEȘTE",
      "problemsTitle": "Ceea ce încetinește un manager regional",
      "problemsDescription": "Prea multe site-uri, nu suficient timp, prea multe date după fapt.",
      "challenges": [
        {
          "title": "Nu poți fi în fiecare restaurant deodată",
          "description": "Când s-a terminat tura proastă, afli din recapitulare - nu din sală.",
          "icon": "operators"
        },
        {
          "title": "Performanța ospătarilor variază cu 30%+",
          "description": "Îi cunoști pe cei mai buni și pe cei rămași în urmă. Dar nu la timp ca să-i împerechezi.",
          "icon": "performance"
        },
        {
          "title": "Scurgerea se ascunde în zgomot",
          "description": "Anulări, băuturi din partea casei, reduceri, personal în exces - mici pe tură, scumpe pe o lună.",
          "icon": "balance"
        },
        {
          "title": "Fiecare locație solicită ajutor deodată",
          "description": "Fără vizibilitate asupra ritmului, prioritizezi din instinct, nu după locația care alunecă de fapt.",
          "icon": "support"
        }
      ],
      "howTitle": "Cum funcționează Sundae pentru managerii regionali",
      "howDescription": "O vizualizare live pentru fiecare tură, fiecare ospătar, fiecare locație.",
      "howSundaeHelps": [
        {
          "title": "Ritmul live în întreaga regiune",
          "description": "Veniturile, clienții și forța de muncă ale fiecărei locații - actualizate pe parcursul turei. Locația care alunecă este semnalată în rezumatul de dimineață, nu în recenzia de vineri.",
          "product": "Pulse",
          "icon": "chart"
        },
        {
          "title": "Inteligență la nivel de ospătar",
          "description": "Rata de upsell, rotația meselor, nota medie pe ospătar. Asociați-vă ospătarul rămas în urmă cu cel mai puternic înainte de sfârșitul turei.",
          "product": "Pulse · Server view",
          "icon": "operators"
        },
        {
          "title": "Scurgeri observate în tură",
          "description": "Tranzacții anulate, grupuri de băuturi din partea casei, reduceri în afara tiparului - au apărut la locație cât mai este timp să vorbiți cu GM.",
          "product": "Insights · Revenue Assurance",
          "icon": "owners"
        },
        {
          "title": "Antrenorul Sundae dă următoarea mișcare",
          "description": "Când un site este cu 14% în urmă, antrenorul recomandă un anumit joc - forță de muncă ajustată, un impuls de loialitate, un schimb de meniu - pe baza a ceea ce a funcționat înainte.",
          "product": "Sundae Intelligence",
          "icon": "intelligence"
        }
      ],
      "outcomesTitle": "Ce se schimbă pentru regiune",
      "outcomesDescription": "Intervenție mai rapidă, site-uri mai clare, mai puține surprize de vineri.",
      "outcomes": [
        {
          "title": "Intervine în tură, nu după",
          "description": "Locația care are nevoie de tine trimite semnalul înainte ca sala să știe. Acțiunea se deplasează la locația potrivită la momentul potrivit.",
          "icon": "speed"
        },
        {
          "title": "Antrenare în loc de audit",
          "description": "Nu mai petreceți vizita urmărind ceea ce a omis raportul. Folosiți vizita pentru a instrui echipa cu privire la ce trebuie să faceți în continuare.",
          "icon": "support"
        },
        {
          "title": "Același playbook, fiecare locație",
          "description": "Ceea ce funcționează la Locația 4 se propagă la Locația 7. Standardele la nivel de regiune rămân stricte fără o notă.",
          "icon": "performance"
        },
        {
          "title": "Recuperați scurgerile pe care nu știați că le aveți",
          "description": "1-3% din venituri se ascunde de obicei în anulări, băuturi din partea casei și suprascrieri. Sundae le scoate la suprafață tură cu tură.",
          "icon": "owners"
        }
      ],
      "ctaTitle": "Vedeți regiunea dvs. într-o singură vizualizare.",
      "ctaDescription": "30 de minute. Locațiile dvs. Tura pe care ai fi gestionat-o altfel.",
      "ctaButton": "Rezervați o prezentare regională"
    },
    "sv": {
      "badge": "För region- och områdeschefer",
      "titleLine1": "Driv tolv enheter.",
      "titleLine2": "Som om du stod i en av dem.",
      "description": "Live-skiftsynlighet över alla platser. Pacing flaggar enheten som behöver dig just nu. Ingen väntan på fredagens sammanfattning.",
      "primaryCta": "Boka en regional genomgång",
      "secondaryCta": "Se Ops Demo",
      "problemsEyebrow": "VAD BROMSAR DIG",
      "problemsTitle": "Det som bromsar en regionchef",
      "problemsDescription": "För många webbplatser, inte tillräckligt med tid, för mycket data i efterhand.",
      "challenges": [
        {
          "title": "Du kan inte vara på varje restaurang samtidigt",
          "description": "När det dåliga skiftet är över får du reda på det från sammanfattningen - inte golvet.",
          "icon": "operators"
        },
        {
          "title": "Servitörernas prestanda varierar med 30 %+",
          "description": "Du känner topppresterarna och eftersläntrarna. Du vet det bara inte i tid för att para ihop dem.",
          "icon": "performance"
        },
        {
          "title": "Läckage gömmer sig i bruset",
          "description": "Annulleringar, bjudningar, rabatter, överbemanning - små per skift, dyra över en månad.",
          "icon": "balance"
        },
        {
          "title": "Varje enhet ber om hjälp på en gång",
          "description": "Utan insyn i tempot prioriterar du på instinkt, inte efter vilken enhet som faktiskt glider.",
          "icon": "support"
        }
      ],
      "howTitle": "Hur Sundae fungerar för regionchefer",
      "howDescription": "En livevy av varje skift, varje servitör, varje enhet.",
      "howSundaeHelps": [
        {
          "title": "Live pacing över hela regionen",
          "description": "Varje enhets intäkter, gäster och arbete - uppdaterade under skiftet. Enheten som glider flaggas i din morgonrapport, inte fredagens genomgång.",
          "product": "Pulse",
          "icon": "chart"
        },
        {
          "title": "Intelligens på servitörsnivå",
          "description": "Merförsäljningsgrad, bordsrotation, snittnota per servitör. Para ihop din släpande servitör med din starkaste innan skiftet tar slut.",
          "product": "Pulse · Server view",
          "icon": "operators"
        },
        {
          "title": "Läckage upptäckt i skiftet",
          "description": "Annullerade transaktioner, kluster av bjudningar, rabatter utanför mönstret - dök upp på platsen medan det fortfarande finns tid att prata med GM.",
          "product": "Insights · Revenue Assurance",
          "icon": "owners"
        },
        {
          "title": "Sundae Coach ger nästa drag",
          "description": "När en sajt ligger 14 % efter, rekommenderar Coach ett specifikt spel - anpassat arbete, en lojalitetspress, ett menybyte - baserat på vad som har fungerat tidigare.",
          "product": "Sundae Intelligence",
          "icon": "intelligence"
        }
      ],
      "outcomesTitle": "Vad förändras för regionen",
      "outcomesDescription": "Snabbare ingripande, vassare sajter, färre fredagsöverraskningar.",
      "outcomes": [
        {
          "title": "Ingripa i skiftet, inte efter",
          "description": "Enheten som behöver dig skickar signalen innan golvet vet. Åtgärden går till rätt enhet vid rätt tidpunkt.",
          "icon": "speed"
        },
        {
          "title": "Coacha istället för att revidera",
          "description": "Sluta spendera besöket på att jaga det som rapporten missade. Använd besöket för att coacha laget om vad de ska göra härnäst.",
          "icon": "support"
        },
        {
          "title": "Samma spelbok, varje enhet",
          "description": "Det som fungerar på Site 4 sprider sig till Site 7. Regionövergripande standarder förblir snäva utan ett memo.",
          "icon": "performance"
        },
        {
          "title": "Återställ läckage som du inte visste att du hade",
          "description": "1-3 % av intäkterna döljer sig vanligtvis i annulleringar, bjudningar och åsidosättningar. Sundae lyfter fram det skift för skift.",
          "icon": "owners"
        }
      ],
      "ctaTitle": "Se din region i en vy.",
      "ctaDescription": "30 minuter. Dina platser. Skiftet skulle du ha hanterat annorlunda.",
      "ctaButton": "Boka en regional genomgång"
    },
    "ms": {
      "badge": "Untuk Pengurus Wilayah & Kawasan",
      "titleLine1": "Jalankan dua belas cawangan.",
      "titleLine2": "Seperti anda berdiri dalam satu.",
      "description": "Keterlihatan syif langsung merentasi setiap lokasi. Pacing menandakan cawangan yang memerlukan anda sekarang. Tidak perlu menunggu ringkasan Jumaat.",
      "primaryCta": "Tempah Walk-through Serantau",
      "secondaryCta": "Lihat Demo Ops",
      "problemsEyebrow": "APA YANG MEMPERLAMBATKAN ANDA",
      "problemsTitle": "Apa yang melambatkan pengurus wilayah",
      "problemsDescription": "Terlalu banyak tapak, tidak cukup masa, terlalu banyak data selepas fakta.",
      "challenges": [
        {
          "title": "Anda tidak boleh berada di setiap restoran sekaligus",
          "description": "Menjelang syif buruk itu berakhir, anda mengetahuinya daripada ringkasan - bukan di lantai.",
          "icon": "operators"
        },
        {
          "title": "Prestasi pelayan berbeza-beza sebanyak 30%+",
          "description": "Anda tahu siapa yang berprestasi tinggi dan yang ketinggalan. Tetapi anda tidak tahu tepat pada masanya untuk memasangkannya.",
          "icon": "performance"
        },
        {
          "title": "Kebocoran bersembunyi dalam bunyi",
          "description": "Pembatalan, hidangan ihsan, diskaun, lebihan kakitangan - kecil setiap syif, mahal selama sebulan.",
          "icon": "balance"
        },
        {
          "title": "Setiap cawangan meminta bantuan sekaligus",
          "description": "Tanpa keterlihatan pacing, anda menetapkan keutamaan mengikut naluri, bukan pada cawangan yang sebenarnya merosot.",
          "icon": "support"
        }
      ],
      "howTitle": "Cara Sundae berfungsi untuk pengurus wilayah",
      "howDescription": "Satu paparan langsung setiap syif, setiap pelayan, setiap cawangan.",
      "howSundaeHelps": [
        {
          "title": "Pacing secara langsung di seluruh wilayah",
          "description": "Hasil, jumlah tetamu dan tenaga kerja setiap lokasi - dikemas kini sepanjang syif. Cawangan yang merosot akan dibenderakan dalam ringkasan pagi anda, bukan ulasan Jumaat.",
          "product": "Pulse",
          "icon": "chart"
        },
        {
          "title": "Kepintaran peringkat pelayan",
          "description": "Kadar jualan tambahan, pusing ganti meja, purata bil setiap pelayan. Gandingkan pelayan tertinggal anda dengan pelayan terkuat anda sebelum syif tamat.",
          "product": "Pulse · Server view",
          "icon": "operators"
        },
        {
          "title": "Kebocoran dikesan dalam syif",
          "description": "Transaksi batal, kelompok hidangan ihsan, diskaun luar corak - muncul di lokasi sementara masih ada masa untuk bercakap dengan GM.",
          "product": "Insights · Revenue Assurance",
          "icon": "owners"
        },
        {
          "title": "Jurulatih Sundae memberikan langkah seterusnya",
          "description": "Apabila tapak bergerak 14% di belakang, Jurulatih mengesyorkan permainan tertentu - buruh terlaras, tolakan kesetiaan, pertukaran menu - berdasarkan perkara yang telah berjaya sebelum ini.",
          "product": "Sundae Intelligence",
          "icon": "intelligence"
        }
      ],
      "outcomesTitle": "Apakah perubahan bagi rantau ini",
      "outcomesDescription": "Campur tangan yang lebih pantas, tapak yang lebih tajam, lebih sedikit kejutan pada hari Jumaat.",
      "outcomes": [
        {
          "title": "Campur tangan dalam syif, bukan selepas",
          "description": "Lokasi yang memerlukan anda menghantar isyarat sebelum lantai tahu. Tindakan bergerak ke cawangan yang betul pada masa yang betul.",
          "icon": "speed"
        },
        {
          "title": "Membimbing dan bukannya mengaudit",
          "description": "Berhenti menghabiskan lawatan mengejar apa yang terlepas dalam laporan. Gunakan lawatan untuk membimbing pasukan tentang perkara yang perlu dilakukan seterusnya.",
          "icon": "support"
        },
        {
          "title": "Buku permainan yang sama, setiap cawangan",
          "description": "Perkara yang berfungsi di Tapak 4 merebak ke Tapak 7. Piawaian seluruh wilayah kekal ketat tanpa memo.",
          "icon": "performance"
        },
        {
          "title": "Pulihkan kebocoran yang anda tidak sedar anda alami",
          "description": "1-3% daripada hasil biasanya bersembunyi dalam pembatalan, hidangan ihsan dan override. Sundae memaparkannya syif demi syif.",
          "icon": "owners"
        }
      ],
      "ctaTitle": "Lihat kawasan anda dalam satu paparan.",
      "ctaDescription": "30 minit. Lokasi anda. Syif yang anda akan kendalikan secara berbeza.",
      "ctaButton": "Tempah Walk-through Serantau"
    },
    "th": {
      "badge": "สำหรับผู้จัดการระดับภูมิภาคและระดับพื้นที่",
      "titleLine1": "บริหารสิบสองสาขา",
      "titleLine2": "เหมือนคุณกำลังยืนอยู่ในสาขาใดสาขาหนึ่ง",
      "description": "การมองเห็นกะแบบเรียลไทม์ทั่วทุกสาขา Pacing ทำเครื่องหมายสาขาที่ต้องการคุณในตอนนี้ ไม่ต้องรอสรุปวันศุกร์",
      "primaryCta": "จอง Walk-through ระดับภูมิภาค",
      "secondaryCta": "ดูการสาธิต Ops",
      "problemsEyebrow": "อะไรทำให้คุณช้าลง",
      "problemsTitle": "สิ่งที่ทำให้ผู้จัดการระดับภูมิภาคช้าลง",
      "problemsDescription": "มีไซต์มากเกินไป เวลาไม่เพียงพอ ข้อมูลมากเกินไปหลังจากนั้น",
      "challenges": [
        {
          "title": "คุณไม่สามารถไปร้านอาหารทุกแห่งพร้อมกันได้",
          "description": "เมื่อถึงเวลาที่กะแย่ ๆ สิ้นสุดลง คุณจะรู้ได้จากบทสรุป ไม่ใช่จากหน้างาน",
          "icon": "operators"
        },
        {
          "title": "ประสิทธิภาพของพนักงานเสิร์ฟแตกต่างกันไป 30%+",
          "description": "คุณรู้ว่าใครทำผลงานได้ดีและใครล้าหลัง แต่คุณไม่รู้ทันเวลาที่จะจับคู่พวกเขา",
          "icon": "performance"
        },
        {
          "title": "การรั่วไหลซ่อนอยู่ในเสียงรบกวน",
          "description": "การยกเลิก ของแถม ส่วนลด พนักงานเกินจำเป็น - เล็กน้อยต่อกะ แต่แพงเมื่อรวมทั้งเดือน",
          "icon": "balance"
        },
        {
          "title": "ทุกสาขาขอความช่วยเหลือพร้อมกัน",
          "description": "หากไม่มีการมองเห็น Pacing คุณจะจัดลำดับความสำคัญตามสัญชาตญาณ ไม่ใช่ตามสาขาที่กำลังทรุดลงจริง ๆ",
          "icon": "support"
        }
      ],
      "howTitle": "Sundae ทำงานอย่างไรสำหรับผู้จัดการระดับภูมิภาค",
      "howDescription": "มุมมองเรียลไทม์เดียวของทุกกะ ทุกพนักงานเสิร์ฟ ทุกสาขา",
      "howSundaeHelps": [
        {
          "title": "Pacing แบบเรียลไทม์ทั่วทั้งภูมิภาค",
          "description": "รายได้ จำนวนลูกค้า และแรงงานของแต่ละสาขา - อัปเดตตลอดทั้งกะ สาขาที่กำลังทรุดลงจะถูกทำเครื่องหมายในสรุปตอนเช้าของคุณ ไม่ใช่บทวิจารณ์ในวันศุกร์",
          "product": "Pulse",
          "icon": "chart"
        },
        {
          "title": "ข้อมูลอัจฉริยะระดับพนักงานเสิร์ฟ",
          "description": "อัตราการขายต่อยอด อัตราการหมุนโต๊ะ ยอดบิลเฉลี่ยตามพนักงานเสิร์ฟ จับคู่พนักงานเสิร์ฟที่ล้าหลังของคุณกับพนักงานที่แข็งแกร่งที่สุดก่อนที่กะจะสิ้นสุดลง",
          "product": "Pulse · Server view",
          "icon": "operators"
        },
        {
          "title": "พบการรั่วไหลในกะ",
          "description": "รายการที่ถูกยกเลิก กลุ่มของแถม ส่วนลดนอกรูปแบบ - ปรากฏที่สาขาในขณะที่ยังมีเวลาพูดคุยกับ GM",
          "product": "Insights · Revenue Assurance",
          "icon": "owners"
        },
        {
          "title": "Sundaeโค้ชให้การเคลื่อนไหวต่อไป",
          "description": "เมื่อสาขาตามหลังอยู่ 14% Coach จะแนะนำแนวทางที่เฉพาะเจาะจง - ปรับกำลังคน ผลักดันความภักดี สลับเมนู - โดยพิจารณาจากสิ่งที่เคยได้ผลมาก่อน",
          "product": "Sundae Intelligence",
          "icon": "intelligence"
        }
      ],
      "outcomesTitle": "การเปลี่ยนแปลงอะไรสำหรับภูมิภาค",
      "outcomesDescription": "การแทรกแซงเร็วขึ้น เว็บไซต์คมชัดขึ้น เซอร์ไพรส์ในวันศุกร์น้อยลง",
      "outcomes": [
        {
          "title": "เข้าแทรกแซงระหว่างกะ ไม่ใช่หลังจากนั้น",
          "description": "สาขาที่ต้องการคุณจะส่งสัญญาณก่อนที่หน้างานจะรู้ การลงมือเคลื่อนไปยังสาขาที่ถูกต้องในเวลาที่เหมาะสม",
          "icon": "speed"
        },
        {
          "title": "โค้ชแทนการตรวจสอบ",
          "description": "หยุดใช้เวลาเยี่ยมสาขาเพื่อไล่ตามสิ่งที่รายงานพลาดไป ใช้การมาเยือนเพื่อฝึกสอนทีมว่าจะทำอย่างไรต่อไป",
          "icon": "support"
        },
        {
          "title": "Playbook เดียวกันทุกสาขา",
          "description": "สิ่งที่ใช้ได้ผลในสาขา 4 จะแพร่ไปยังสาขา 7 มาตรฐานทั่วทั้งภูมิภาคยังคงเข้มงวดโดยไม่ต้องมีบันทึกช่วยจำ",
          "icon": "performance"
        },
        {
          "title": "กู้คืนการรั่วไหลที่คุณไม่รู้ว่ามีอยู่",
          "description": "โดยทั่วไปแล้ว 1-3% ของรายได้จะซ่อนอยู่ในการยกเลิก ของแถม และการแทนที่ Sundae จะเผยให้เห็นทีละกะ",
          "icon": "owners"
        }
      ],
      "ctaTitle": "ดูภูมิภาคของคุณในมุมมองเดียว",
      "ctaDescription": "30 นาที สาขาของคุณ กะที่คุณจะจัดการแตกต่างออกไป",
      "ctaButton": "จอง Walk-through ระดับภูมิภาค"
    }
  }
} as const
