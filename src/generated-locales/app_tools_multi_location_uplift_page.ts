// Generated website local copy overrides for src/app/tools/multi-location-uplift/page.tsx.
// Do not edit by hand; regenerate from the English local copy source.
export const generatedLocalCopy = {
  "localizedCopy": {
    "de": {
      "back": "Zurück zu den Tools",
      "hero": {
        "badge": "Portfolio-Intelligenz",
        "title": "Umsatzsteigerungsrechner für mehrere Standorte",
        "description": "Berechnen Sie die potenziellen Umsatzwirkungen von operativen Verbesserungen an mehreren Standorten."
      },
      "form": {
        "title": "Geben Sie Ihre Daten für mehrere Standorte ein",
        "description": "Schätzen Sie die Umsatzwirkung von Leistungsverbesserungen.",
        "locationsLabel": "Anzahl der Standorte",
        "locationsPlaceholder": "z.B. 10",
        "revenueLabel": "Durchschnittlicher monatlicher Umsatz pro Standort (€)",
        "revenuePlaceholder": "z.B. 50000",
        "improvementLabel": "Zielverbesserung (%)",
        "improvementPlaceholder": "z.B. 5",
        "tip": "Typische Verbesserungen liegen zwischen 2-10% bei systematischer Optimierung.",
        "button": "Umsatzsteigerung berechnen"
      },
      "results": {
        "title": "Geschätzte Umsatzwirkung",
        "currentRevenue": "Aktueller Jahresumsatz",
        "potentialUplift": "Potenzielle jährliche Steigerung",
        "upliftPerLocation": "Steigerung pro Standort",
        "timeframe": "Erwarteter Zeitraum",
        "areasTitle": "Wichtige Verbesserungsbereiche",
        "labor": "Arbeitsoptimierung",
        "menu": "Menügestaltung",
        "operations": "Betriebliche Exzellenz",
        "benchmarking": "Standort-Benchmarking",
        "note": "Diese Schätzungen basieren auf Branchenbenchmarks. Tatsächliche Ergebnisse hängen von den aktuellen Leistungsniveaus, Marktbedingungen und der Ausführungsqualität ab. Sundae hilft, Chancen an allen Standorten zu identifizieren und zu verfolgen."
      },
      "timeframe": {
        "small": "3-6 Monate mit gezielten operativen Verbesserungen",
        "medium": "6-12 Monate mit konsequenter Optimierung",
        "large": "12-18 Monate mit strategischen Initiativen",
        "xlarge": "18-24 Monate mit umfassenden Transformationen"
      },
      "cta": {
        "intro": "Möchten Sie spezifische Verbesserungschancen an Ihren Standorten identifizieren?",
        "button": "Sehen Sie, wie Sundae helfen kann"
      },
      "errors": {
        "invalidNumbers": "Bitte geben Sie gültige Zahlen ein"
      }
    },
    "fr": {
      "back": "Retour aux outils",
      "hero": {
        "badge": "Intelligence de Portefeuille",
        "title": "Estimateur d'Augmentation de Revenus Multi-Emplacements",
        "description": "Calculez l'impact potentiel des améliorations opérationnelles sur plusieurs emplacements."
      },
      "form": {
        "title": "Entrez Vos Données Multi-Emplacements",
        "description": "Estimez l'impact sur les revenus des améliorations de performance.",
        "locationsLabel": "Nombre d'Emplacements",
        "locationsPlaceholder": "ex. 10",
        "revenueLabel": "Revenu Mensuel Moyen par Emplacement (€)",
        "revenuePlaceholder": "ex. 50000",
        "improvementLabel": "Amélioration Ciblée (%)",
        "improvementPlaceholder": "ex. 5",
        "tip": "Les améliorations typiques varient de 2 à 10 % avec une optimisation systématique.",
        "button": "Calculer l'Augmentation de Revenus"
      },
      "results": {
        "title": "Impact Estimé sur les Revenus",
        "currentRevenue": "Revenu Annuel Actuel",
        "potentialUplift": "Augmentation Annuelle Potentielle",
        "upliftPerLocation": "Augmentation par Emplacement",
        "timeframe": "Délai Prévu",
        "areasTitle": "Domaines Clés d'Amélioration",
        "labor": "Optimisation du Travail",
        "menu": "Ingénierie du Menu",
        "operations": "Excellence Opérationnelle",
        "benchmarking": "Évaluation des Emplacements",
        "note": "Ces estimations sont basées sur des références sectorielles. Les résultats réels dépendent des niveaux de performance actuels, des conditions du marché et de la qualité d'exécution. Sundae aide à identifier et suivre les opportunités dans tous les emplacements."
      },
      "timeframe": {
        "small": "3-6 mois avec des améliorations opérationnelles ciblées",
        "medium": "6-12 mois avec une optimisation constante",
        "large": "12-18 mois avec des initiatives stratégiques",
        "xlarge": "18-24 mois avec une transformation majeure"
      },
      "cta": {
        "intro": "Vous souhaitez identifier des opportunités d'amélioration spécifiques dans vos emplacements ?",
        "button": "Voir comment Sundae peut aider"
      },
      "errors": {
        "invalidNumbers": "Veuillez entrer des nombres valides"
      }
    },
    "es": {
      "back": "Volver a Herramientas",
      "hero": {
        "badge": "Inteligencia de Portafolio",
        "title": "Estimador de Aumento de Ingresos Multi-Locación",
        "description": "Calcula el impacto potencial en los ingresos de las mejoras operativas en múltiples ubicaciones."
      },
      "form": {
        "title": "Ingresa tus Datos Multi-Locación",
        "description": "Estima el impacto en los ingresos de las mejoras en el rendimiento.",
        "locationsLabel": "Número de Ubicaciones",
        "locationsPlaceholder": "p. ej., 10",
        "revenueLabel": "Ingreso Promedio Mensual por Ubicación ($)",
        "revenuePlaceholder": "p. ej., 50000",
        "improvementLabel": "Mejora Objetivo (%)",
        "improvementPlaceholder": "p. ej., 5",
        "tip": "Las mejoras típicas oscilan entre el 2-10% con optimización sistemática.",
        "button": "Calcular Aumento de Ingresos"
      },
      "results": {
        "title": "Impacto Estimado en los Ingresos",
        "currentRevenue": "Ingreso Anual Actual",
        "potentialUplift": "Aumento Anual Potencial",
        "upliftPerLocation": "Aumento Por Ubicación",
        "timeframe": "Plazo Esperado",
        "areasTitle": "Áreas Clave de Mejora",
        "labor": "Optimización Laboral",
        "menu": "Ingeniería de Menú",
        "operations": "Excelencia Operativa",
        "benchmarking": "Comparación de Ubicaciones",
        "note": "Estas estimaciones se basan en benchmarks de la industria. Los resultados reales dependen de los niveles de rendimiento actuales, las condiciones del mercado y la calidad de la ejecución. Sundae ayuda a identificar y rastrear oportunidades en todas las ubicaciones."
      },
      "timeframe": {
        "small": "3-6 meses con mejoras operativas enfocadas",
        "medium": "6-12 meses con optimización constante",
        "large": "12-18 meses con iniciativas estratégicas",
        "xlarge": "18-24 meses con transformación mayor"
      },
      "cta": {
        "intro": "¿Quieres identificar oportunidades específicas de mejora en tus ubicaciones?",
        "button": "Ver Cómo Sundae Puede Ayudar"
      },
      "errors": {
        "invalidNumbers": "Por favor, ingresa números válidos"
      }
    },
    "ar": {
      "back": "العودة إلى الأدوات",
      "hero": {
        "badge": "ذكاء المحفظة",
        "title": "مُقدّر زيادة الإيرادات عبر المواقع المتعددة",
        "description": "احسب التأثير المحتمل للإيرادات الناتج عن تحسينات العمليات عبر مواقع متعددة."
      },
      "form": {
        "title": "أدخل بياناتك عبر المواقع المتعددة",
        "description": "قدّر تأثير الإيرادات الناتج عن تحسينات الأداء.",
        "locationsLabel": "عدد المواقع",
        "locationsPlaceholder": "مثل، 10",
        "revenueLabel": "متوسط الإيرادات الشهرية لكل موقع ($)",
        "revenuePlaceholder": "مثل، 50000",
        "improvementLabel": "نسبة التحسين المستهدفة (%)",
        "improvementPlaceholder": "مثل، 5",
        "tip": "تتراوح التحسينات النموذجية بين 2-10% مع التحسين المنهجي.",
        "button": "احسب زيادة الإيرادات"
      },
      "results": {
        "title": "تأثير الإيرادات المقدر",
        "currentRevenue": "الإيرادات السنوية الحالية",
        "potentialUplift": "الزيادة السنوية المحتملة",
        "upliftPerLocation": "الزيادة لكل موقع",
        "timeframe": "الإطار الزمني المتوقع",
        "areasTitle": "مجالات التحسين الرئيسية",
        "labor": "تحسين العمالة",
        "menu": "هندسة القائمة",
        "operations": "التميز التشغيلي",
        "benchmarking": "تقييم المواقع",
        "note": "تستند هذه التقديرات إلى معايير الصناعة. تعتمد النتائج الفعلية على مستويات الأداء الحالية، وظروف السوق، وجودة التنفيذ. تساعد Sundae في تحديد وتتبع الفرص عبر جميع المواقع."
      },
      "timeframe": {
        "small": "3-6 أشهر مع تحسينات تشغيلية مركزة",
        "medium": "6-12 شهراً مع تحسين مستمر",
        "large": "12-18 شهراً مع مبادرات استراتيجية",
        "xlarge": "18-24 شهراً مع تحول كبير"
      },
      "cta": {
        "intro": "هل ترغب في تحديد فرص تحسين محددة عبر مواقعك؟",
        "button": "اكتشف كيف يمكن أن تساعدك Sundae"
      },
      "errors": {
        "invalidNumbers": "يرجى إدخال أرقام صحيحة"
      }
    },
    "nl": {
      "back": "Terug naar Tools",
      "hero": {
        "badge": "Portfolio Intelligentie",
        "title": "Multi-Locatie Omzetverhogingsschatter",
        "description": "Bereken de potentiële omzetimpact van operationele verbeteringen op meerdere locaties."
      },
      "form": {
        "title": "Voer Uw Multi-Locatie Gegevens In",
        "description": "Schat de omzetimpact van prestatieverbeteringen.",
        "locationsLabel": "Aantal Locaties",
        "locationsPlaceholder": "bijv. 10",
        "revenueLabel": "Gemiddelde Maandelijkse Omzet per Locatie (€)",
        "revenuePlaceholder": "bijv. 50000",
        "improvementLabel": "Doelverbetering (%)",
        "improvementPlaceholder": "bijv. 5",
        "tip": "Typische verbeteringen variëren van 2-10% met systematische optimalisatie.",
        "button": "Bereken Omzetverhoging"
      },
      "results": {
        "title": "Geschatte Omzetimpact",
        "currentRevenue": "Huidige Jaaromzet",
        "potentialUplift": "Potentiële Jaarverhoging",
        "upliftPerLocation": "Verhoging per Locatie",
        "timeframe": "Verwachte Tijdspanne",
        "areasTitle": "Belangrijke Verbetergebieden",
        "labor": "Arbeidsoptimalisatie",
        "menu": "Menu-engineering",
        "operations": "Operationele Uitmuntendheid",
        "benchmarking": "Locatiebenchmarking",
        "note": "Deze schattingen zijn gebaseerd op branchebenchmarks. Werkelijke resultaten zijn afhankelijk van huidige prestatieniveaus, marktomstandigheden en uitvoeringskwaliteit. Sundae helpt bij het identificeren en volgen van kansen op alle locaties."
      },
      "timeframe": {
        "small": "3-6 maanden met gerichte operationele verbeteringen",
        "medium": "6-12 maanden met consistente optimalisatie",
        "large": "12-18 maanden met strategische initiatieven",
        "xlarge": "18-24 maanden met grote transformatie"
      },
      "cta": {
        "intro": "Wilt u specifieke verbeterkansen op uw locaties identificeren?",
        "button": "Bekijk Hoe Sundae Kan Helpen"
      },
      "errors": {
        "invalidNumbers": "Voer geldige nummers in"
      }
    },
    "hi": {
      "back": "उपकरणों पर वापस जाएं",
      "hero": {
        "badge": "पोर्टफोलियो इंटेलिजेंस",
        "title": "मल्टी-लोकेशन राजस्व वृद्धि अनुमानक",
        "description": "कई स्थानों में संचालन में सुधार के संभावित राजस्व प्रभाव की गणना करें।"
      },
      "form": {
        "title": "अपने मल्टी-लोकेशन डेटा दर्ज करें",
        "description": "प्रदर्शन सुधार के राजस्व प्रभाव का अनुमान लगाएं।",
        "locationsLabel": "स्थान की संख्या",
        "locationsPlaceholder": "जैसे, 10",
        "revenueLabel": "प्रति स्थान औसत मासिक राजस्व ($)",
        "revenuePlaceholder": "जैसे, 50000",
        "improvementLabel": "लक्ष्य सुधार (%)",
        "improvementPlaceholder": "जैसे, 5",
        "tip": "सामान्य सुधार 2-10% के बीच होते हैं, व्यवस्थित अनुकूलन के साथ।",
        "button": "राजस्व वृद्धि की गणना करें"
      },
      "results": {
        "title": "अनुमानित राजस्व प्रभाव",
        "currentRevenue": "वर्तमान वार्षिक राजस्व",
        "potentialUplift": "संभावित वार्षिक वृद्धि",
        "upliftPerLocation": "प्रति स्थान वृद्धि",
        "timeframe": "अपेक्षित समय सीमा",
        "areasTitle": "मुख्य सुधार क्षेत्र",
        "labor": "श्रम अनुकूलन",
        "menu": "मेनू इंजीनियरिंग",
        "operations": "संचालन उत्कृष्टता",
        "benchmarking": "स्थान बेंचमार्किंग",
        "note": "ये अनुमान उद्योग बेंचमार्क पर आधारित हैं। वास्तविक परिणाम वर्तमान प्रदर्शन स्तरों, बाजार की स्थितियों और निष्पादन की गुणवत्ता पर निर्भर करते हैं। Sundae सभी स्थानों में अवसरों की पहचान और ट्रैक करने में मदद करता है।"
      },
      "timeframe": {
        "small": "केंद्रित संचालन सुधार के साथ 3-6 महीने",
        "medium": "संगत अनुकूलन के साथ 6-12 महीने",
        "large": "रणनीतिक पहलों के साथ 12-18 महीने",
        "xlarge": "महत्वपूर्ण परिवर्तन के साथ 18-24 महीने"
      },
      "cta": {
        "intro": "क्या आप अपने स्थानों में विशिष्ट सुधार के अवसरों की पहचान करना चाहते हैं?",
        "button": "देखें कि Sundae कैसे मदद कर सकता है"
      },
      "errors": {
        "invalidNumbers": "कृपया मान्य संख्या दर्ज करें"
      }
    },
    "ur": {
      "back": "اوزار پر واپس جائیں",
      "hero": {
        "badge": "پورٹ فولیو انٹیلی جنس",
        "title": "کئی مقامات کی آمدنی میں اضافہ کا تخمینہ",
        "description": "کئی مقامات پر عملی بہتریوں کے ممکنہ آمدنی کے اثرات کا حساب لگائیں۔"
      },
      "form": {
        "title": "اپ数据 داخل کریں",
        "description": "کارکردگی کی بہتریوں کے آمدنی کے اثرات کا تخمینہ لگائیں۔",
        "locationsLabel": "مقامات کی تعداد",
        "locationsPlaceholder": "جیسے، 10",
        "revenueLabel": "ہر مقام کی اوسط ماہانہ آمدنی ($)",
        "revenuePlaceholder": "جیسے، 50000",
        "improvementLabel": "ہدف بہتری (%)",
        "improvementPlaceholder": "جیسے، 5",
        "tip": "عام بہتریاں 2-10% کے درمیان ہوتی ہیں جو منظم اصلاح کے ساتھ ہوتی ہیں۔",
        "button": "آمدنی میں اضافہ کا حساب لگائیں"
      },
      "results": {
        "title": "تخمینی آمدنی کا اثر",
        "currentRevenue": "موجودہ سالانہ آمدنی",
        "potentialUplift": "ممکنہ سالانہ اضافہ",
        "upliftPerLocation": "ہر مقام پر اضافہ",
        "timeframe": "متوقع وقت",
        "areasTitle": "اہم بہتری کے شعبے",
        "labor": "محنت کی اصلاح",
        "menu": "مینیو انجینئرنگ",
        "operations": "عملی شانداری",
        "benchmarking": "مقام کی بینچ مارکنگ",
        "note": "یہ تخمینے صنعتی بینچ مارک پر مبنی ہیں۔ حقیقی نتائج موجودہ کارکردگی کی سطح، مارکیٹ کے حالات، اور عملدرآمد کے معیار پر منحصر ہیں۔ Sundae تمام مقامات پر مواقع کی شناخت اور ٹریک کرنے میں مدد کرتا ہے۔"
      },
      "timeframe": {
        "small": "مرکوز عملی بہتریوں کے ساتھ 3-6 ماہ",
        "medium": "مسلسل اصلاح کے ساتھ 6-12 ماہ",
        "large": "اسٹریٹجک اقدامات کے ساتھ 12-18 ماہ",
        "xlarge": "بڑے تبدیلی کے ساتھ 18-24 ماہ"
      },
      "cta": {
        "intro": "کیا آپ اپنے مقامات پر مخصوص بہتری کے مواقع کی شناخت کرنا چاہتے ہیں؟",
        "button": "دیکھیں کہ Sundae کس طرح مدد کر سکتا ہے"
      },
      "errors": {
        "invalidNumbers": "براہ کرم درست نمبر درج کریں"
      }
    },
    "it": {
      "back": "Torna agli Strumenti",
      "hero": {
        "badge": "Intelligenza del Portafoglio",
        "title": "Stimatore dell'Aumento di Fatturato Multi-Locale",
        "description": "Calcola l'impatto potenziale sul fatturato delle migliorie operative in più sedi."
      },
      "form": {
        "title": "Inserisci i Dati Multi-Locale",
        "description": "Stima l'impatto sul fatturato delle migliorie delle performance.",
        "locationsLabel": "Numero di Sedi",
        "locationsPlaceholder": "es. 10",
        "revenueLabel": "Fatturato Mensile Medio per Sede ($)",
        "revenuePlaceholder": "es. 50000",
        "improvementLabel": "Miglioria Obiettivo (%)",
        "improvementPlaceholder": "es. 5",
        "tip": "Le migliorie tipiche variano dal 2 al 10% con ottimizzazione sistematica.",
        "button": "Calcola Aumento di Fatturato"
      },
      "results": {
        "title": "Impatto Stimato sul Fatturato",
        "currentRevenue": "Fatturato Annuale Attuale",
        "potentialUplift": "Aumento Annuale Potenziale",
        "upliftPerLocation": "Aumento per Sede",
        "timeframe": "Tempistiche Attese",
        "areasTitle": "Aree Chiave di Miglioria",
        "labor": "Ottimizzazione del Lavoro",
        "menu": "Ingegneria del Menu",
        "operations": "Eccellenza Operativa",
        "benchmarking": "Benchmarking delle Sedi",
        "note": "Queste stime si basano su benchmark di settore. I risultati effettivi dipendono dai livelli di performance attuali, dalle condizioni di mercato e dalla qualità dell'esecuzione. Sundae aiuta a identificare e monitorare le opportunità in tutte le sedi."
      },
      "timeframe": {
        "small": "3-6 mesi con miglioramenti operativi mirati",
        "medium": "6-12 mesi con ottimizzazione costante",
        "large": "12-18 mesi con iniziative strategiche",
        "xlarge": "18-24 mesi con trasformazione significativa"
      },
      "cta": {
        "intro": "Vuoi identificare opportunità di miglioramento specifiche nelle tue sedi?",
        "button": "Scopri Come Sundae Può Aiutarti"
      },
      "errors": {
        "invalidNumbers": "Inserisci numeri validi"
      }
    },
    "pt": {
      "back": "Voltar para Ferramentas",
      "hero": {
        "badge": "Inteligência de Portfólio",
        "title": "Estimador de Aumento de Receita Multi-Localização",
        "description": "Calcule o impacto potencial na receita de melhorias operacionais em várias localizações."
      },
      "form": {
        "title": "Insira Seus Dados Multi-Localização",
        "description": "Estime o impacto na receita de melhorias de desempenho.",
        "locationsLabel": "Número de Localizações",
        "locationsPlaceholder": "ex.: 10",
        "revenueLabel": "Receita Média Mensal por Localização (R$)",
        "revenuePlaceholder": "ex.: 50000",
        "improvementLabel": "Melhoria Alvo (%)",
        "improvementPlaceholder": "ex.: 5",
        "tip": "Melhorias típicas variam de 2-10% com otimização sistemática.",
        "button": "Calcular Aumento de Receita"
      },
      "results": {
        "title": "Impacto Estimado na Receita",
        "currentRevenue": "Receita Anual Atual",
        "potentialUplift": "Aumento Anual Potencial",
        "upliftPerLocation": "Aumento Por Localização",
        "timeframe": "Prazo Esperado",
        "areasTitle": "Áreas Chave de Melhoria",
        "labor": "Otimização de Mão de Obra",
        "menu": "Engenharia de Cardápio",
        "operations": "Excelência Operacional",
        "benchmarking": "Benchmarking de Localizações",
        "note": "Essas estimativas são baseadas em benchmarks da indústria. Resultados reais dependem dos níveis de desempenho atuais, condições de mercado e qualidade de execução. A Sundae ajuda a identificar e acompanhar oportunidades em todas as localizações."
      },
      "timeframe": {
        "small": "3-6 meses com melhorias operacionais focadas",
        "medium": "6-12 meses com otimização consistente",
        "large": "12-18 meses com iniciativas estratégicas",
        "xlarge": "18-24 meses com transformação significativa"
      },
      "cta": {
        "intro": "Quer identificar oportunidades específicas de melhoria em suas localizações?",
        "button": "Veja Como a Sundae Pode Ajudar"
      },
      "errors": {
        "invalidNumbers": "Por favor, insira números válidos"
      }
    },
    "pl": {
      "back": "Powrót do Narzędzi",
      "hero": {
        "badge": "Inteligencja Portfela",
        "title": "Estimator Wzrostu Przychodów dla Wiele Lokalizacji",
        "description": "Oblicz potencjalny wpływ przychodów wynikający z usprawnień operacyjnych w wielu lokalizacjach."
      },
      "form": {
        "title": "Wprowadź Dane dla Wiele Lokalizacji",
        "description": "Osestimate wpływ przychodów wynikający z usprawnień wydajności.",
        "locationsLabel": "Liczba Lokalizacji",
        "locationsPlaceholder": "np. 10",
        "revenueLabel": "Średni Miesięczny Przychód na Lokalizację ($)",
        "revenuePlaceholder": "np. 50000",
        "improvementLabel": "Docelowa Poprawa (%)",
        "improvementPlaceholder": "np. 5",
        "tip": "Typowe poprawy wahają się od 2-10% przy systematycznej optymalizacji.",
        "button": "Oblicz Wzrost Przychodów"
      },
      "results": {
        "title": "Szacowany Wpływ na Przychody",
        "currentRevenue": "Aktualne Roczne Przychody",
        "potentialUplift": "Potencjalny Roczny Wzrost",
        "upliftPerLocation": "Wzrost na Lokalizację",
        "timeframe": "Oczekiwany Czas Realizacji",
        "areasTitle": "Kluczowe Obszary Usprawnień",
        "labor": "Optymalizacja Pracy",
        "menu": "Inżynieria Menu",
        "operations": "Doskonalenie Operacyjne",
        "benchmarking": "Benchmarking Lokalizacji",
        "note": "Te szacunki opierają się na benchmarkach branżowych. Rzeczywiste wyniki zależą od aktualnych poziomów wydajności, warunków rynkowych i jakości wykonania. Sundae pomaga identyfikować i śledzić możliwości w wszystkich lokalizacjach."
      },
      "timeframe": {
        "small": "3-6 miesięcy z ukierunkowanymi usprawnieniami operacyjnymi",
        "medium": "6-12 miesięcy z konsekwentną optymalizacją",
        "large": "12-18 miesięcy z inicjatywami strategicznymi",
        "xlarge": "18-24 miesięcy z dużą transformacją"
      },
      "cta": {
        "intro": "Chcesz zidentyfikować konkretne możliwości poprawy w swoich lokalizacjach?",
        "button": "Zobacz, jak Sundae może pomóc"
      },
      "errors": {
        "invalidNumbers": "Proszę wprowadzić prawidłowe liczby"
      }
    },
    "zh-Hans": {
      "back": "返回工具",
      "hero": {
        "badge": "投资组合智能",
        "title": "多地点收入提升估算器",
        "description": "计算多个地点运营改善的潜在收入影响。"
      },
      "form": {
        "title": "输入您的多地点数据",
        "description": "估算绩效改善的收入影响。",
        "locationsLabel": "地点数量",
        "locationsPlaceholder": "例如，10",
        "revenueLabel": "每个地点的平均月收入（$）",
        "revenuePlaceholder": "例如，50000",
        "improvementLabel": "目标改善（%）",
        "improvementPlaceholder": "例如，5",
        "tip": "典型改善范围为2-10%，通过系统优化实现。",
        "button": "计算收入提升"
      },
      "results": {
        "title": "预计收入影响",
        "currentRevenue": "当前年收入",
        "potentialUplift": "潜在年提升",
        "upliftPerLocation": "每个地点的提升",
        "timeframe": "预期时间框架",
        "areasTitle": "关键改善领域",
        "labor": "劳动力优化",
        "menu": "菜单工程",
        "operations": "运营卓越",
        "benchmarking": "地点基准",
        "note": "这些估算基于行业基准。实际结果取决于当前绩效水平、市场条件和执行质量。Sundae帮助识别和跟踪所有地点的机会。"
      },
      "timeframe": {
        "small": "3-6个月，专注于运营改善",
        "medium": "6-12个月，持续优化",
        "large": "12-18个月，战略性举措",
        "xlarge": "18-24个月，重大转型"
      },
      "cta": {
        "intro": "想要识别您地点的具体改善机会吗？",
        "button": "查看Sundae如何提供帮助"
      },
      "errors": {
        "invalidNumbers": "请输入有效的数字"
      }
    },
    "ja": {
      "back": "ツールに戻る",
      "hero": {
        "badge": "ポートフォリオインテリジェンス",
        "title": "複数ロケーション収益向上推定ツール",
        "description": "複数のロケーションにおける運営改善の潜在的な収益影響を計算します。"
      },
      "form": {
        "title": "複数ロケーションデータを入力",
        "description": "パフォーマンス改善の収益影響を推定します。",
        "locationsLabel": "ロケーション数",
        "locationsPlaceholder": "例：10",
        "revenueLabel": "ロケーションあたりの平均月間収益 ($)",
        "revenuePlaceholder": "例：50000",
        "improvementLabel": "目標改善 (%)",
        "improvementPlaceholder": "例：5",
        "tip": "典型的な改善は2-10%の範囲で、体系的な最適化が必要です。",
        "button": "収益向上を計算"
      },
      "results": {
        "title": "推定収益影響",
        "currentRevenue": "現在の年間収益",
        "potentialUplift": "潜在的な年間向上",
        "upliftPerLocation": "ロケーションあたりの向上",
        "timeframe": "期待されるタイムフレーム",
        "areasTitle": "主要改善エリア",
        "labor": "労働最適化",
        "menu": "メニューエンジニアリング",
        "operations": "運営の卓越性",
        "benchmarking": "ロケーションベンチマーキング",
        "note": "これらの推定値は業界のベンチマークに基づいています。実際の結果は現在のパフォーマンスレベル、市場条件、実行の質に依存します。Sundaeはすべてのロケーションでの機会を特定し、追跡するのに役立ちます。"
      },
      "timeframe": {
        "small": "集中した運営改善で3-6ヶ月",
        "medium": "一貫した最適化で6-12ヶ月",
        "large": "戦略的イニシアチブで12-18ヶ月",
        "xlarge": "大規模な変革で18-24ヶ月"
      },
      "cta": {
        "intro": "ロケーション全体で特定の改善機会を特定したいですか？",
        "button": "Sundaeがどのように役立つかを見る"
      },
      "errors": {
        "invalidNumbers": "有効な数字を入力してください"
      }
    },
    "tr": {
      "back": "Araçlara Geri Dön",
      "hero": {
        "badge": "Portföy Zekası",
        "title": "Çoklu Lokasyon Gelir Artışı Hesaplayıcı",
        "description": "Birden fazla lokasyondaki operasyonel iyileştirmelerin potansiyel gelir etkisini hesaplayın."
      },
      "form": {
        "title": "Çoklu Lokasyon Verilerinizi Girin",
        "description": "Performans iyileştirmelerinin gelir etkisini tahmin edin.",
        "locationsLabel": "Lokasyon Sayısı",
        "locationsPlaceholder": "örn., 10",
        "revenueLabel": "Lokasyon Başına Ortalama Aylık Gelir ($)",
        "revenuePlaceholder": "örn., 50000",
        "improvementLabel": "Hedef İyileştirme (%)",
        "improvementPlaceholder": "örn., 5",
        "tip": "Tipik iyileştirmeler sistematik optimizasyon ile %2-10 arasında değişir.",
        "button": "Gelir Artışını Hesapla"
      },
      "results": {
        "title": "Tahmini Gelir Etkisi",
        "currentRevenue": "Mevcut Yıllık Gelir",
        "potentialUplift": "Potansiyel Yıllık Artış",
        "upliftPerLocation": "Lokasyon Başına Artış",
        "timeframe": "Beklenen Zaman Çerçevesi",
        "areasTitle": "Ana İyileştirme Alanları",
        "labor": "İş Gücü Optimizasyonu",
        "menu": "Menü Mühendisliği",
        "operations": "Operasyonel Mükemmellik",
        "benchmarking": "Lokasyon Karşılaştırması",
        "note": "Bu tahminler sektör karşılaştırmalarına dayanmaktadır. Gerçek sonuçlar mevcut performans seviyelerine, piyasa koşullarına ve uygulama kalitesine bağlıdır. Sundae, tüm lokasyonlar arasında fırsatları tanımlamaya ve takip etmeye yardımcı olur."
      },
      "timeframe": {
        "small": "Odaklanmış operasyonel iyileştirmelerle 3-6 ay",
        "medium": "Tutarlı optimizasyon ile 6-12 ay",
        "large": "Stratejik girişimlerle 12-18 ay",
        "xlarge": "Büyük dönüşüm ile 18-24 ay"
      },
      "cta": {
        "intro": "Lokasyonlarınızda belirli iyileştirme fırsatlarını mı tanımlamak istiyorsunuz?",
        "button": "Sundae'nin Size Nasıl Yardımcı Olabileceğini Görün"
      },
      "errors": {
        "invalidNumbers": "Lütfen geçerli sayılar girin"
      }
    },
    "ko": {
      "back": "도구로 돌아가기",
      "hero": {
        "badge": "포트폴리오 인텔리전스",
        "title": "다중 위치 수익 증가 추정기",
        "description": "여러 위치에서 운영 개선의 잠재적 수익 영향을 계산합니다."
      },
      "form": {
        "title": "다중 위치 데이터 입력",
        "description": "성과 개선의 수익 영향을 추정합니다.",
        "locationsLabel": "위치 수",
        "locationsPlaceholder": "예: 10",
        "revenueLabel": "위치당 평균 월 수익 ($)",
        "revenuePlaceholder": "예: 50000",
        "improvementLabel": "목표 개선 (%)",
        "improvementPlaceholder": "예: 5",
        "tip": "일반적인 개선 범위는 체계적인 최적화로 2-10%입니다.",
        "button": "수익 증가 계산"
      },
      "results": {
        "title": "추정 수익 영향",
        "currentRevenue": "현재 연간 수익",
        "potentialUplift": "잠재적 연간 증가",
        "upliftPerLocation": "위치당 증가",
        "timeframe": "예상 기간",
        "areasTitle": "주요 개선 영역",
        "labor": "인력 최적화",
        "menu": "메뉴 엔지니어링",
        "operations": "운영 우수성",
        "benchmarking": "위치 벤치마킹",
        "note": "이 추정치는 업계 벤치마크를 기반으로 합니다. 실제 결과는 현재 성과 수준, 시장 조건 및 실행 품질에 따라 달라집니다. Sundae는 모든 위치에서 기회를 식별하고 추적하는 데 도움을 줍니다."
      },
      "timeframe": {
        "small": "집중적인 운영 개선으로 3-6개월",
        "medium": "지속적인 최적화로 6-12개월",
        "large": "전략적 이니셔티브로 12-18개월",
        "xlarge": "대규모 변혁으로 18-24개월"
      },
      "cta": {
        "intro": "위치별로 특정 개선 기회를 식별하고 싶으신가요?",
        "button": "Sundae가 어떻게 도와줄 수 있는지 보기"
      },
      "errors": {
        "invalidNumbers": "유효한 숫자를 입력해 주세요"
      }
    },
    "id": {
      "back": "Kembali ke Alat",
      "hero": {
        "badge": "Inteligensi Portofolio",
        "title": "Estimasi Peningkatan Pendapatan Multi-Lokasi",
        "description": "Hitung potensi dampak pendapatan dari perbaikan operasional di berbagai lokasi."
      },
      "form": {
        "title": "Masukkan Data Multi-Lokasi Anda",
        "description": "Perkirakan dampak pendapatan dari perbaikan kinerja.",
        "locationsLabel": "Jumlah Lokasi",
        "locationsPlaceholder": "mis., 10",
        "revenueLabel": "Rata-rata Pendapatan Bulanan per Lokasi ($)",
        "revenuePlaceholder": "mis., 50000",
        "improvementLabel": "Target Peningkatan (%)",
        "improvementPlaceholder": "mis., 5",
        "tip": "Peningkatan tipikal berkisar antara 2-10% dengan optimasi sistematis.",
        "button": "Hitung Peningkatan Pendapatan"
      },
      "results": {
        "title": "Perkiraan Dampak Pendapatan",
        "currentRevenue": "Pendapatan Tahunan Saat Ini",
        "potentialUplift": "Peningkatan Tahunan Potensial",
        "upliftPerLocation": "Peningkatan per Lokasi",
        "timeframe": "Perkiraan Waktu",
        "areasTitle": "Area Peningkatan Utama",
        "labor": "Optimasi Tenaga Kerja",
        "menu": "Rekayasa Menu",
        "operations": "Keunggulan Operasional",
        "benchmarking": "Benchmarking Lokasi",
        "note": "Perkiraan ini berdasarkan benchmark industri. Hasil aktual tergantung pada tingkat kinerja saat ini, kondisi pasar, dan kualitas eksekusi. Sundae membantu mengidentifikasi dan melacak peluang di semua lokasi."
      },
      "timeframe": {
        "small": "3-6 bulan dengan perbaikan operasional yang terfokus",
        "medium": "6-12 bulan dengan optimasi yang konsisten",
        "large": "12-18 bulan dengan inisiatif strategis",
        "xlarge": "18-24 bulan dengan transformasi besar"
      },
      "cta": {
        "intro": "Ingin mengidentifikasi peluang perbaikan spesifik di lokasi Anda?",
        "button": "Lihat Bagaimana Sundae Dapat Membantu"
      },
      "errors": {
        "invalidNumbers": "Silakan masukkan angka yang valid"
      }
    },
    "vi": {
      "back": "Quay lại Công cụ",
      "hero": {
        "badge": "Trí tuệ Danh mục",
        "title": "Ước lượng Tăng trưởng Doanh thu Đa Địa điểm",
        "description": "Tính toán tác động doanh thu tiềm năng của các cải tiến hoạt động trên nhiều địa điểm."
      },
      "form": {
        "title": "Nhập Dữ liệu Đa Địa điểm của Bạn",
        "description": "Ước lượng tác động doanh thu của các cải tiến hiệu suất.",
        "locationsLabel": "Số lượng Địa điểm",
        "locationsPlaceholder": "vd: 10",
        "revenueLabel": "Doanh thu Trung bình Hàng tháng mỗi Địa điểm ($)",
        "revenuePlaceholder": "vd: 50000",
        "improvementLabel": "Cải tiến Mục tiêu (%)",
        "improvementPlaceholder": "vd: 5",
        "tip": "Các cải tiến điển hình dao động từ 2-10% với tối ưu hóa hệ thống.",
        "button": "Tính Toán Tăng Trưởng Doanh Thu"
      },
      "results": {
        "title": "Tác động Doanh thu Ước lượng",
        "currentRevenue": "Doanh thu Hàng năm Hiện tại",
        "potentialUplift": "Tăng trưởng Hàng năm Tiềm năng",
        "upliftPerLocation": "Tăng trưởng Mỗi Địa điểm",
        "timeframe": "Khung Thời gian Dự kiến",
        "areasTitle": "Các Khu vực Cải tiến Chính",
        "labor": "Tối ưu hóa Lao động",
        "menu": "Kỹ thuật Thực đơn",
        "operations": "Xuất sắc Vận hành",
        "benchmarking": "So sánh Địa điểm",
        "note": "Các ước lượng này dựa trên các tiêu chuẩn ngành. Kết quả thực tế phụ thuộc vào mức hiệu suất hiện tại, điều kiện thị trường và chất lượng thực hiện. Sundae giúp xác định và theo dõi cơ hội trên tất cả các địa điểm."
      },
      "timeframe": {
        "small": "3-6 tháng với các cải tiến hoạt động tập trung",
        "medium": "6-12 tháng với tối ưu hóa liên tục",
        "large": "12-18 tháng với các sáng kiến chiến lược",
        "xlarge": "18-24 tháng với chuyển đổi lớn"
      },
      "cta": {
        "intro": "Bạn muốn xác định các cơ hội cải tiến cụ thể trên các địa điểm của mình?",
        "button": "Xem Sundae Có Thể Giúp Gì"
      },
      "errors": {
        "invalidNumbers": "Vui lòng nhập số hợp lệ"
      }
    },
    "sv": {
      "back": "Tillbaka till Verktyg",
      "hero": {
        "badge": "Portföljintelligens",
        "title": "Beräknare för Intäktsökning på Flera Platser",
        "description": "Beräkna den potentiella intäkts påverkan av operationella förbättringar över flera platser."
      },
      "form": {
        "title": "Ange Din Data för Flera Platser",
        "description": "Skatta intäkts påverkan av prestationsförbättringar.",
        "locationsLabel": "Antal Platser",
        "locationsPlaceholder": "t.ex. 10",
        "revenueLabel": "Genomsnittlig Månatlig Intäkt per Plats ($)",
        "revenuePlaceholder": "t.ex. 50000",
        "improvementLabel": "Målinriktad Förbättring (%)",
        "improvementPlaceholder": "t.ex. 5",
        "tip": "Typiska förbättringar ligger mellan 2-10% med systematisk optimering.",
        "button": "Beräkna Intäktsökning"
      },
      "results": {
        "title": "Beräknad Intäkts påverkan",
        "currentRevenue": "Nuvarande Årliga Intäkter",
        "potentialUplift": "Potentiell Årlig Ökning",
        "upliftPerLocation": "Ökning per Plats",
        "timeframe": "Förväntad Tidsram",
        "areasTitle": "Nyckelområden för Förbättring",
        "labor": "Arbetskraftsoptimering",
        "menu": "Menyplanering",
        "operations": "Operationell Excellens",
        "benchmarking": "Platsbenchmarking",
        "note": "Dessa uppskattningar baseras på branschstandarder. Faktiska resultat beror på nuvarande prestationsnivåer, marknadsförhållanden och genomförandekvalitet. Sundae hjälper till att identifiera och spåra möjligheter över alla platser."
      },
      "timeframe": {
        "small": "3-6 månader med fokuserade operationella förbättringar",
        "medium": "6-12 månader med konsekvent optimering",
        "large": "12-18 månader med strategiska initiativ",
        "xlarge": "18-24 månader med större transformation"
      },
      "cta": {
        "intro": "Vill du identifiera specifika förbättringsmöjligheter över dina platser?",
        "button": "Se Hur Sundae Kan Hjälpa"
      },
      "errors": {
        "invalidNumbers": "Vänligen ange giltiga nummer"
      }
    },
    "ro": {
      "back": "Înapoi la Instrumente",
      "hero": {
        "badge": "Inteligența Portofoliului",
        "title": "Estimator de Creștere a Veniturilor pentru Multiple Locații",
        "description": "Calculează impactul potențial al veniturilor din îmbunătățirile operaționale în mai multe locații."
      },
      "form": {
        "title": "Introduceți Datele Dvs. pentru Multiple Locații",
        "description": "Estimați impactul veniturilor din îmbunătățirile de performanță.",
        "locationsLabel": "Numărul de Locații",
        "locationsPlaceholder": "de ex., 10",
        "revenueLabel": "Venitul Mediu Lunar pe Locație ($)",
        "revenuePlaceholder": "de ex., 50000",
        "improvementLabel": "Îmbunătățire Țintă (%)",
        "improvementPlaceholder": "de ex., 5",
        "tip": "Îmbunătățirile tipice variază între 2-10% cu optimizare sistematică.",
        "button": "Calculează Creșterea Veniturilor"
      },
      "results": {
        "title": "Impactul Estimat al Veniturilor",
        "currentRevenue": "Venitul Anual Actual",
        "potentialUplift": "Creșterea Potențială Anuală",
        "upliftPerLocation": "Creșterea pe Locație",
        "timeframe": "Interval de Timp Așteptat",
        "areasTitle": "Zone Cheie de Îmbunătățire",
        "labor": "Optimizarea Forței de Muncă",
        "menu": "Inginerie de Meniu",
        "operations": "Excelență Operațională",
        "benchmarking": "Compararea Locațiilor",
        "note": "Aceste estimări se bazează pe repere din industrie. Rezultatele reale depind de nivelurile actuale de performanță, condițiile de piață și calitatea execuției. Sundae ajută la identificarea și urmărirea oportunităților în toate locațiile."
      },
      "timeframe": {
        "small": "3-6 luni cu îmbunătățiri operaționale concentrate",
        "medium": "6-12 luni cu optimizare constantă",
        "large": "12-18 luni cu inițiative strategice",
        "xlarge": "18-24 luni cu transformări majore"
      },
      "cta": {
        "intro": "Doriți să identificați oportunități specifice de îmbunătățire în locațiile dvs.?",
        "button": "Vezi Cum Poate Ajuta Sundae"
      },
      "errors": {
        "invalidNumbers": "Vă rugăm să introduceți numere valide"
      }
    },
    "bn": {
      "back": "টুলসে ফিরে যান",
      "hero": {
        "badge": "পোর্টফোলিও ইন্টেলিজেন্স",
        "title": "মাল্টি-লোকেশন রাজস্ব বৃদ্ধি অনুমানকারী",
        "description": "একাধিক স্থানে কার্যকরী উন্নতির সম্ভাব্য রাজস্ব প্রভাব গণনা করুন।"
      },
      "form": {
        "title": "আপনার মাল্টি-লোকেশন ডেটা প্রবেশ করুন",
        "description": "পারফরম্যান্স উন্নতির রাজস্ব প্রভাব অনুমান করুন।",
        "locationsLabel": "লোকেশনের সংখ্যা",
        "locationsPlaceholder": "যেমন, 10",
        "revenueLabel": "প্রতি লোকেশনে গড় মাসিক রাজস্ব ($)",
        "revenuePlaceholder": "যেমন, 50000",
        "improvementLabel": "লক্ষ্য উন্নতি (%)",
        "improvementPlaceholder": "যেমন, 5",
        "tip": "সাধারণত উন্নতির পরিমাণ 2-10% এর মধ্যে থাকে সিস্টেম্যাটিক অপটিমাইজেশনের সাথে।",
        "button": "রাজস্ব বৃদ্ধি গণনা করুন"
      },
      "results": {
        "title": "অনুমানিত রাজস্ব প্রভাব",
        "currentRevenue": "বর্তমান বার্ষিক রাজস্ব",
        "potentialUplift": "সম্ভাব্য বার্ষিক বৃদ্ধি",
        "upliftPerLocation": "প্রতি লোকেশনে বৃদ্ধি",
        "timeframe": "প্রত্যাশিত সময়সীমা",
        "areasTitle": "মূল উন্নতির ক্ষেত্র",
        "labor": "শ্রম অপটিমাইজেশন",
        "menu": "মেনু ইঞ্জিনিয়ারিং",
        "operations": "কার্যকরী উৎকর্ষতা",
        "benchmarking": "লোকেশন বেঞ্চমার্কিং",
        "note": "এই অনুমানগুলি শিল্প বেঞ্চমার্কের উপর ভিত্তি করে। প্রকৃত ফলাফল বর্তমান পারফরম্যান্স স্তর, বাজারের অবস্থান এবং কার্যকরী গুণমানের উপর নির্ভর করে। Sundae সমস্ত স্থানে সুযোগ চিহ্নিত করতে এবং ট্র্যাক করতে সহায়তা করে।"
      },
      "timeframe": {
        "small": "কেন্দ্রিত কার্যকরী উন্নতির সাথে 3-6 মাস",
        "medium": "নিয়মিত অপটিমাইজেশনের সাথে 6-12 মাস",
        "large": "কৌশলগত উদ্যোগের সাথে 12-18 মাস",
        "xlarge": "মহান রূপান্তরের সাথে 18-24 মাস"
      },
      "cta": {
        "intro": "আপনার লোকেশনগুলোর মধ্যে নির্দিষ্ট উন্নতির সুযোগ চিহ্নিত করতে চান?",
        "button": "দেখুন Sundae কীভাবে সাহায্য করতে পারে"
      },
      "errors": {
        "invalidNumbers": "দয়া করে বৈধ সংখ্যা প্রবেশ করুন"
      }
    },
    "th": {
      "back": "กลับไปที่เครื่องมือ",
      "hero": {
        "badge": "ข้อมูลเชิงพาณิชย์",
        "title": "การประเมินการเพิ่มรายได้หลายสถานที่",
        "description": "คำนวณผลกระทบทางรายได้ที่อาจเกิดขึ้นจากการปรับปรุงการดำเนินงานในหลายสถานที่."
      },
      "form": {
        "title": "กรอกข้อมูลหลายสถานที่ของคุณ",
        "description": "ประเมินผลกระทบทางรายได้จากการปรับปรุงประสิทธิภาพ.",
        "locationsLabel": "จำนวนสถานที่",
        "locationsPlaceholder": "เช่น 10",
        "revenueLabel": "รายได้เฉลี่ยต่อเดือนต่อสถานที่ ($)",
        "revenuePlaceholder": "เช่น 50000",
        "improvementLabel": "การปรับปรุงเป้าหมาย (%)",
        "improvementPlaceholder": "เช่น 5",
        "tip": "การปรับปรุงทั่วไปอยู่ในช่วง 2-10% โดยมีการปรับแต่งอย่างเป็นระบบ.",
        "button": "คำนวณการเพิ่มรายได้"
      },
      "results": {
        "title": "ผลกระทบทางรายได้ที่ประเมิน",
        "currentRevenue": "รายได้ประจำปีปัจจุบัน",
        "potentialUplift": "การเพิ่มรายปีที่เป็นไปได้",
        "upliftPerLocation": "การเพิ่มต่อสถานที่",
        "timeframe": "กรอบเวลาโดยประมาณ",
        "areasTitle": "พื้นที่การปรับปรุงหลัก",
        "labor": "การเพิ่มประสิทธิภาพแรงงาน",
        "menu": "การออกแบบเมนู",
        "operations": "ความเป็นเลิศในการดำเนินงาน",
        "benchmarking": "การเปรียบเทียบสถานที่",
        "note": "การประเมินเหล่านี้อิงจากมาตรฐานอุตสาหกรรม ผลลัพธ์จริงขึ้นอยู่กับระดับประสิทธิภาพปัจจุบัน สภาพตลาด และคุณภาพการดำเนินการ. Sundae ช่วยในการระบุและติดตามโอกาสในทุกสถานที่."
      },
      "timeframe": {
        "small": "3-6 เดือนกับการปรับปรุงการดำเนินงานที่มุ่งเน้น",
        "medium": "6-12 เดือนกับการปรับแต่งอย่างต่อเนื่อง",
        "large": "12-18 เดือนกับโครงการเชิงกลยุทธ์",
        "xlarge": "18-24 เดือนกับการเปลี่ยนแปลงครั้งใหญ่"
      },
      "cta": {
        "intro": "ต้องการระบุโอกาสการปรับปรุงเฉพาะในสถานที่ของคุณหรือไม่?",
        "button": "ดูว่า Sundae สามารถช่วยได้อย่างไร"
      },
      "errors": {
        "invalidNumbers": "กรุณาใส่หมายเลขที่ถูกต้อง"
      }
    },
    "ms": {
      "back": "Kembali ke Alat",
      "hero": {
        "badge": "Intelligence Portfolio",
        "title": "Penganggar Kenaikan Pendapatan Multi-Lokasi",
        "description": "Kira kesan potensi pendapatan daripada penambahbaikan operasi di pelbagai lokasi."
      },
      "form": {
        "title": "Masukkan Data Multi-Lokasi Anda",
        "description": "Anggarkan kesan pendapatan daripada penambahbaikan prestasi.",
        "locationsLabel": "Bilangan Lokasi",
        "locationsPlaceholder": "cth., 10",
        "revenueLabel": "Pendapatan Bulanan Purata bagi Setiap Lokasi ($)",
        "revenuePlaceholder": "cth., 50000",
        "improvementLabel": "Sasaran Penambahbaikan (%)",
        "improvementPlaceholder": "cth., 5",
        "tip": "Penambahbaikan biasa berkisar antara 2-10% dengan pengoptimuman sistematik.",
        "button": "Kira Kenaikan Pendapatan"
      },
      "results": {
        "title": "Kesan Pendapatan yang Dianggarkan",
        "currentRevenue": "Pendapatan Tahunan Semasa",
        "potentialUplift": "Kenaikan Tahunan Potensi",
        "upliftPerLocation": "Kenaikan Per Lokasi",
        "timeframe": "Jangka Masa Dijangka",
        "areasTitle": "Bidang Penambahbaikan Utama",
        "labor": "Pengoptimuman Tenaga Kerja",
        "menu": "Reka Bentuk Menu",
        "operations": "Kecemerlangan Operasi",
        "benchmarking": "Penanda Aras Lokasi",
        "note": "Anggaran ini berdasarkan penanda aras industri. Hasil sebenar bergantung kepada tahap prestasi semasa, keadaan pasaran, dan kualiti pelaksanaan. Sundae membantu mengenal pasti dan menjejak peluang di semua lokasi."
      },
      "timeframe": {
        "small": "3-6 bulan dengan penambahbaikan operasi yang fokus",
        "medium": "6-12 bulan dengan pengoptimuman yang konsisten",
        "large": "12-18 bulan dengan inisiatif strategik",
        "xlarge": "18-24 bulan dengan transformasi besar"
      },
      "cta": {
        "intro": "Ingin mengenal pasti peluang penambahbaikan khusus di lokasi anda?",
        "button": "Lihat Bagaimana Sundae Boleh Membantu"
      },
      "errors": {
        "invalidNumbers": "Sila masukkan nombor yang sah"
      }
    }
  }
} as const
