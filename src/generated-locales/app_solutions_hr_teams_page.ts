// Generated website local copy overrides for src/app/solutions/hr-teams/page.tsx.
// Structural fields and protected glossary terms normalized from English source; review prose translations before release.
export const generatedLocalCopy = {
  "localizedCopy": {
    "ar": {
      "badge": "لفرق الموارد البشرية والأفراد",
      "titleLine1": "جدول زمني مبني على الوردية.",
      "titleLine2": "ليس على الأسبوع الماضي.",
      "description": "تباين العمالة الحية حسب الموقع. تم وضع علامة على مخاطر العمل الإضافي قبل حدوثها. يتم قياس الإنتاجية لكل ساعة FOH، وليس لكل فترة دفع.",
      "primaryCta": "احجز جولة عمل",
      "secondaryCta": "انظر عرض العمل",
      "problemsEyebrow": "حيث يتسرب العمل",
      "problemsTitle": "حيث تتسرب ميزانية العمل",
      "problemsDescription": "الجداول مبنية على الغريزة، ويتم اكتشاف التباين بعد كشوف المرتبات.",
      "challenges": [
        {
          "title": "التعرض لـ OT غير مرئي حتى كشف الرواتب",
          "description": "بحلول الوقت الذي ترى فيه خط العمل الإضافي، تكون قد دفعته بالفعل.",
          "icon": "balance"
        },
        {
          "title": "الجداول المبنية على أغلفة الأسبوع الماضي",
          "description": "لقد تحرك الطلب. الجدول الزمني لم يفعل ذلك. العمل٪ ينجرف معه.",
          "icon": "operators"
        },
        {
          "title": "تم الإبلاغ عن الإنتاجية، وهي غير قابلة للتنفيذ",
          "description": "أنت تعرف التغطية لكل ساعة FOH شهريًا. ليس بالوردية، وليس بالخادم، وليس بالوقت المناسب للتدريب.",
          "icon": "performance"
        },
        {
          "title": "لا يوجد إنذار مبكر بشأن التغيب",
          "description": "يبدأ التحول السيئ عندما يصل عدم الحضور الساعة 4:15 مساءً. ستسمعون عنها في ملخص الغد.",
          "icon": "support"
        }
      ],
      "howTitle": "كيف تعمل Sundae للموارد البشرية والأفراد",
      "howDescription": "إشارة العمل الحية، والوقاية من OT، والإنتاجية عند دقة التحول.",
      "howSundaeHelps": [
        {
          "title": "العمالة الحية٪ حسب الموقع",
          "description": "تكلفة العمالة بالساعة مقابل الطلب. يتم وضع علامة على الموقع الذي يتجاوز الميزانية بينما لا يزال بإمكانك القطع.",
          "product": "Pulse · Labor",
          "icon": "operators"
        },
        {
          "title": "التعرض OT قبل أن يضرب",
          "description": "تتنبأ Sundae بخط OT حسب الموقع لبقية الأسبوع، بحيث يمكنك إعادة توزيع ساعات العمل، وليس استيعاب التكلفة.",
          "product": "Insights · Labor",
          "icon": "balance"
        },
        {
          "title": "الإنتاجية لكل ساعة FOH لكل خادم",
          "description": "يغطي/ساعة، مبيعات/ساعة، معدل البيع - لكل خادم، لكل وردية. المدرب بالأرقام وليس بالانطباعات.",
          "product": "Pulse · Server view",
          "icon": "performance"
        },
        {
          "title": "توصيات الجدولة من الطلب",
          "description": "يتوقع Foresight أعداد الضيوف قبل 14 يومًا - وتقترح Sundae خطة العمالة المطابقة.",
          "product": "Foresight",
          "icon": "forecasting"
        }
      ],
      "outcomesTitle": "ما التغييرات بالنسبة للموارد البشرية",
      "outcomesDescription": "جداول زمنية أكثر صرامة، ومفاجآت أقل، وإنتاجية يمكنك إظهارها.",
      "outcomes": [
        {
          "title": "قطع التعرض OT قبل أن يهبط",
          "description": "إعادة تخصيص ساعات العمل في منتصف الأسبوع، وليس بعد تشغيل كشوف المرتبات.",
          "icon": "speed"
        },
        {
          "title": "العمل٪ أقرب إلى الخطة، كل أسبوع",
          "description": "يضيق التباين من الفوضى الشهرية إلى الدقة الأسبوعية.",
          "icon": "performance"
        },
        {
          "title": "إنتاجية الخادم التي يمكنك مقارنتها",
          "description": "إنتاجية حية لكل نوبة عمل تدعم التدريب والمراجعات ومكالمات الترويج.",
          "icon": "owners"
        },
        {
          "title": "جداول زمنية تتناسب مع صالة المطعم",
          "description": "جداول زمنية مبنية على الطلب وعلى توقعات Foresight - وليس على نمط الأسبوع الماضي.",
          "icon": "support"
        }
      ],
      "crewCallout": {
        "eyebrow": "مدعومة من قبل طاقم Sundae",
        "title": "الركيزة التشغيلية التي تغذي هذا الذكاء.",
        "description": "يدير Crew الجدولة والوقت والحضور وكشوف المرتبات متعددة المناطق (الولايات المتحدة وكندا والمملكة المتحدة والاتحاد الأوروبي ودول مجلس التعاون الخليجي - محرك واحد)، وأكثر من 60 سطحًا للموارد البشرية - تصبح كل نوبة عمل وساعة ودورة دفع إشارة تدعم ذكاء العمل في هذه الصفحة. اختياري: قم بإحضار HR الخاص بك (Bayzat، Personio، Gusto) وستظل Sundae تقرأ نفس الإشارة.",
        "ctaText": "Explore Sundae Crew"
      },
      "ctaTitle": "شاهد عمالتك أثناء الحركة.",
      "ctaDescription": "30 دقيقة. الجدول الزمني الخاص بك. OT الذي كنت ستلتقطه قبل أن تهبط.",
      "ctaButton": "احجز جولة عمل"
    },
    "es": {
      "badge": "Para equipos de recursos humanos y personal",
      "titleLine1": "Horario para el turno.",
      "titleLine2": "No durante la semana pasada.",
      "description": "Variación de mano de obra viva por ubicación. Riesgo de horas extras señalado antes de que suceda. Productividad medida por hora FOH, no por período de pago.",
      "primaryCta": "Agenda una sesión laboral",
      "secondaryCta": "Ver demostración laboral",
      "problemsEyebrow": "DONDE SE FUGA LA MANO DE OBRA",
      "problemsTitle": "Donde se fuga el presupuesto de personal",
      "problemsDescription": "Horarios basados en el instinto, variación descubierta después de la nómina.",
      "challenges": [
        {
          "title": "Exposición a OT invisible hasta la nómina",
          "description": "Cuando vea la línea de horas extras, ya las habrá pagado.",
          "icon": "balance"
        },
        {
          "title": "Horarios basados en los comensales de la semana pasada",
          "description": "La demanda se ha movido. El horario no. El porcentaje de mano de obra se desvía con ella.",
          "icon": "operators"
        },
        {
          "title": "Productividad reportada, no accionable",
          "description": "Conoces los comensales por hora FOH al mes. Ni por turno, ni por camarero, ni a tiempo para formar.",
          "icon": "performance"
        },
        {
          "title": "No hay alerta temprana sobre el ausentismo",
          "description": "El mal turno comienza cuando alguien no se presenta a las 4:15 p.m. Lo sabrás en el resumen de mañana.",
          "icon": "support"
        }
      ],
      "howTitle": "Cómo funciona Sundae para Recursos Humanos y Personas",
      "howDescription": "Señal de trabajo en vivo, prevención de OT, productividad en granularidad de turno.",
      "howSundaeHelps": [
        {
          "title": "% de mano de obra viva por ubicación",
          "description": "Costo laboral hora por hora frente a la demanda. El sitio que supera el presupuesto se marca mientras aún puedes recortarlo.",
          "product": "Pulse · Labor",
          "icon": "operators"
        },
        {
          "title": "Exposición al OT antes de que llegue",
          "description": "Sundae pronostica la línea de tiempo extra por ubicación para el resto de la semana, de modo que usted redistribuya las horas, no absorba el costo.",
          "product": "Insights · Labor",
          "icon": "balance"
        },
        {
          "title": "Productividad por hora FOH, por camarero",
          "description": "Comensales/hora, ventas/hora, tasa de venta adicional: por camarero, por turno. Forma con números, no con impresiones.",
          "product": "Pulse · Server view",
          "icon": "performance"
        },
        {
          "title": "Recomendaciones de horario a partir de la demanda",
          "description": "Foresight proyecta los comensales con 14 días de antelación: Sundae sugiere el plan de personal que coincide.",
          "product": "Foresight",
          "icon": "forecasting"
        }
      ],
      "outcomesTitle": "¿Qué cambios para RRHH?",
      "outcomesDescription": "Horarios más ajustados, menos sorpresas, productividad que puedes demostrar.",
      "outcomes": [
        {
          "title": "Reduzca la exposición a OT antes de que aterrice",
          "description": "Reasigne horas a mitad de semana, no después de la ejecución de la nómina.",
          "icon": "speed"
        },
        {
          "title": "% de mano de obra más cerca del plan, cada semana",
          "description": "La variación se reduce del caos mensual a la precisión semanal.",
          "icon": "performance"
        },
        {
          "title": "Productividad del camarero que puedes comparar",
          "description": "Productividad en vivo por turno que respalda la formación, las evaluaciones y las decisiones de ascenso.",
          "icon": "owners"
        },
        {
          "title": "Horarios que coinciden con el piso",
          "description": "Los cronogramas impulsados por la demanda se basaron en proyecciones de Foresight, no en el patrón de la semana pasada.",
          "icon": "support"
        }
      ],
      "crewCallout": {
        "eyebrow": "RESPALDADO POR EL EQUIPO SUNDAE",
        "title": "El sustrato operativo que alimenta esta inteligencia.",
        "description": "Crew ejecuta programación, tiempo y asistencia, nómina multirregional (EE. UU., Canadá, Reino Unido, UE, CCG: un motor) y más de 60 superficies de recursos humanos: cada turno, hora y ciclo de pago se convierte en una señal que impulsa la inteligencia laboral en esta página. Opcional: traiga su propio HR (Bayzat, Personio, Gusto) y Sundae seguirá leyendo la misma señal.",
        "ctaText": "Explore Sundae Crew"
      },
      "ctaTitle": "Vea su personal en movimiento.",
      "ctaDescription": "30 minutos. Tu horario. El OT que habrías atrapado antes de que aterrizara.",
      "ctaButton": "Agenda una sesión laboral"
    },
    "de": {
      "badge": "Für HR- und Personalteams",
      "titleLine1": "Zeitplan für die Schicht.",
      "titleLine2": "Nicht für letzte Woche.",
      "description": "Live-Arbeitsvarianz nach Standort. Überstundenrisiko wird erkannt, bevor es eintritt. Die Produktivität wird pro FOH-Stunde gemessen, nicht pro Lohnperiode.",
      "primaryCta": "Buchen Sie einen Arbeitsrundgang",
      "secondaryCta": "Siehe Arbeitsdemo",
      "problemsEyebrow": "Wo Arbeitskräfte auslaufen",
      "problemsTitle": "Wo das Arbeitsbudget ausläuft",
      "problemsDescription": "Zeitpläne basieren auf Instinkt, Abweichungen werden erst nach der Gehaltsabrechnung entdeckt.",
      "challenges": [
        {
          "title": "OT-Exposition unsichtbar bis zur Gehaltsabrechnung",
          "description": "Wenn Sie die Überstundenlinie sehen, haben Sie diese bereits bezahlt.",
          "icon": "balance"
        },
        {
          "title": "Zeitpläne, die auf den Gästezahlen der letzten Woche basieren",
          "description": "Die Nachfrage hat sich verschoben. Der Zeitplan nicht. Der Personalkostenanteil driftet mit.",
          "icon": "operators"
        },
        {
          "title": "Produktivität gemeldet, nicht umsetzbar",
          "description": "Sie kennen die Deckungen pro FOH-Stunde und Monat. Nicht nach Schicht, nicht nach Server, nicht rechtzeitig zum Coach.",
          "icon": "performance"
        },
        {
          "title": "Keine Frühwarnung bei Fehlzeiten",
          "description": "Die schlechte Schicht beginnt, wenn um 16:15 Uhr ein Nichterscheinen eintrifft. Davon erfahren Sie in der morgigen Zusammenfassung.",
          "icon": "support"
        }
      ],
      "howTitle": "Wie Sundae für HR & People funktioniert",
      "howDescription": "Live-Personalsignal, OT-Prävention, Produktivität auf Schichtebene.",
      "howSundaeHelps": [
        {
          "title": "Live-Personalkosten in % nach Standort",
          "description": "Personalkosten pro Stunde im Verhältnis zur Nachfrage. Der Standort, der das Budget überschreitet, wird markiert, solange Sie noch kürzen können.",
          "product": "Pulse · Labor",
          "icon": "operators"
        },
        {
          "title": "OT-Exposition, bevor es zuschlägt",
          "description": "Sundae prognostiziert die OT-Linie nach Standort für den Rest der Woche - so verteilen Sie die Stunden neu und tragen nicht die Kosten.",
          "product": "Insights · Labor",
          "icon": "balance"
        },
        {
          "title": "Produktivität pro FOH-Stunde und pro Kellner",
          "description": "Gäste/Stunde, Umsatz/Stunde, Upsell-Rate - pro Kellner, pro Schicht. Coachen Sie mit Zahlen, nicht mit Eindrücken.",
          "product": "Pulse · Server view",
          "icon": "performance"
        },
        {
          "title": "Schichtempfehlungen aus der Nachfrage",
          "description": "Foresight prognostiziert die Gästezahlen 14 Tage im Voraus - Sundae schlägt den passenden Personalplan vor.",
          "product": "Foresight",
          "icon": "forecasting"
        }
      ],
      "outcomesTitle": "Was sich für HR ändert",
      "outcomesDescription": "Engere Zeitpläne, weniger Überraschungen, Produktivität, die Sie zeigen können.",
      "outcomes": [
        {
          "title": "Reduzieren Sie die OT-Belichtung, bevor sie eintrifft",
          "description": "Weisen Sie die Stunden zur Wochenmitte neu zu, nicht nach der Lohn- und Gehaltsabrechnung.",
          "icon": "speed"
        },
        {
          "title": "Der Arbeitsaufwand liegt jede Woche % näher am Plan",
          "description": "Die Varianz verringert sich vom monatlichen Chaos zur wöchentlichen Präzision.",
          "icon": "performance"
        },
        {
          "title": "Vergleichbare Kellnerproduktivität",
          "description": "Live-Produktivität pro Schicht als Grundlage für Coaching, Beurteilungen und Beförderungsentscheidungen.",
          "icon": "owners"
        },
        {
          "title": "Zeitpläne, die zum Geschehen im Lokal passen",
          "description": "Nachfragegesteuerte Zeitpläne auf Basis der Foresight-Prognosen - nicht das Muster der letzten Woche.",
          "icon": "support"
        }
      ],
      "crewCallout": {
        "eyebrow": "UNTERSTÜTZT VON SUNDAE CREW",
        "title": "Das operative Substrat, das diese Intelligenz speist.",
        "description": "Crew übernimmt Dienstplanung, Zeiterfassung, Gehaltsabrechnung in mehreren Regionen (USA, Kanada, Großbritannien, EU, GCC - eine Engine) und über 60 HR-Oberflächen - jede Schicht, jede Stunde und jeder Lohnzyklus wird zum Signal, das die Personalinformationen auf dieser Seite antreibt. Optional: Bringen Sie Ihre eigene Personalabteilung mit (Bayzat, Personio, Gusto) und Sundae liest weiterhin dasselbe Signal.",
        "ctaText": "Entdecken Sie Sundae Crew"
      },
      "ctaTitle": "Erleben Sie Ihre Arbeit in Bewegung.",
      "ctaDescription": "30 Minuten. Ihr Zeitplan. Das OT, das du gefangen hättest, bevor es gelandet wäre.",
      "ctaButton": "Buchen Sie einen Arbeitsrundgang"
    },
    "fr": {
      "badge": "Pour les équipes RH et People",
      "titleLine1": "Un planning calé sur le service.",
      "titleLine2": "Pas pour la semaine dernière.",
      "description": "Variation du travail en direct par emplacement. Le risque d’heures supplémentaires est signalé avant qu’il ne se produise. Productivité mesurée par heure FOH et non par période de paie.",
      "primaryCta": "Réservez une visite guidée du travail",
      "secondaryCta": "Voir la démo du travail",
      "problemsEyebrow": "OÙ LA MAIN-D'ŒUVRE FUITE",
      "problemsTitle": "Là où le budget du travail fuit",
      "problemsDescription": "Horaires construits à l'instinct, écart découvert après la paie.",
      "challenges": [
        {
          "title": "Exposition OT invisible jusqu'à la paie",
          "description": "Au moment où vous voyez la ligne des heures supplémentaires, vous les avez déjà payées.",
          "icon": "balance"
        },
        {
          "title": "Horaires bâtis sur les couverts de la semaine dernière",
          "description": "La demande a évolué. Pas le planning. Le % de main-d'œuvre dérive avec elle.",
          "icon": "operators"
        },
        {
          "title": "Productivité signalée, non exploitable",
          "description": "Vous connaissez les couverts par heure FOH et par mois. Pas par service, pas par serveur, pas à temps pour coacher.",
          "icon": "performance"
        },
        {
          "title": "Pas d'alerte précoce en cas d'absentéisme",
          "description": "Le mauvais quart de travail commence lorsqu'une non-présentation arrive à 16h15. Vous en entendrez parler dans le récapitulatif de demain.",
          "icon": "support"
        }
      ],
      "howTitle": "Comment Sundae fonctionne pour les RH et les ressources humaines",
      "howDescription": "Signal de travail en direct, prévention des OT, productivité à la granularité des équipes.",
      "howSundaeHelps": [
        {
          "title": "% de main d'œuvre vivante par emplacement",
          "description": "Coût de la main-d'œuvre heure par heure par rapport à la demande. Le site qui dépasse son budget est signalé alors que vous pouvez encore le réduire.",
          "product": "Pulse · Labor",
          "icon": "operators"
        },
        {
          "title": "Exposition OT avant qu'elle ne frappe",
          "description": "Sundae prévoit la ligne OT par emplacement pour le reste de la semaine - vous redistribuez ainsi les heures, sans absorber les coûts.",
          "product": "Insights · Labor",
          "icon": "balance"
        },
        {
          "title": "Productivité par heure FOH et par serveur",
          "description": "Couverts/heure, ventes/heure, taux de vente incitative - par serveur, par service. Coachez avec des chiffres, pas des impressions.",
          "product": "Pulse · Server view",
          "icon": "performance"
        },
        {
          "title": "Recommandations de planning à partir de la demande",
          "description": "Foresight projette les couverts à 14 jours - Sundae suggère le plan de personnel correspondant.",
          "product": "Foresight",
          "icon": "forecasting"
        }
      ],
      "outcomesTitle": "Quels changements pour les RH",
      "outcomesDescription": "Des horaires plus serrés, moins de surprises, une productivité dont vous pouvez faire preuve.",
      "outcomes": [
        {
          "title": "Réduisez l’exposition à l’OT avant qu’il n’atterrisse",
          "description": "Réaffectez les heures en milieu de semaine, et non après le traitement de la paie.",
          "icon": "speed"
        },
        {
          "title": "% de main d'œuvre plus proche du plan, chaque semaine",
          "description": "La variance se réduit du chaos mensuel à la précision hebdomadaire.",
          "icon": "performance"
        },
        {
          "title": "Une productivité de serveur comparable",
          "description": "Productivité en direct par équipe qui prend en charge le coaching, les évaluations et les appels de promotion.",
          "icon": "owners"
        },
        {
          "title": "Des horaires adaptés à la salle",
          "description": "Des horaires pilotés par la demande, bâtis sur les projections Foresight - pas le schéma de la semaine dernière.",
          "icon": "support"
        }
      ],
      "crewCallout": {
        "eyebrow": "PROPULSÉ PAR SUNDAE CREW",
        "title": "Le substrat opérationnel qui alimente cette intelligence.",
        "description": "Crew gère la planification, le temps et les présences, la paie multirégionale (États-Unis, Canada, Royaume-Uni, UE, GCC - un seul moteur) et plus de 60 surfaces RH - chaque quart, chaque heure et chaque cycle de paie devient le signal qui alimente l'intelligence de la main-d'œuvre sur cette page. Facultatif : apportez votre propre HR (Bayzat, Personio, Gusto) et Sundae lit toujours le même signal.",
        "ctaText": "Explore Sundae Crew"
      },
      "ctaTitle": "Voyez votre travail en mouvement.",
      "ctaDescription": "30 minutes. Votre emploi du temps. L'OT que vous auriez attrapé avant son atterrissage.",
      "ctaButton": "Réservez une visite guidée du travail"
    },
    "pt": {
      "badge": "Para equipes de RH e pessoas",
      "titleLine1": "Cronograma para o turno.",
      "titleLine2": "Não na semana passada.",
      "description": "Variação de mão de obra ativa por local. Risco de horas extras sinalizado antes de acontecer. Produtividade medida por hora FOH, não por período de pagamento.",
      "primaryCta": "Agende uma avaliação trabalhista",
      "secondaryCta": "Veja Demonstração Trabalhista",
      "problemsEyebrow": "ONDE A MÃO DE OBRA VAZA",
      "problemsTitle": "Onde o orçamento de mão de obra vaza",
      "problemsDescription": "Cronogramas construídos por instinto, variações descobertas após a folha de pagamento.",
      "challenges": [
        {
          "title": "Exposição OT invisível até a folha de pagamento",
          "description": "No momento em que você vê a linha das horas extras, você já pagou.",
          "icon": "balance"
        },
        {
          "title": "Escalas baseadas nos clientes da semana passada",
          "description": "A demanda mudou. A escala não. A % de mão de obra desvia junto.",
          "icon": "operators"
        },
        {
          "title": "Produtividade relatada, não acionável",
          "description": "Você conhece os clientes por hora FOH por mês. Nem por turno, nem por garçom, nem a tempo de treinar.",
          "icon": "performance"
        },
        {
          "title": "Nenhum aviso prévio sobre absentismo",
          "description": "O turno ruim começa quando um não comparecimento chega às 16h15. Você ouvirá sobre isso na recapitulação de amanhã.",
          "icon": "support"
        }
      ],
      "howTitle": "Como o Sundae funciona para RH e Pessoas",
      "howDescription": "Sinal de trabalho ativo, prevenção de TO, produtividade na granularidade do turno.",
      "howSundaeHelps": [
        {
          "title": "% de mão de obra viva por local",
          "description": "Custo de mão de obra hora a hora em relação à demanda. O site que ultrapassou o orçamento é sinalizado enquanto você ainda pode cortar.",
          "product": "Pulse · Labor",
          "icon": "operators"
        },
        {
          "title": "Exposição OT antes de atingir",
          "description": "Sundae prevê a linha OT por local para o resto da semana - para que você redistribua as horas, não absorva o custo.",
          "product": "Insights · Labor",
          "icon": "balance"
        },
        {
          "title": "Produtividade por hora FOH, por garçom",
          "description": "Clientes/hora, vendas/hora, taxa de upsell - por garçom, por turno. Treine com números, não com impressões.",
          "product": "Pulse · Server view",
          "icon": "performance"
        },
        {
          "title": "Recomendações de escala a partir da demanda",
          "description": "O Foresight projeta os clientes com 14 dias de antecedência - o Sundae sugere o plano de mão de obra correspondente.",
          "product": "Foresight",
          "icon": "forecasting"
        }
      ],
      "outcomesTitle": "O que muda para o RH",
      "outcomesDescription": "Prazos mais apertados, menos surpresas, produtividade que você pode demonstrar.",
      "outcomes": [
        {
          "title": "Reduza a exposição ao OT antes que ele chegue",
          "description": "Realoque as horas no meio da semana, não após a execução da folha de pagamento.",
          "icon": "speed"
        },
        {
          "title": "% de mão de obra mais próxima do planejado, toda semana",
          "description": "A variação diminui do caos mensal para a precisão semanal.",
          "icon": "performance"
        },
        {
          "title": "Produtividade do garçom que você pode comparar",
          "description": "Produtividade ao vivo por turno que embasa treinamento, avaliações e decisões de promoção.",
          "icon": "owners"
        },
        {
          "title": "Escalas que combinam com o salão",
          "description": "Escalas orientadas pela demanda, construídas sobre as projeções do Foresight - não sobre o padrão da semana passada.",
          "icon": "support"
        }
      ],
      "crewCallout": {
        "eyebrow": "APOIADO PELA EQUIPE DO SUNDAE",
        "title": "O substrato operacional que alimenta esta inteligência.",
        "description": "O Crew administra escala, ponto e presença, folha de pagamento multirregional (EUA, Canadá, Reino Unido, UE, GCC - um motor) e mais de 60 superfícies de RH - cada turno, hora e ciclo de pagamento se torna um sinal que alimenta a inteligência de mão de obra nesta página. Opcional: traga seu próprio RH (Bayzat, Personio, Gusto) e o Sundae continua lendo o mesmo sinal.",
        "ctaText": "Explore Sundae Crew"
      },
      "ctaTitle": "Veja seu trabalho em movimento.",
      "ctaDescription": "30 minutos. Sua programação. O OT que você teria capturado antes de pousar.",
      "ctaButton": "Agende uma avaliação trabalhista"
    },
    "nl": {
      "badge": "Voor HR- en people-teams",
      "titleLine1": "Schema voor de dienst.",
      "titleLine2": "Niet voor vorige week.",
      "description": "Verschil in levende arbeid per locatie. Het risico van overwerk wordt gemarkeerd voordat het zich voordoet. Productiviteit gemeten per FOH-uur, niet per betaalperiode.",
      "primaryCta": "Boek een Labor Walk-through",
      "secondaryCta": "Zie Arbeidsdemo",
      "problemsEyebrow": "WAAR ARBEID LEKT",
      "problemsTitle": "Waar het arbeidsbudget lekt",
      "problemsDescription": "Schema's gebouwd op instinct, variantie ontdekt na loonadministratie.",
      "challenges": [
        {
          "title": "OT-blootstelling onzichtbaar tot aan de salarisadministratie",
          "description": "Tegen de tijd dat u de overurenlijn ziet, heeft u deze al betaald.",
          "icon": "balance"
        },
        {
          "title": "Schema's gebaseerd op de gasten van vorige week",
          "description": "De vraag is verschoven. Het rooster niet. Het arbeidspercentage drijft mee.",
          "icon": "operators"
        },
        {
          "title": "Productiviteit gerapporteerd, niet uitvoerbaar",
          "description": "U kent de gasten per FOH-uur per maand. Niet per dienst, niet per bediening, niet op tijd om te coachen.",
          "icon": "performance"
        },
        {
          "title": "Geen vroegtijdige waarschuwing bij verzuim",
          "description": "De slechte dienst begint wanneer er om 16.15 uur een no-show is. Je hoort erover in de samenvatting van morgen.",
          "icon": "support"
        }
      ],
      "howTitle": "Hoe Sundae werkt voor HR & People",
      "howDescription": "Live arbeidssignaal, OT-preventie, productiviteit bij granulariteit van de ploegendienst.",
      "howSundaeHelps": [
        {
          "title": "Levende arbeid% per locatie",
          "description": "Arbeidskosten per uur vergeleken met de vraag. De site die het budget overschrijdt, wordt gemarkeerd terwijl u nog kunt trimmen.",
          "product": "Pulse · Labor",
          "icon": "operators"
        },
        {
          "title": "OT-blootstelling voordat het toeslaat",
          "description": "Sundae voorspelt de OT-lijn per locatie voor de rest van de week, zodat u de uren opnieuw verdeelt en niet de kosten op zich neemt.",
          "product": "Insights · Labor",
          "icon": "balance"
        },
        {
          "title": "Productiviteit per FOH-uur, per bediening",
          "description": "Gasten/uur, omzet/uur, upsell-percentage - per bediening, per dienst. Coach met cijfers, niet met indrukken.",
          "product": "Pulse · Server view",
          "icon": "performance"
        },
        {
          "title": "Plan aanbevelingen op basis van de vraag",
          "description": "Foresight-projecten bestrijken 14 dagen - Sundae stelt het arbeidsplan voor dat daarbij past.",
          "product": "Foresight",
          "icon": "forecasting"
        }
      ],
      "outcomesTitle": "Wat verandert er voor HR",
      "outcomesDescription": "Strakkere schema's, minder verrassingen, productiviteit die u kunt laten zien.",
      "outcomes": [
        {
          "title": "Verminder de OT-blootstelling voordat deze landt",
          "description": "Verdeel de uren halverwege de week opnieuw, niet na de loonadministratie.",
          "icon": "speed"
        },
        {
          "title": "Arbeid % dichter bij plan, elke week",
          "description": "De variantie verkleint van maandelijkse chaos naar wekelijkse precisie.",
          "icon": "performance"
        },
        {
          "title": "Bedieningsproductiviteit die u kunt vergelijken",
          "description": "Live productiviteit per dienst die coaching, beoordelingen en promotiebeslissingen ondersteunt.",
          "icon": "owners"
        },
        {
          "title": "Roosters die passen bij de vloer",
          "description": "Vraaggestuurde roosters gebouwd op Foresight-projecties - niet het patroon van vorige week.",
          "icon": "support"
        }
      ],
      "crewCallout": {
        "eyebrow": "AANGEDREVEN DOOR SUNDAE CREW",
        "title": "Het operationele substraat dat deze intelligentie voedt.",
        "description": "Crew beheert planning, tijdregistratie, loonadministratie voor meerdere regio's (VS, Canada, VK, EU, GCC - één engine) en meer dan 60 HR-oppervlakken - elke dienst, uur en betaalcyclus wordt een signaal dat de arbeidsintelligentie op deze pagina aanstuurt. Optioneel: neem je eigen HR mee (Bayzat, Personio, Gusto) en Sundae leest nog steeds hetzelfde signaal.",
        "ctaText": "Ontdek Sundae Crew"
      },
      "ctaTitle": "Zie uw personeel in beweging.",
      "ctaDescription": "30 minuten. Jouw schema. De OT die je zou hebben gepakt voordat hij landde.",
      "ctaButton": "Boek een Labor Walk-through"
    },
    "ur": {
      "badge": "HR اور لوگوں کی ٹیموں کے لیے",
      "titleLine1": "شفٹ کے لیے شیڈول۔",
      "titleLine2": "پچھلے ہفتے کے لیے نہیں۔",
      "description": "مقام کے لحاظ سے لیبر کا براہ راست تغیر۔ اوور ٹائم کے خطرے کو ہونے سے پہلے ہی جھنڈا لگا دیا گیا۔ پیداواری صلاحیت فی FOH گھنٹہ ماپا جاتا ہے، فی تنخواہ کی مدت نہیں۔",
      "primaryCta": "لیبر واک تھرو بک کریں۔",
      "secondaryCta": "لیبر ڈیمو دیکھیں",
      "problemsEyebrow": "جہاں لیبر لیک ہوتی ہے۔",
      "problemsTitle": "جہاں لیبر بجٹ لیک ہو جاتا ہے۔",
      "problemsDescription": "جبلت پر بنائے گئے نظام الاوقات، تنخواہ کے بعد دریافت ہونے والا تغیر۔",
      "challenges": [
        {
          "title": "پے رول تک OT کی نمائش پوشیدہ ہے۔",
          "description": "جب تک آپ اوور ٹائم لائن دیکھیں گے، آپ اسے پہلے ہی ادا کر چکے ہوں گے۔",
          "icon": "balance"
        },
        {
          "title": "پچھلے ہفتے کے کور پر بنائے گئے شیڈولز",
          "description": "مطالبہ منتقل ہو گیا ہے۔ شیڈول میں نہیں ہے۔ لیبر % اس کے ساتھ بہتی ہے۔",
          "icon": "operators"
        },
        {
          "title": "پیداواریت کی اطلاع دی گئی، قابل عمل نہیں۔",
          "description": "آپ جانتے ہیں کہ فی FOH گھنٹے کے حساب سے ماہانہ کور۔ شفٹ کے ذریعے نہیں، سرور کے ذریعے نہیں، کوچنگ کے وقت نہیں۔",
          "icon": "performance"
        },
        {
          "title": "غیر حاضری پر کوئی ابتدائی انتباہ نہیں۔",
          "description": "خراب شفٹ اس وقت شروع ہوتی ہے جب شام 4:15 پر کوئی نو-شو ہوتا ہے۔ آپ کل کے خلاصے میں اس کے بارے میں سنیں گے۔",
          "icon": "support"
        }
      ],
      "howTitle": "Sundae HR اور لوگوں کے لیے کیسے کام کرتا ہے۔",
      "howDescription": "لائیو لیبر سگنل، OT کی روک تھام، شفٹ گرینولریٹی پر پیداوری۔",
      "howSundaeHelps": [
        {
          "title": "لائیو لیبر % مقام کے لحاظ سے",
          "description": "طلب کے مقابلے میں گھنٹے بہ گھنٹہ مزدوری کی قیمت۔ بجٹ سے زیادہ چلنے والی سائٹ کو جھنڈا لگایا جاتا ہے جب کہ آپ ابھی بھی تراش سکتے ہیں۔",
          "product": "Pulse · Labor",
          "icon": "operators"
        },
        {
          "title": "OT کی نمائش اس کے لگنے سے پہلے",
          "description": "Sundae باقی ہفتے کے لیے مقام کے لحاظ سے OT لائن کی پیشن گوئی کرتا ہے - اس لیے آپ گھنٹے کی دوبارہ تقسیم کرتے ہیں، لاگت کو جذب نہیں کرتے۔",
          "product": "Insights · Labor",
          "icon": "balance"
        },
        {
          "title": "پیداواری صلاحیت فی FOH گھنٹہ، فی سرور",
          "description": "کور/گھنٹہ، سیلز/گھنٹہ، اپ سیل ریٹ - فی سرور، فی شفٹ۔ نمبروں کے ساتھ کوچ کریں، نقوش نہیں۔",
          "product": "Pulse · Server view",
          "icon": "performance"
        },
        {
          "title": "مانگ سے شیڈول کی سفارشات",
          "description": "Foresight 14 دن پہلے مہمانوں کی تعداد کی پیش گوئی کرتا ہے - Sundae اس سے مماثل لیبر پلان تجویز کرتا ہے۔",
          "product": "Foresight",
          "icon": "forecasting"
        }
      ],
      "outcomesTitle": "HR کے لیے کیا تبدیلیاں؟",
      "outcomesDescription": "سخت نظام الاوقات، کم حیرت، پیداواری صلاحیت جو آپ دکھا سکتے ہیں۔",
      "outcomes": [
        {
          "title": "اترنے سے پہلے OT کی نمائش کاٹ دیں۔",
          "description": "ہفتے کے وسط میں اوقات دوبارہ ترتیب دیں، پے رول چلانے کے بعد نہیں۔",
          "icon": "speed"
        },
        {
          "title": "لیبر % پلان کے قریب، ہر ہفتے",
          "description": "تغیر ماہانہ افراتفری سے ہفتہ وار درستگی تک کم ہوتا ہے۔",
          "icon": "performance"
        },
        {
          "title": "سرور کی پیداواری صلاحیت جس کا آپ موازنہ کر سکتے ہیں۔",
          "description": "لائیو، فی شفٹ پروڈکٹیوٹی جو کوچنگ، جائزوں اور پروموشن کالز کو سپورٹ کرتی ہے۔",
          "icon": "owners"
        },
        {
          "title": "شیڈول جو ہال سے میل کھاتے ہیں۔",
          "description": "مانگ پر مبنی شیڈول جو Foresight کی پیش گوئیوں پر بنائے گئے ہیں - نہ کہ پچھلے ہفتے کے انداز پر۔",
          "icon": "support"
        }
      ],
      "crewCallout": {
        "eyebrow": "SUNDAE کریو کی حمایت یافتہ",
        "title": "آپریشنل سبسٹریٹ جو اس ذہانت کو کھلاتا ہے۔",
        "description": "عملہ شیڈولنگ، وقت اور حاضری، ملٹی ریجن پے رول (US, Canada, UK, EU, GCC - ایک انجن) اور 60+ HR سطحوں پر چلتا ہے - ہر شفٹ، گھنٹہ، اور پے سائیکل سگنل بنتا ہے جو اس صفحہ پر لیبر انٹیلی جنس کو طاقت دیتا ہے۔ اختیاری: اپنا HR لائیں (Bayzat, Personio, Gusto) اور Sundae پھر بھی وہی سگنل پڑھتا ہے۔",
        "ctaText": "Sundae Crew کو دریافت کریں۔"
      },
      "ctaTitle": "اپنی محنت کو حرکت میں دیکھیں۔",
      "ctaDescription": "30 منٹ آپ کا شیڈول۔ جس OT کو آپ نے اترنے سے پہلے پکڑ لیا ہوگا۔",
      "ctaButton": "لیبر واک تھرو بک کریں۔"
    },
    "hi": {
      "badge": "मानव संसाधन एवं लोगों की टीमों के लिए",
      "titleLine1": "शिफ्ट के लिए शेड्यूल.",
      "titleLine2": "पिछले सप्ताह के लिए नहीं.",
      "description": "स्थान के अनुसार लाइव श्रम भिन्नता। ओवरटाइम जोखिम होने से पहले ही चिह्नित कर लिया गया। उत्पादकता प्रति एफओएच घंटे मापी जाती है, प्रति भुगतान अवधि नहीं।",
      "primaryCta": "लेबर वॉक-थ्रू बुक करें",
      "secondaryCta": "लेबर डेमो देखें",
      "problemsEyebrow": "जहां श्रम लीक होता है",
      "problemsTitle": "जहां श्रम बजट लीक हो जाता है",
      "problemsDescription": "अनुसूचियाँ वृत्ति पर निर्मित होती हैं, पेरोल के बाद भिन्नता का पता चलता है।",
      "challenges": [
        {
          "title": "पेरोल तक ओटी एक्सपोज़र अदृश्य",
          "description": "जब तक आप ओवरटाइम लाइन देखते हैं, तब तक आप उसका भुगतान कर चुके होते हैं।",
          "icon": "balance"
        },
        {
          "title": "पिछले सप्ताह के कवर पर बनाए गए शेड्यूल",
          "description": "मांग आगे बढ़ी है. शेड्यूल नहीं है. श्रम % इसके साथ बह जाता है।",
          "icon": "operators"
        },
        {
          "title": "उत्पादकता रिपोर्ट की गई, कार्रवाई योग्य नहीं",
          "description": "आप महीने दर महीने प्रति एफओएच घंटे कवर जानते हैं। न शिफ्ट के हिसाब से, न सर्वर के हिसाब से, न कोच के समय पर।",
          "icon": "performance"
        },
        {
          "title": "अनुपस्थिति पर कोई पूर्व चेतावनी नहीं",
          "description": "ख़राब शिफ्ट तब शुरू होती है जब शाम 4:15 बजे कोई नो-शो होता है। आप इसके बारे में कल के सारांश में सुनेंगे।",
          "icon": "support"
        }
      ],
      "howTitle": "Sundae एचआर और लोगों के लिए कैसे काम करता है",
      "howDescription": "लाइव लेबर सिग्नल, ओटी रोकथाम, शिफ्ट ग्रैन्युलैरिटी पर उत्पादकता।",
      "howSundaeHelps": [
        {
          "title": "स्थान के अनुसार लाइव श्रम %",
          "description": "मांग के विरुद्ध घंटे-दर-घंटे श्रम लागत। बजट से अधिक चल रही साइट को चिह्नित किया जाता है जबकि आप अभी भी ट्रिम कर सकते हैं।",
          "product": "Pulse · Labor",
          "icon": "operators"
        },
        {
          "title": "हिट होने से पहले ओटी एक्सपोज़र",
          "description": "Sundae सप्ताह के बाकी दिनों के लिए स्थान के आधार पर ओटी लाइन का पूर्वानुमान लगाता है - इसलिए आप घंटों का पुनर्वितरण करते हैं, लागत को अवशोषित नहीं करते हैं।",
          "product": "Insights · Labor",
          "icon": "balance"
        },
        {
          "title": "प्रति FOH घंटे, प्रति सर्वर उत्पादकता",
          "description": "कवर/घंटा, बिक्री/घंटा, अपसेल दर - प्रति सर्वर, प्रति शिफ्ट। संख्याओं से प्रशिक्षित करें, छापों से नहीं।",
          "product": "Pulse · Server view",
          "icon": "performance"
        },
        {
          "title": "मांग से शेड्यूल की अनुशंसाएँ",
          "description": "Foresight 14 दिन पहले मेहमानों की संख्या का पूर्वानुमान लगाता है - Sundae उससे मेल खाने वाली श्रम योजना सुझाता है।",
          "product": "Foresight",
          "icon": "forecasting"
        }
      ],
      "outcomesTitle": "एचआर के लिए क्या बदलाव?",
      "outcomesDescription": "सख्त कार्यक्रम, कम आश्चर्य, उत्पादकता जो आप दिखा सकते हैं।",
      "outcomes": [
        {
          "title": "उतरने से पहले ओटी एक्सपोज़र को काट दें",
          "description": "सप्ताह के मध्य में घंटों का पुनः आबंटन करें, पेरोल चलने के बाद नहीं।",
          "icon": "speed"
        },
        {
          "title": "श्रम % योजना के करीब, हर सप्ताह",
          "description": "भिन्नता मासिक अराजकता से साप्ताहिक परिशुद्धता तक सीमित हो जाती है।",
          "icon": "performance"
        },
        {
          "title": "सर्वर उत्पादकता की आप तुलना कर सकते हैं",
          "description": "लाइव, प्रति-शिफ्ट उत्पादकता जो कोचिंग, समीक्षा और प्रचार कॉल का समर्थन करती है।",
          "icon": "owners"
        },
        {
          "title": "शेड्यूल जो फ़्लोर से मेल खाते हैं",
          "description": "मांग-संचालित शेड्यूल जो Foresight के पूर्वानुमानों पर बने हैं - न कि पिछले सप्ताह के पैटर्न पर।",
          "icon": "support"
        }
      ],
      "crewCallout": {
        "eyebrow": "संडे क्रू द्वारा समर्थित",
        "title": "परिचालन सब्सट्रेट जो इस बुद्धिमत्ता को पोषित करता है।",
        "description": "क्रू शेड्यूलिंग, समय और उपस्थिति, बहु-क्षेत्र पेरोल (यूएस, कनाडा, यूके, ईयू, जीसीसी - एक इंजन), और 60+ एचआर सतह - प्रत्येक शिफ्ट, घंटे और वेतन-चक्र को सिग्नल बनाता है जो इस पृष्ठ पर श्रम खुफिया को शक्ति प्रदान करता है। वैकल्पिक: अपना स्वयं का एचआर (बेज़ैट, पर्सोनियो, गुस्टो) लाएँ और Sundae अभी भी वही सिग्नल पढ़ता है।",
        "ctaText": "Explore Sundae Crew"
      },
      "ctaTitle": "अपने श्रम को गति में देखें।",
      "ctaDescription": "30 मिनट. आपका शेड्यूल. ओटी को आपने उतरने से पहले ही पकड़ लिया होगा।",
      "ctaButton": "लेबर वॉक-थ्रू बुक करें"
    },
    "pl": {
      "badge": "Dla zespołów HR i personalnych",
      "titleLine1": "Harmonogram zmiany.",
      "titleLine2": "Nie przez ostatni tydzień.",
      "description": "Różnice w pracy na żywo według lokalizacji. Ryzyko wystąpienia nadgodzin jest sygnalizowane, zanim do nich dojdzie. Produktywność mierzona na godzinę FOH, a nie na okres rozliczeniowy.",
      "primaryCta": "Zarezerwuj spacer po pracy",
      "secondaryCta": "Zobacz Demo Pracy",
      "problemsEyebrow": "GDZIE WYCIEKA ROBOCIZNA",
      "problemsTitle": "Gdzie przecieka budżet na personel",
      "problemsDescription": "Harmonogramy tworzone na podstawie instynktu, rozbieżności wykryte po rozliczeniu płac.",
      "challenges": [
        {
          "title": "Ekspozycja OT niewidoczna do momentu wypłaty",
          "description": "Zanim zobaczysz linię za nadgodziny, już je zapłaciłeś.",
          "icon": "balance"
        },
        {
          "title": "Harmonogramy oparte na gościach z zeszłego tygodnia",
          "description": "Popyt się przesunął. Harmonogram nie. % robocizny dryfuje wraz z nim.",
          "icon": "operators"
        },
        {
          "title": "Produktywność zgłoszona, brak możliwości podjęcia działań",
          "description": "Znasz liczbę gości na godzinę FOH w skali miesiąca. Nie na zmianę, nie na kelnera, nie na czas, by coachować.",
          "icon": "performance"
        },
        {
          "title": "Brak wczesnego ostrzegania o nieobecności",
          "description": "Zła zmiana zaczyna się, gdy niestawienie się ma miejsce o 16:15. O tym usłyszycie w jutrzejszym podsumowaniu.",
          "icon": "support"
        }
      ],
      "howTitle": "Jak Sundae działa dla HR i ludzi",
      "howDescription": "Sygnał pracy na żywo, zapobieganie OT, produktywność przy szczegółowości zmiany.",
      "howSundaeHelps": [
        {
          "title": "% siły roboczej na żywo według lokalizacji",
          "description": "Koszt pracy godzina po godzinie w stosunku do popytu. Witryna, której budżet przekracza budżet, zostaje oznaczona, podczas gdy nadal można ją przyciąć.",
          "product": "Pulse · Labor",
          "icon": "operators"
        },
        {
          "title": "Ekspozycja OT, zanim uderzy",
          "description": "Sundae prognozuje linię OT według lokalizacji na resztę tygodnia, dzięki czemu możesz redystrybuować godziny, a nie przejmować koszty.",
          "product": "Insights · Labor",
          "icon": "balance"
        },
        {
          "title": "Wydajność na godzinę FOH, na kelnera",
          "description": "Goście/godzinę, sprzedaż/godzinę, wskaźnik dosprzedaży - na kelnera, na zmianę. Coachuj liczbami, a nie wrażeniami.",
          "product": "Pulse · Server view",
          "icon": "performance"
        },
        {
          "title": "Rekomendacje grafiku na podstawie popytu",
          "description": "Foresight prognozuje liczbę gości na 14 dni naprzód - Sundae sugeruje dopasowany plan personelu.",
          "product": "Foresight",
          "icon": "forecasting"
        }
      ],
      "outcomesTitle": "Jakie zmiany dla HR",
      "outcomesDescription": "Napięte harmonogramy, mniej niespodzianek, produktywność, którą możesz wykazać.",
      "outcomes": [
        {
          "title": "Zmniejsz ekspozycję OT, zanim wyląduje",
          "description": "Realokuj godziny pracy w środku tygodnia, a nie po zakończeniu listy płac.",
          "icon": "speed"
        },
        {
          "title": "Pracy % bliżej planu, co tydzień",
          "description": "Wariancja zawęża się od miesięcznego chaosu do tygodniowej precyzji.",
          "icon": "performance"
        },
        {
          "title": "Produktywność kelnerów, którą możesz porównać",
          "description": "Produktywność na żywo na zmianę, która wspiera coaching, oceny i decyzje o awansie.",
          "icon": "owners"
        },
        {
          "title": "Harmonogramy dopasowane do podłogi",
          "description": "Harmonogramy oparte na popycie oparte na prognozach Foresight - a nie na wzorcu z zeszłego tygodnia.",
          "icon": "support"
        }
      ],
      "crewCallout": {
        "eyebrow": "NAPĘDZANE PRZEZ SUNDAE CREW",
        "title": "Operacyjne podłoże, które zasila tę inteligencję.",
        "description": "Crew zarządza grafikiem, czasem i obecnością, listą płac w wielu regionach (USA, Kanada, Wielka Brytania, UE, GCC - jeden silnik) i ponad 60 powierzchniami HR - każda zmiana, godzina i cykl płac staje się sygnałem napędzającym inteligencję personalną na tej stronie. Opcjonalnie: przynieś własne HR (Bayzat, Personio, Gusto), a Sundae nadal odczytuje ten sam sygnał.",
        "ctaText": "Poznaj Sundae Crew"
      },
      "ctaTitle": "Zobacz swoją pracę w ruchu.",
      "ctaDescription": "30 minut. Twój harmonogram. OT, który złapałbyś zanim wylądował.",
      "ctaButton": "Zarezerwuj spacer po pracy"
    },
    "it": {
      "badge": "Per i team delle risorse umane e del personale",
      "titleLine1": "Programma per il turno.",
      "titleLine2": "Non per la settimana scorsa.",
      "description": "Variazione della manodopera viva in base alla località. Il rischio di straordinario è segnalato prima che si verifichi. Produttività misurata per ora FOH, non per periodo di paga.",
      "primaryCta": "Prenota una visita guidata del lavoro",
      "secondaryCta": "Vedi Demo del lavoro",
      "problemsEyebrow": "DOVE SI DISPERDE LA MANODOPERA",
      "problemsTitle": "Dove si disperde il budget del personale",
      "problemsDescription": "Orari costruiti sull'istinto, varianza scoperta dopo il libro paga.",
      "challenges": [
        {
          "title": "Esposizione OT invisibile fino al libro paga",
          "description": "Quando vedi la riga dello straordinario, lo hai già pagato.",
          "icon": "balance"
        },
        {
          "title": "Orari basati sui coperti della settimana scorsa",
          "description": "La domanda si è spostata. L'orario no. La percentuale di manodopera va alla deriva con essa.",
          "icon": "operators"
        },
        {
          "title": "Produttività segnalata, non utilizzabile",
          "description": "Conosci i coperti per ora FOH al mese. Non per turno, non per cameriere, non in tempo per fare coaching.",
          "icon": "performance"
        },
        {
          "title": "Nessun preavviso sull’assenteismo",
          "description": "Il brutto turno inizia quando una mancata presentazione arriva alle 16:15. Ne sentirete parlare nel riepilogo di domani.",
          "icon": "support"
        }
      ],
      "howTitle": "Come funziona Sundae per le risorse umane e le persone",
      "howDescription": "Segnale di manodopera in tempo reale, prevenzione OT, produttività alla granularità del turno.",
      "howSundaeHelps": [
        {
          "title": "% manodopera viva per località",
          "description": "Costo del lavoro orario rispetto alla domanda. Il sito che supera il budget viene contrassegnato mentre è ancora possibile ritagliarlo.",
          "product": "Pulse · Labor",
          "icon": "operators"
        },
        {
          "title": "Esposizione OT prima che colpisca",
          "description": "Sundae prevede la linea OT in base alla posizione per il resto della settimana, in modo da ridistribuire le ore e non assorbire i costi.",
          "product": "Insights · Labor",
          "icon": "balance"
        },
        {
          "title": "Produttività per ora FOH, per cameriere",
          "description": "Coperti/ora, vendite/ora, tasso di upselling: per cameriere, per turno. Fai coaching con i numeri, non con le impressioni.",
          "product": "Pulse · Server view",
          "icon": "performance"
        },
        {
          "title": "Consigli di turnazione dalla domanda",
          "description": "Foresight proietta i coperti con 14 giorni di anticipo - Sundae suggerisce il piano del personale corrispondente.",
          "product": "Foresight",
          "icon": "forecasting"
        }
      ],
      "outcomesTitle": "Cosa cambia per le risorse umane",
      "outcomesDescription": "Programmi più serrati, meno sorprese, produttività che puoi mostrare.",
      "outcomes": [
        {
          "title": "Taglia l'esposizione OT prima che arrivi",
          "description": "Ridistribuire le ore a metà settimana, non dopo l'esecuzione delle buste paga.",
          "icon": "speed"
        },
        {
          "title": "% di manodopera più vicina al piano, ogni settimana",
          "description": "La varianza si restringe dal caos mensile alla precisione settimanale.",
          "icon": "performance"
        },
        {
          "title": "Produttività del cameriere che puoi confrontare",
          "description": "Produttività in tempo reale, per turno, che supporta coaching, valutazioni e decisioni di promozione.",
          "icon": "owners"
        },
        {
          "title": "Orari che rispecchiano la sala",
          "description": "Orari guidati dalla domanda, costruiti sulle proiezioni di Foresight - non sul ritmo della settimana scorsa.",
          "icon": "support"
        }
      ],
      "crewCallout": {
        "eyebrow": "ALIMENTATO DA SUNDAE CREW",
        "title": "Il substrato operativo che alimenta questa intelligenza.",
        "description": "Crew gestisce la pianificazione dei turni, orari e presenze, buste paga multi-regione (Stati Uniti, Canada, Regno Unito, UE, GCC - un solo motore) e oltre 60 superfici HR: ogni turno, ora e ciclo di retribuzione diventa un segnale che alimenta l'intelligence sulla manodopera in questa pagina. Opzionale: porta il tuo HR (Bayzat, Personio, Gusto) e Sundae continuerà a leggere lo stesso segnale.",
        "ctaText": "Esplora Sundae Crew"
      },
      "ctaTitle": "Guarda la tua manodopera in movimento.",
      "ctaDescription": "30 minuti. Il tuo programma. L'OT che avresti catturato prima che atterrasse.",
      "ctaButton": "Prenota una visita guidata del lavoro"
    },
    "zh-Hans": {
      "badge": "对于人力资源和人员团队",
      "titleLine1": "为当前班次排班。",
      "titleLine2": "而非沿用上周。",
      "description": "不同地点的实时劳动力差异。加班风险在发生之前就被标记出来。生产力是按 FOH 小时而不是按工资周期来衡量的。",
      "primaryCta": "预约劳工演练",
      "secondaryCta": "查看劳动演示",
      "problemsEyebrow": "劳动力流失的地方",
      "problemsTitle": "劳动力预算泄漏的地方",
      "problemsDescription": "时间表建立在直觉上，工资发放后发现差异。",
      "challenges": [
        {
          "title": "加班暴露在工资发放之前是看不见的",
          "description": "当你看到加班线时，你已经付了钱。",
          "icon": "balance"
        },
        {
          "title": "根据上周客流量制定的排班",
          "description": "需求已经变化，排班却没有。劳动力百分比随之偏移。",
          "icon": "operators"
        },
        {
          "title": "已报告生产力，但不可采取行动",
          "description": "您只知道每月每个 FOH 小时的客流量。不分班次，不分服务员，也来不及辅导。",
          "icon": "performance"
        },
        {
          "title": "没有缺勤预警",
          "description": "当下午 4:15 有人缺勤未到时，糟糕的班次就开始了。你要到明天的回顾里才会知道。",
          "icon": "support"
        }
      ],
      "howTitle": "Sundae如何为人力资源和人员服务",
      "howDescription": "实时劳动力信号、加班预防、轮班粒度的生产力。",
      "howSundaeHelps": [
        {
          "title": "按地点划分的活劳动力百分比",
          "description": "按小时计算的劳动力成本与需求的关系。超出预算的网站会被标记，而您仍然可以进行修剪。",
          "product": "Pulse · Labor",
          "icon": "operators"
        },
        {
          "title": "OT 暴露在它到来之前",
          "description": "Sundae会按地点预测本周剩余时间的加班时间，因此您可以重新分配时间，而不是吸收成本。",
          "product": "Insights · Labor",
          "icon": "balance"
        },
        {
          "title": "每位服务员每 FOH 小时的生产力",
          "description": "客流/小时、销售额/小时、追加销售率 -  - 按服务员、按班次。用数字而非印象来辅导。",
          "product": "Pulse · Server view",
          "icon": "performance"
        },
        {
          "title": "根据需求安排推荐",
          "description": "Foresight 项目涵盖 14 天--Sundae 建议匹配的劳工计划。",
          "product": "Foresight",
          "icon": "forecasting"
        }
      ],
      "outcomesTitle": "HR 有何变化",
      "outcomesDescription": "日程安排更紧凑，惊喜更少，您可以展示生产力。",
      "outcomes": [
        {
          "title": "在其落地之前减少 OT 暴露",
          "description": "在周中重新分配工作时间，而不是在工资发放之后。",
          "icon": "speed"
        },
        {
          "title": "劳动力百分比每周更贴近计划",
          "description": "差异从每月的混乱收窄到每周的精确。",
          "icon": "performance"
        },
        {
          "title": "可供横向对比的服务员生产力",
          "description": "实时、按班次的生产力，支撑辅导、绩效评估和晋升决策。",
          "icon": "owners"
        },
        {
          "title": "与楼层相匹配的时间表",
          "description": "需求驱动的时间表建立在 Foresight 预测的基础上，而不是上周的模式。",
          "icon": "support"
        }
      ],
      "crewCallout": {
        "eyebrow": "由 SUNDAE CREW 提供支持",
        "title": "支撑这套智能的运营底座。",
        "description": "Crew 负责调度、考勤、多地区薪资（美国、加拿大、英国、欧盟、海湾合作委员会 - 一个引擎）和 60 多个 HR 界面 - 每个班次、每个小时和薪资周期都成为为该页面上的劳动力情报提供动力的信号。可选：带上您自己的 HR（Bayzat、Personio、Gusto），Sundae仍然读取相同的信号。",
        "ctaText": "Explore Sundae Crew"
      },
      "ctaTitle": "查看您的劳动动态。",
      "ctaDescription": "30分钟。你的日程安排。在它落地之前你会抓住的 OT。",
      "ctaButton": "预约劳工演练"
    },
    "tr": {
      "badge": "İK ve İnsan Ekipleri için",
      "titleLine1": "Vardiya için program yapın.",
      "titleLine2": "Geçen hafta için değil.",
      "description": "Konuma göre canlı emek farkı. Fazla mesai riski gerçekleşmeden önce işaretlenir. Verimlilik, ödeme dönemi başına değil, FOH saati başına ölçülür.",
      "primaryCta": "İşçilik İncelemesi Rezervasyonu Yapın",
      "secondaryCta": "İşçi Demosunu Görün",
      "problemsEyebrow": "İŞGÜCÜ NEREDE KAÇIYOR",
      "problemsTitle": "İşgücü bütçesinin sızdığı yer",
      "problemsDescription": "İçgüdülere dayalı programlar, maaş bordrosu sonrasında keşfedilen farklılıklar.",
      "challenges": [
        {
          "title": "Fazla mesai maaş bordrosuna kadar görünmez",
          "description": "Fazla mesai çizgisini gördüğünüzde zaten ödemiş olursunuz.",
          "icon": "balance"
        },
        {
          "title": "Geçen haftanın misafir sayısına göre oluşturulan programlar",
          "description": "Talep değişti. Program değişmedi. İşgücü yüzdesi de onunla birlikte kayıyor.",
          "icon": "operators"
        },
        {
          "title": "Üretkenlik rapor edildi, işlem yapılabilir değil",
          "description": "Aylık FOH saati başına misafir sayısını biliyorsunuz. Vardiyaya göre değil, garsona göre değil, koçluk yapmaya yetişecek zamanda değil.",
          "icon": "performance"
        },
        {
          "title": "Devamsızlık konusunda erken uyarı yok",
          "description": "Kötü vardiya, saat 16:15'te bir gelmeme (no-show) yaşandığında başlar. Bunu yarınki özette duyarsınız.",
          "icon": "support"
        }
      ],
      "howTitle": "Sundae İK ve İnsanlar için nasıl çalışıyor?",
      "howDescription": "Canlı işgücü sinyali, fazla mesaiyi önleme, vardiyada üretkenlik ayrıntı düzeyi.",
      "howSundaeHelps": [
        {
          "title": "Konuma göre canlı emek yüzdesi",
          "description": "Talebe göre saat başı işçilik maliyeti. Bütçeyi aşan site, siz hâlâ kırpma fırsatınız varken işaretlenir.",
          "product": "Pulse · Labor",
          "icon": "operators"
        },
        {
          "title": "Vuruştan önce fazla uzatmaya maruz kalma",
          "description": "Sundae, haftanın geri kalanı için fazla mesai hattını konuma göre tahmin eder; böylece saatleri yeniden dağıtırsınız, maliyeti karşılamazsınız.",
          "product": "Insights · Labor",
          "icon": "balance"
        },
        {
          "title": "Garson başına FOH saati başına üretkenlik",
          "description": "Misafir/saat, satış/saat, ek satış oranı - garson başına, vardiya başına. İzlenimlerle değil, rakamlarla koçluk yapın.",
          "product": "Pulse · Server view",
          "icon": "performance"
        },
        {
          "title": "Talepten gelen vardiya önerileri",
          "description": "Foresight misafir sayısını 14 gün öncesinden tahmin eder - Sundae buna uyan işgücü planını önerir.",
          "product": "Foresight",
          "icon": "forecasting"
        }
      ],
      "outcomesTitle": "İK için neler değişiyor?",
      "outcomesDescription": "Daha sıkı programlar, daha az sürpriz, gösterebileceğiniz üretkenlik.",
      "outcomes": [
        {
          "title": "İnmeden önce OT maruziyetini kesin",
          "description": "Çalışma saatlerini maaş bordrosu bittikten sonra değil, hafta ortasında yeniden tahsis edin.",
          "icon": "speed"
        },
        {
          "title": "İşgücü yüzdesi her hafta plana yaklaşıyor",
          "description": "Varyans, aylık kaostan haftalık kesinliğe doğru daralmaktadır.",
          "icon": "performance"
        },
        {
          "title": "Karşılaştırabileceğiniz garson üretkenliği",
          "description": "Koçluğu, değerlendirmeleri ve terfi kararlarını destekleyen canlı, vardiya başına üretkenlik.",
          "icon": "owners"
        },
        {
          "title": "Salona uygun programlar",
          "description": "Foresight tahminlerine dayalı, talep odaklı programlar - geçen haftanın düzeni değil.",
          "icon": "support"
        }
      ],
      "crewCallout": {
        "eyebrow": "SUNDAE EKİP TARAFINDAN DESTEKLENİYOR",
        "title": "Bu zekayı besleyen operasyonel alt tabaka.",
        "description": "Crew; vardiya planlamasını, zaman ve devam takibini, çok bölgeli bordroyu (ABD, Kanada, İngiltere, AB, GCC - tek motor) ve 60'tan fazla İK yüzeyini çalıştırır; her vardiya, saat ve bordro döngüsü bu sayfadaki işgücü zekasını besleyen bir sinyale dönüşür. İsteğe bağlı: kendi HR'nizi (Bayzat, Personio, Gusto) getirin, Sundae yine aynı sinyali okur.",
        "ctaText": "Sundae Crew'u keşfedin"
      },
      "ctaTitle": "Emeğinizi hareket halinde görün.",
      "ctaDescription": "30 dakika. Sizin programınız. İnmeden önce yakalayacağınız OT.",
      "ctaButton": "İşçilik İncelemesi Rezervasyonu Yapın"
    },
    "ja": {
      "badge": "人事および人事チーム向け",
      "titleLine1": "シフトのスケジュール。",
      "titleLine2": "先週のことではありません。",
      "description": "場所ごとの実際の労働力の差異。残業のリスクが発生する前にフラグが立てられます。生産性は給与期間ごとではなく、FOH 時間ごとに測定されます。",
      "primaryCta": "労務ウォークスルーを予約する",
      "secondaryCta": "労務デモを見る",
      "problemsEyebrow": "労働力が漏れる場所",
      "problemsTitle": "労働予算が漏れる場所",
      "problemsDescription": "スケジュールは本能に基づいて作成され、差異は給与計算後に判明します。",
      "challenges": [
        {
          "title": "OT エクスポージャーは給与計算まで目に見えない",
          "description": "残業代のラインが見えたときには、すでに支払いが終わっています。",
          "icon": "balance"
        },
        {
          "title": "先週の客数に基づいて作成されたスケジュール",
          "description": "需要は変化しました。スケジュールは変わっていません。労働率もそれに伴ってずれていきます。",
          "icon": "operators"
        },
        {
          "title": "生産性は報告されましたが、実用的ではありません",
          "description": "FOH 時間あたりの客数を月単位で把握できます。シフトごとでも、接客スタッフごとでもなく、コーチングに間に合いません。",
          "icon": "performance"
        },
        {
          "title": "欠勤の早期通知なし",
          "description": "悪いシフトは、午後 4 時 15 分にノーショーが発生したときに始まります。それについては明日の要約で聞きます。",
          "icon": "support"
        }
      ],
      "howTitle": "人事と人事におけるSundaeの仕組み",
      "howDescription": "リアルタイムの労働シグナル、OT 予防、シフト粒度での生産性。",
      "howSundaeHelps": [
        {
          "title": "場所別の生きた労働力の割合（%）",
          "description": "需要に対する時間ごとの人件費。予算を超過しているサイトには、トリミングできる間にフラグが立てられます。",
          "product": "Pulse · Labor",
          "icon": "operators"
        },
        {
          "title": "影響を受ける前のOTエクスポージャー",
          "description": "Sundae は、週の残りの地域ごとに OT ラインを予測します。そのため、コストを吸収するのではなく、時間を再配分することになります。",
          "product": "Insights · Labor",
          "icon": "balance"
        },
        {
          "title": "接客スタッフごと・FOH 時間あたりの生産性",
          "description": "客数/時間、売上/時間、アップセル率 - 接客スタッフごと、シフトごと。印象ではなく数字でコーチングする。",
          "product": "Pulse · Server view",
          "icon": "performance"
        },
        {
          "title": "需要に応じたシフトの推奨",
          "description": "Foresight は 14 日先までの客数を予測します。Sundae はそれに合った人員計画を提案します。",
          "product": "Foresight",
          "icon": "forecasting"
        }
      ],
      "outcomesTitle": "人事にとって何が変わるのか",
      "outcomesDescription": "スケジュールが厳しくなり、予期せぬ事態が減り、生産性を発揮できるようになります。",
      "outcomes": [
        {
          "title": "着地する前にOTの露出をカットする",
          "description": "給与計算後ではなく、週の半ばに時間を再割り当てします。",
          "icon": "speed"
        },
        {
          "title": "毎週、計画に近づく労働力の割合",
          "description": "分散は月次のカオスから週次の精度まで狭まります。",
          "icon": "performance"
        },
        {
          "title": "比較できる接客スタッフの生産性",
          "description": "コーチング、評価、昇進の判断を支える、シフトごとのライブ生産性。",
          "icon": "owners"
        },
        {
          "title": "フロアに合わせたスケジュール",
          "description": "先週のパターンではなく、Foresight の予測に基づいて構築された需要主導のスケジュール。",
          "icon": "support"
        }
      ],
      "crewCallout": {
        "eyebrow": "SUNDAE CREW によるサポート",
        "title": "このインテリジェンスを支える運用基盤。",
        "description": "Crew は、シフト管理、勤怠管理、複数地域の給与計算 (米国、カナダ、英国、EU、GCC - 1 つのエンジン)、および 60 以上の HR サーフェスを実行します。すべてのシフト、時間、給与サイクルが、このページの労務インテリジェンスを支えるシグナルとなります。オプション: 独自の HR (Bayzat、Personio、Gusto) を持参しても、Sundae は同じシグナルを読み取ります。",
        "ctaText": "Explore Sundae Crew"
      },
      "ctaTitle": "あなたの労務をリアルタイムで見てください。",
      "ctaDescription": "30分。あなたのスケジュール。着地する前に捕まえられたはずのOT。",
      "ctaButton": "労務ウォークスルーを予約する"
    },
    "sv": {
      "badge": "För HR & People Teams",
      "titleLine1": "Schema för skiftet.",
      "titleLine2": "Inte för förra veckan.",
      "description": "Skillnad i levande arbete efter plats. Övertidsrisk flaggad innan den inträffar. Produktivitet mätt per FOH-timme, inte per löneperiod.",
      "primaryCta": "Boka en Labor Walk-through",
      "secondaryCta": "Se Labor Demo",
      "problemsEyebrow": "DÄR ARBETSKRAFTEN LÄCKER",
      "problemsTitle": "Där personalbudgeten läcker",
      "problemsDescription": "Scheman byggda på instinkt, varians upptäckt efter löneutbetalning.",
      "challenges": [
        {
          "title": "OT exponering osynlig fram till lönelistan",
          "description": "När du ser övertidsgränsen har du redan betalat den.",
          "icon": "balance"
        },
        {
          "title": "Scheman byggda på förra veckans gäster",
          "description": "Efterfrågan har flyttat sig. Schemat har inte. Arbetskraftsprocenten driver med den.",
          "icon": "operators"
        },
        {
          "title": "Produktivitet rapporterad, inte åtgärdsbar",
          "description": "Du känner till gäster per FOH-timme per månad. Inte per skift, inte per servitör, inte i tid för att coacha.",
          "icon": "performance"
        },
        {
          "title": "Ingen tidig varning om frånvaro",
          "description": "Det dåliga skiftet börjar när en utebliven ankomst landar kl. 16:15. Du hör om det i morgondagens sammanfattning.",
          "icon": "support"
        }
      ],
      "howTitle": "Hur Sundae fungerar för HR & People",
      "howDescription": "Live arbetskraftssignal, OT-förebyggande, produktivitet vid skift granularitet.",
      "howSundaeHelps": [
        {
          "title": "Levande arbetskraft % per plats",
          "description": "Arbetskostnad timme för timme mot efterfrågan. Webbplatsen som överskrider budgeten flaggas medan du fortfarande kan trimma.",
          "product": "Pulse · Labor",
          "icon": "operators"
        },
        {
          "title": "OT-exponering innan den slår",
          "description": "Sundae förutser OT-linjen efter plats för resten av veckan - så att du omfördelar timmar, inte absorberar kostnaden.",
          "product": "Insights · Labor",
          "icon": "balance"
        },
        {
          "title": "Produktivitet per FOH-timme, per servitör",
          "description": "Gäster/timme, försäljning/timme, merförsäljningsgrad - per servitör, per skift. Coacha med siffror, inte intryck.",
          "product": "Pulse · Server view",
          "icon": "performance"
        },
        {
          "title": "Schemarekommendationer från efterfrågan",
          "description": "Foresight prognostiserar gästantalet 14 dagar framåt - Sundae föreslår den personalplan som matchar.",
          "product": "Foresight",
          "icon": "forecasting"
        }
      ],
      "outcomesTitle": "Vad förändras för HR",
      "outcomesDescription": "Tätare scheman, färre överraskningar, produktivitet du kan visa.",
      "outcomes": [
        {
          "title": "Klipp OT-exponeringen innan den landar",
          "description": "Omfördela timmar mitt i veckan, inte efter lönekörningen.",
          "icon": "speed"
        },
        {
          "title": "Arbetskraft % närmare plan, varje vecka",
          "description": "Variansen minskar från månatligt kaos till veckovis precision.",
          "icon": "performance"
        },
        {
          "title": "Servitörsproduktivitet du kan jämföra",
          "description": "Live produktivitet per skift som stödjer coaching, utvärderingar och befordringsbeslut.",
          "icon": "owners"
        },
        {
          "title": "Scheman som matchar verksamheten",
          "description": "Efterfrågestyrda scheman byggda på Foresight-prognoser - inte förra veckans mönster.",
          "icon": "support"
        }
      ],
      "crewCallout": {
        "eyebrow": "DRIVS AV SUNDAE CREW",
        "title": "Det operativa substratet som matar denna intelligens.",
        "description": "Crew sköter schemaläggning, tid och närvaro, lönehantering i flera regioner (USA, Kanada, Storbritannien, EU, GCC - en motor) och 60+ HR-ytor - varje skift, timme och lönecykel blir en signal som driver arbetskraftsintelligensen på den här sidan. Valfritt: ta med din egen HR (Bayzat, Personio, Gusto) och Sundae läser fortfarande samma signal.",
        "ctaText": "Utforska Sundae Crew"
      },
      "ctaTitle": "Se ditt arbete i rörelse.",
      "ctaDescription": "30 minuter. Ditt schema. OT du skulle ha fångat innan den landade.",
      "ctaButton": "Boka en Labor Walk-through"
    },
    "id": {
      "badge": "Untuk Tim SDM & Manusia",
      "titleLine1": "Jadwal shiftnya.",
      "titleLine2": "Tidak untuk minggu lalu.",
      "description": "Varians tenaga kerja nyata berdasarkan lokasi. Risiko lembur ditandai sebelum terjadi. Produktivitas diukur per jam FOH, bukan per periode pembayaran.",
      "primaryCta": "Pesan Panduan Ketenagakerjaan",
      "secondaryCta": "Lihat Demo Buruh",
      "problemsEyebrow": "DIMANA KEBOCORAN TENAGA KERJA",
      "problemsTitle": "Dimana anggaran tenaga kerja bocor",
      "problemsDescription": "Jadwal dibangun berdasarkan insting, varians ditemukan setelah pembayaran gaji.",
      "challenges": [
        {
          "title": "Eksposur OT tidak terlihat sampai gajian",
          "description": "Pada saat Anda melihat garis lembur, Anda sudah membayarnya.",
          "icon": "balance"
        },
        {
          "title": "Jadwal dibuat berdasarkan jumlah tamu minggu lalu",
          "description": "Permintaan telah bergeser. Jadwalnya belum. % tenaga kerja ikut bergeser bersamanya.",
          "icon": "operators"
        },
        {
          "title": "Produktivitas dilaporkan, tidak dapat ditindaklanjuti",
          "description": "Anda tahu jumlah tamu per jam FOH per bulan. Bukan per shift, bukan per pelayan, bukan tepat waktu untuk membina.",
          "icon": "performance"
        },
        {
          "title": "Tidak ada peringatan dini tentang ketidakhadiran",
          "description": "Pergeseran buruk dimulai ketika ketidakhadiran terjadi pada pukul 16:15. Anda mendengarnya di rekap besok.",
          "icon": "support"
        }
      ],
      "howTitle": "Bagaimana Sundae bekerja untuk HR & People",
      "howDescription": "Sinyal tenaga kerja langsung, pencegahan lembur, produktivitas pada perincian shift.",
      "howSundaeHelps": [
        {
          "title": "% tenaga kerja hidup berdasarkan lokasi",
          "description": "Biaya tenaga kerja jam demi jam dibandingkan permintaan. Situs yang melebihi anggaran akan ditandai selagi Anda masih dapat memangkasnya.",
          "product": "Pulse · Labor",
          "icon": "operators"
        },
        {
          "title": "Eksposur OT sebelum terjadi",
          "description": "Sundae memperkirakan garis OT berdasarkan lokasi untuk sisa minggu ini - jadi Anda mendistribusikan ulang jam kerja, bukan menanggung biayanya.",
          "product": "Insights · Labor",
          "icon": "balance"
        },
        {
          "title": "Produktivitas per jam FOH, per pelayan",
          "description": "Tamu/jam, penjualan/jam, tingkat upsell - per pelayan, per shift. Bina dengan angka, bukan kesan.",
          "product": "Pulse · Server view",
          "icon": "performance"
        },
        {
          "title": "Rekomendasi jadwal dari permintaan",
          "description": "Foresight memproyeksikan jumlah tamu 14 hari ke depan - Sundae menyarankan rencana tenaga kerja yang sesuai.",
          "product": "Foresight",
          "icon": "forecasting"
        }
      ],
      "outcomesTitle": "Perubahan apa yang terjadi pada HR",
      "outcomesDescription": "Jadwal yang lebih ketat, lebih sedikit kejutan, produktivitas dapat Anda tunjukkan.",
      "outcomes": [
        {
          "title": "Pangkas eksposur OT sebelum terjadi",
          "description": "Alokasikan kembali jam kerja pada pertengahan minggu, bukan setelah proses penggajian.",
          "icon": "speed"
        },
        {
          "title": "% tenaga kerja mendekati rencana, setiap minggu",
          "description": "Varians menyempit dari kekacauan bulanan menjadi presisi mingguan.",
          "icon": "performance"
        },
        {
          "title": "Produktivitas pelayan yang dapat Anda bandingkan",
          "description": "Produktivitas langsung per shift yang mendukung pembinaan, evaluasi, dan keputusan promosi.",
          "icon": "owners"
        },
        {
          "title": "Jadwal yang sesuai dengan lantai",
          "description": "Jadwal berdasarkan permintaan dibangun berdasarkan proyeksi Foresight - bukan pola minggu lalu.",
          "icon": "support"
        }
      ],
      "crewCallout": {
        "eyebrow": "DIDUKUNG OLEH CREW SUNDAE",
        "title": "Substrat operasional yang mendukung kecerdasan ini.",
        "description": "Crew menjalankan penjadwalan, waktu & kehadiran, penggajian multi-wilayah (AS, Kanada, Inggris, UE, GCC - satu mesin), dan 60+ permukaan SDM - setiap shift, jam, dan siklus pembayaran menjadi sinyal yang mendukung intelijen ketenagakerjaan di halaman ini. Opsional: bawa HR sendiri (Bayzat, Personio, Gusto) dan Sundae tetap membaca sinyal yang sama.",
        "ctaText": "Explore Sundae Crew"
      },
      "ctaTitle": "Lihat tenaga kerja Anda bergerak.",
      "ctaDescription": "30 menit. Jadwal Anda. OT yang pasti Anda tangkap sebelum terjadi.",
      "ctaButton": "Pesan Panduan Ketenagakerjaan"
    },
    "ko": {
      "badge": "HR 및 인력 팀용",
      "titleLine1": "교대 일정.",
      "titleLine2": "지난주는 아닙니다.",
      "description": "위치별 실제 노동력 차이. 초과 근무 위험이 발생하기 전에 표시됩니다. 생산성은 급여 기간이 아닌 FOH 시간 기준으로 측정됩니다.",
      "primaryCta": "노동 견학 예약",
      "secondaryCta": "노동 데모 보기",
      "problemsEyebrow": "노동력이 누출되는 곳",
      "problemsTitle": "인건비가 새는 곳",
      "problemsDescription": "본능에 따라 만들어진 일정, 급여 이후에 발견된 차이.",
      "challenges": [
        {
          "title": "급여가 지급될 때까지 OT 노출이 보이지 않음",
          "description": "초과 근무 시간이 표시될 때쯤에는 이미 비용을 지불한 것입니다.",
          "icon": "balance"
        },
        {
          "title": "지난주 손님 수를 바탕으로 만들어진 일정",
          "description": "수요는 이동했습니다. 일정은 그대로입니다. 노동 %도 그에 따라 흔들립니다.",
          "icon": "operators"
        },
        {
          "title": "생산성이 보고되었으나 실행 불가",
          "description": "FOH 시간당 손님 수를 월별로만 알 수 있습니다. 교대별도, 서버(접객 직원)별도 아니고, 코칭할 시간에 맞춰서도 아닙니다.",
          "icon": "performance"
        },
        {
          "title": "결근에 대한 조기 경고 없음",
          "description": "나쁜 교대는 오후 4시 15분에 노쇼가 시작되면 시작됩니다. 내일 요약에서 이에 대해 듣게 됩니다.",
          "icon": "support"
        }
      ],
      "howTitle": "Sundae가 HR 및 인력 부문에서 작동하는 방식",
      "howDescription": "실시간 노동 신호, OT 예방, 교대 단위의 생산성.",
      "howSundaeHelps": [
        {
          "title": "위치별 실시간 노동 %",
          "description": "수요 대비 시간당 인건비. 예산을 초과하는 사이트는 아직 다듬을 수 있는 동안 플래그가 지정됩니다.",
          "product": "Pulse · Labor",
          "icon": "operators"
        },
        {
          "title": "OT가 발생하기 전에 노출",
          "description": "Sundae는 남은 주 동안 위치별로 OT 라인을 예측하므로 비용을 흡수하지 않고 시간을 재분배할 수 있습니다.",
          "product": "Insights · Labor",
          "icon": "balance"
        },
        {
          "title": "FOH 시간당, 서버(접객 직원)당 생산성",
          "description": "손님 수/시간, 매출/시간, 상향 판매율 - 서버당, 교대당. 인상이 아닌 숫자로 코칭하세요.",
          "product": "Pulse · Server view",
          "icon": "performance"
        },
        {
          "title": "수요 기반 일정 추천",
          "description": "Foresight는 14일 앞선 손님 수를 예측합니다 - Sundae는 이에 맞는 인력 계획을 제안합니다.",
          "product": "Foresight",
          "icon": "forecasting"
        }
      ],
      "outcomesTitle": "HR의 변화",
      "outcomesDescription": "더 빡빡한 일정, 더 적은 놀라움, 생산성을 보여줄 수 있습니다.",
      "outcomes": [
        {
          "title": "OT 노출이 발생하기 전에 차단하세요.",
          "description": "급여 지급 이후가 아니라 주중에 시간을 재할당합니다.",
          "icon": "speed"
        },
        {
          "title": "매주 계획에 근접한 노동 %",
          "description": "분산은 월간 혼란에서 주간 정밀도로 좁아집니다.",
          "icon": "performance"
        },
        {
          "title": "비교할 수 있는 서버(접객 직원) 생산성",
          "description": "코칭, 평가, 승진 결정을 뒷받침하는 실시간 교대별 생산성입니다.",
          "icon": "owners"
        },
        {
          "title": "매장 상황에 맞는 일정",
          "description": "지난주 패턴이 아닌 Foresight 예측을 바탕으로 구축된 수요 중심 일정입니다.",
          "icon": "support"
        }
      ],
      "crewCallout": {
        "eyebrow": "SUNDAE CREW의 지원",
        "title": "이 지능을 제공하는 작동 기반입니다.",
        "description": "Crew는 일정 관리, 근태 관리, 다중 지역 급여(미국, 캐나다, 영국, EU, GCC - 단일 엔진) 및 60개 이상의 HR 표면을 실행합니다. 모든 교대, 시간 및 급여 주기는 이 페이지의 노동 인텔리전스를 강화하는 신호가 됩니다. 선택 사항: 자신의 HR(Bayzat, Personio, Gusto)을 가져와도 Sundae는 여전히 동일한 신호를 읽습니다.",
        "ctaText": "Explore Sundae Crew"
      },
      "ctaTitle": "당신의 노동이 어떻게 움직이는지 보십시오.",
      "ctaDescription": "30분. 귀하의 일정. 착륙하기 전에 잡았을 OT.",
      "ctaButton": "노동 견학 예약"
    },
    "ro": {
      "badge": "Pentru echipele de HR și oameni",
      "titleLine1": "Program pentru tură.",
      "titleLine2": "Nu pentru săptămâna trecută.",
      "description": "Variația forței de muncă în viață în funcție de locație. Riscul de ore suplimentare a fost semnalat înainte de a se întâmpla. Productivitatea măsurată pe oră FOH, nu pe perioadă de plată.",
      "primaryCta": "Rezervați o prezentare a forței de muncă",
      "secondaryCta": "Vezi Demo Muncii",
      "problemsEyebrow": "UNDE SE SCURGE FORȚA DE MUNCĂ",
      "problemsTitle": "Unde se scurge bugetul de personal",
      "problemsDescription": "Programe construite pe instinct, varianță descoperită după salarizare.",
      "challenges": [
        {
          "title": "Expunerea OT invizibilă până la salarizare",
          "description": "Până când vezi linia orelor suplimentare, ai plătit-o deja.",
          "icon": "balance"
        },
        {
          "title": "Programe construite pe clienții de săptămâna trecută",
          "description": "Cererea s-a mutat. Programul nu. % forței de muncă derivă odată cu ea.",
          "icon": "operators"
        },
        {
          "title": "Productivitatea raportată, nu poate fi acționată",
          "description": "Știți clienții pe oră FOH pe lună. Nu pe tură, nu pe ospătar, nu la timp pentru a face coaching.",
          "icon": "performance"
        },
        {
          "title": "Nicio avertizare timpurie cu privire la absenteism",
          "description": "Schimbarea proastă începe când o neprezentare aterizează la 16:15. Auzi despre asta în recapitularea de mâine.",
          "icon": "support"
        }
      ],
      "howTitle": "Cum funcționează Sundae pentru HR și oameni",
      "howDescription": "Semnal live de muncă, prevenire OT, productivitate la granularitatea schimburilor.",
      "howSundaeHelps": [
        {
          "title": "Forță de muncă vie % după locație",
          "description": "Costul forței de muncă oră cu oră față de cerere. Site-ul care depășește bugetul este semnalat în timp ce încă puteți tăia.",
          "product": "Pulse · Labor",
          "icon": "operators"
        },
        {
          "title": "Expunerea OT înainte de a lovi",
          "description": "Sundae prognozează linia OT în funcție de locație pentru restul săptămânii - astfel încât să redistribuiți orele, nu să absorbiți costurile.",
          "product": "Insights · Labor",
          "icon": "balance"
        },
        {
          "title": "Productivitate pe oră FOH, pe ospătar",
          "description": "Clienți/oră, vânzări/oră, rată de upsell - pe ospătar, pe tură. Faceți coaching cu cifre, nu cu impresii.",
          "product": "Pulse · Server view",
          "icon": "performance"
        },
        {
          "title": "Recomandări de program din cerere",
          "description": "Foresight proiectează numărul de clienți cu 14 zile înainte - Sundae sugerează planul de personal care se potrivește.",
          "product": "Foresight",
          "icon": "forecasting"
        }
      ],
      "outcomesTitle": "Ce se schimbă pentru HR",
      "outcomesDescription": "Programe mai strânse, mai puține surprize, productivitate pe care o poți demonstra.",
      "outcomes": [
        {
          "title": "Reduceți expunerea OT înainte de a ateriza",
          "description": "Realocați orele la mijlocul săptămânii, nu după rularea salariilor.",
          "icon": "speed"
        },
        {
          "title": "Muncă % mai aproape de plan, în fiecare săptămână",
          "description": "Varianta se restrânge de la haos lunar la precizie săptămânală.",
          "icon": "performance"
        },
        {
          "title": "Productivitatea ospătarului pe care o puteți compara",
          "description": "Productivitate live, pe tură, care susține coaching-ul, evaluările și deciziile de promovare.",
          "icon": "owners"
        },
        {
          "title": "Programe care se potrivesc cu sala",
          "description": "Programe ghidate de cerere, construite pe proiecțiile Foresight - nu pe tiparul de săptămâna trecută.",
          "icon": "support"
        }
      ],
      "crewCallout": {
        "eyebrow": "SUSȚINUT DE SUNDAE CREW",
        "title": "Substratul operațional care alimentează această inteligență.",
        "description": "Crew rulează planificarea, pontajul și prezența, salarizarea în mai multe regiuni (SUA, Canada, Marea Britanie, UE, GCC - un singur motor) și peste 60 de suprafețe HR - fiecare tură, oră și ciclu de plată devine un semnal care alimentează inteligența forței de muncă de pe această pagină. Opțional: aduceți-vă propriul HR (Bayzat, Personio, Gusto) și Sundae citește în continuare același semnal.",
        "ctaText": "Explorați Sundae Crew"
      },
      "ctaTitle": "Vedeți-vă munca în mișcare.",
      "ctaDescription": "30 de minute. Programul tău. OT pe care l-ai fi prins înainte de a ateriza.",
      "ctaButton": "Rezervați o prezentare a forței de muncă"
    },
    "bn": {
      "badge": "এইচআর ও পিপল টিমের জন্য",
      "titleLine1": "শিফটের জন্য সময়সূচী।",
      "titleLine2": "গত সপ্তাহের জন্য নয়।",
      "description": "অবস্থান অনুসারে লাইভ শ্রম বৈচিত্র। ওভারটাইম ঝুঁকি এটি ঘটার আগে পতাকাঙ্কিত. প্রতি FOH ঘন্টায় উৎপাদনশীলতা পরিমাপ করা হয়, প্রতি বেতনের সময়কাল নয়।",
      "primaryCta": "একটি লেবার ওয়াক-থ্রু বুক করুন",
      "secondaryCta": "লেবার ডেমো দেখুন",
      "problemsEyebrow": "যেখানে শ্রম লিক",
      "problemsTitle": "যেখানে শ্রম বাজেট ফাঁস হয়",
      "problemsDescription": "প্রবৃত্তির উপর নির্মিত সময়সূচী, বেতনের পরে আবিষ্কৃত ভিন্নতা।",
      "challenges": [
        {
          "title": "বেতন পর্যন্ত OT এক্সপোজার অদৃশ্য",
          "description": "যখন আপনি ওভারটাইম লাইন দেখতে পান, আপনি ইতিমধ্যেই এটি পরিশোধ করেছেন।",
          "icon": "balance"
        },
        {
          "title": "গত সপ্তাহের অতিথি সংখ্যার ভিত্তিতে তৈরি সময়সূচী",
          "description": "চাহিদা সরে গেছে। সময়সূচী সরেনি। শ্রম % তার সাথে প্রবাহিত হয়।",
          "icon": "operators"
        },
        {
          "title": "উৎপাদনশীলতা রিপোর্ট করা হয়েছে, কর্মযোগ্য নয়",
          "description": "আপনি প্রতি মাসে FOH ঘন্টা প্রতি অতিথি সংখ্যা জানেন। শিফট অনুযায়ী নয়, পরিবেশনকর্মী অনুযায়ী নয়, কোচিং করার মতো সময়মতো নয়।",
          "icon": "performance"
        },
        {
          "title": "অনুপস্থিতির বিষয়ে কোনো আগাম সতর্কতা নেই",
          "description": "খারাপ শিফট শুরু হয় যখন একটি নো-শো 4:15pm এ অবতরণ করে। আপনি আগামীকাল এর সংক্ষিপ্ত বিবরণ এটি সম্পর্কে শুনতে.",
          "icon": "support"
        }
      ],
      "howTitle": "কিভাবে Sundae HR এবং মানুষের জন্য কাজ করে",
      "howDescription": "লাইভ শ্রম সংকেত, OT প্রতিরোধ, শিফট গ্রানুলারিটিতে উৎপাদনশীলতা।",
      "howSundaeHelps": [
        {
          "title": "অবস্থান অনুসারে লাইভ লেবার %",
          "description": "চাহিদার বিপরীতে ঘন্টার পর ঘন্টা শ্রম খরচ। আপনি এখনও ছাঁটাই করতে পারেন এমন সময় বাজেটের উপর চলমান সাইটটি পতাকাঙ্কিত হয়৷",
          "product": "Pulse · Labor",
          "icon": "operators"
        },
        {
          "title": "এটি আঘাত করার আগে OT এক্সপোজার",
          "description": "Sundae সপ্তাহের বাকি অংশের জন্য অবস্থান অনুসারে OT লাইনের পূর্বাভাস দেয় - যাতে আপনি খরচ শোষণ না করে ঘন্টাগুলি পুনরায় বিতরণ করেন।",
          "product": "Insights · Labor",
          "icon": "balance"
        },
        {
          "title": "প্রতি FOH ঘন্টা, প্রতি পরিবেশনকর্মী উৎপাদনশীলতা",
          "description": "অতিথি/ঘন্টা, বিক্রয়/ঘণ্টা, আপসেল রেট - প্রতি পরিবেশনকর্মী, প্রতি শিফট। সংখ্যা দিয়ে কোচ করুন, ইমপ্রেশন দিয়ে নয়।",
          "product": "Pulse · Server view",
          "icon": "performance"
        },
        {
          "title": "চাহিদা থেকে সময়সূচীর সুপারিশ",
          "description": "Foresight 14 দিন আগে অতিথি সংখ্যার পূর্বাভাস দেয় - Sundae তার সাথে মানানসই শ্রম পরিকল্পনা সুপারিশ করে।",
          "product": "Foresight",
          "icon": "forecasting"
        }
      ],
      "outcomesTitle": "HR জন্য কি পরিবর্তন",
      "outcomesDescription": "কঠোর সময়সূচী, কম চমক, উত্পাদনশীলতা আপনি দেখাতে পারেন।",
      "outcomes": [
        {
          "title": "এটি অবতরণ আগে OT এক্সপোজার কাটা",
          "description": "সপ্তাহের মাঝামাঝি সময় পুনঃনির্ধারণ করুন, বেতনের পরে নয়।",
          "icon": "speed"
        },
        {
          "title": "শ্রম % পরিকল্পনার কাছাকাছি, প্রতি সপ্তাহে",
          "description": "বৈচিত্র মাসিক বিশৃঙ্খলা থেকে সাপ্তাহিক নির্ভুলতা পর্যন্ত সঙ্কুচিত হয়।",
          "icon": "performance"
        },
        {
          "title": "পরিবেশনকর্মীর উৎপাদনশীলতা যা আপনি তুলনা করতে পারেন",
          "description": "লাইভ, প্রতি-শিফট উৎপাদনশীলতা যা কোচিং, মূল্যায়ন এবং পদোন্নতির সিদ্ধান্তকে সমর্থন করে।",
          "icon": "owners"
        },
        {
          "title": "ফ্লোরের সাথে মানানসই সময়সূচী",
          "description": "চাহিদা-চালিত সময়সূচী যা Foresight পূর্বাভাসের উপর তৈরি - গত সপ্তাহের প্যাটার্নে নয়।",
          "icon": "support"
        }
      ],
      "crewCallout": {
        "eyebrow": "SUNDAE CREW দ্বারা চালিত",
        "title": "অপারেশনাল সাবস্ট্রেট যা এই বুদ্ধিমত্তাকে শক্তি জোগায়।",
        "description": "Crew সময়সূচী, সময় ও উপস্থিতি, বহু-অঞ্চল বেতন (US, Canada, UK, EU, GCC - একটি ইঞ্জিন), এবং 60+ HR সারফেস চালায় - প্রতিটি শিফট, ঘন্টা এবং বেতন-চক্র এই পৃষ্ঠায় শ্রম বুদ্ধিমত্তাকে শক্তি জোগানো সংকেতে পরিণত হয়। ঐচ্ছিক: আপনার নিজস্ব HR (Bayzat, Personio, Gusto) আনুন এবং Sundae এখনও একই সংকেত পড়ে।",
        "ctaText": "Explore Sundae Crew"
      },
      "ctaTitle": "আপনার শ্রমকে চলমান অবস্থায় দেখুন।",
      "ctaDescription": "30 মিনিট আপনার সময়সূচী. আপনি যে OTটি অবতরণ করার আগেই ধরতেন।",
      "ctaButton": "একটি লেবার ওয়াক-থ্রু বুক করুন"
    },
    "vi": {
      "badge": "Dành cho nhóm nhân sự và nhân sự",
      "titleLine1": "Lên lịch cho ca làm việc.",
      "titleLine2": "Không phải cho tuần trước.",
      "description": "Chênh lệch lao động trực tiếp theo địa điểm. Rủi ro ngoài giờ được gắn cờ trước khi nó xảy ra. Năng suất được đo theo giờ FOH, không phải theo kỳ lương.",
      "primaryCta": "Đặt buổi hướng dẫn về nhân sự",
      "secondaryCta": "Xem demo nhân sự",
      "problemsEyebrow": "Ở ĐÂU LAO ĐỘNG RÒ RỈ",
      "problemsTitle": "Trường hợp ngân sách lao động bị rò rỉ",
      "problemsDescription": "Lịch trình được xây dựng theo bản năng, sự khác biệt được phát hiện sau khi trả lương.",
      "challenges": [
        {
          "title": "Tiếp xúc OT vô hình cho đến khi trả lương",
          "description": "Vào thời điểm bạn nhìn thấy dòng làm thêm giờ, bạn đã thanh toán xong.",
          "icon": "balance"
        },
        {
          "title": "Lịch trình xây trên lượng khách của tuần trước",
          "description": "Nhu cầu đã dịch chuyển. Lịch trình thì không. % lao động trôi theo nó.",
          "icon": "operators"
        },
        {
          "title": "Báo cáo năng suất, không thể thực hiện được",
          "description": "Bạn biết lượng khách theo giờ FOH theo tháng. Không theo ca, không theo nhân viên phục vụ, không kịp để huấn luyện.",
          "icon": "performance"
        },
        {
          "title": "Không có cảnh báo sớm về việc vắng mặt",
          "description": "Sự thay đổi tồi tệ bắt đầu khi một người vắng mặt hạ cánh lúc 4:15 chiều. Bạn sẽ nghe về nó trong bản tóm tắt ngày mai.",
          "icon": "support"
        }
      ],
      "howTitle": "Sundae hoạt động như thế nào đối với Nhân sự & Con người",
      "howDescription": "Tín hiệu lao động trực tiếp, phòng ngừa OT, năng suất ở mức độ chi tiết của ca.",
      "howSundaeHelps": [
        {
          "title": "Lao động trực tiếp % theo địa điểm",
          "description": "Chi phí lao động theo giờ so với nhu cầu. Trang web vượt quá ngân sách sẽ bị gắn cờ trong khi bạn vẫn có thể cắt bớt.",
          "product": "Pulse · Labor",
          "icon": "operators"
        },
        {
          "title": "Tiếp xúc OT trước khi nó chạm tới",
          "description": "Sundae dự báo tuyến OT theo địa điểm cho những ngày còn lại trong tuần - do đó, bạn phân bổ lại số giờ chứ không phải tính chi phí.",
          "product": "Insights · Labor",
          "icon": "balance"
        },
        {
          "title": "Năng suất mỗi giờ FOH, trên mỗi nhân viên phục vụ",
          "description": "Khách/giờ, doanh số/giờ, tỷ lệ bán thêm - trên mỗi nhân viên phục vụ, mỗi ca. Huấn luyện bằng những con số, không phải cảm nhận.",
          "product": "Pulse · Server view",
          "icon": "performance"
        },
        {
          "title": "Đề xuất lịch trình từ nhu cầu",
          "description": "Foresight dự báo lượng khách trước 14 ngày - Sundae đề xuất kế hoạch nhân sự phù hợp.",
          "product": "Foresight",
          "icon": "forecasting"
        }
      ],
      "outcomesTitle": "Những thay đổi gì đối với nhân sự",
      "outcomesDescription": "Lịch trình chặt chẽ hơn, ít bất ngờ hơn, bạn có thể thể hiện năng suất.",
      "outcomes": [
        {
          "title": "Cắt mức độ hiển thị OT trước khi nó hạ cánh",
          "description": "Phân bổ lại số giờ vào giữa tuần, không phải sau khi tính lương.",
          "icon": "speed"
        },
        {
          "title": "Tỷ lệ lao động gần đạt kế hoạch, hàng tuần",
          "description": "Phương sai thu hẹp từ sự hỗn loạn hàng tháng đến độ chính xác hàng tuần.",
          "icon": "performance"
        },
        {
          "title": "Năng suất nhân viên phục vụ bạn có thể so sánh",
          "description": "Năng suất trực tiếp theo từng ca, hỗ trợ huấn luyện, đánh giá và quyết định thăng chức.",
          "icon": "owners"
        },
        {
          "title": "Lịch trình khớp với thực tế nhà hàng",
          "description": "Lịch trình điều phối theo nhu cầu, xây trên dự báo Foresight - không phải khuôn mẫu của tuần trước.",
          "icon": "support"
        }
      ],
      "crewCallout": {
        "eyebrow": "ĐƯỢC HỖ TRỢ BỞI SUNDAE CREW",
        "title": "Chất nền hoạt động nuôi dưỡng trí thông minh này.",
        "description": "Crew vận hành lập lịch, chấm công và điểm danh, bảng lương đa khu vực (Hoa Kỳ, Canada, Vương quốc Anh, EU, GCC - một engine) và hơn 60 bề mặt nhân sự - mỗi ca, giờ và chu kỳ trả lương trở thành tín hiệu tiếp sức cho trí tuệ lao động trên trang này. Tùy chọn: mang theo HR của riêng bạn (Bayzat, Personio, Gusto) và Sundae vẫn đọc cùng một tín hiệu.",
        "ctaText": "Khám phá Sundae Crew"
      },
      "ctaTitle": "Xem nhân sự của bạn vận hành theo thời gian thực.",
      "ctaDescription": "30 phút. Lịch trình của bạn. OT bạn có thể đã bắt được trước khi nó xảy ra.",
      "ctaButton": "Đặt buổi hướng dẫn về nhân sự"
    },
    "th": {
      "badge": "สำหรับทีมงานทรัพยากรบุคคลและบุคลากร",
      "titleLine1": "กำหนดการสำหรับกะ",
      "titleLine2": "ไม่ใช่สำหรับสัปดาห์ที่แล้ว",
      "description": "ผลต่างแรงงานสดตามสถานที่ ความเสี่ยงในการต่อเวลาถูกตั้งค่าสถานะก่อนที่จะเกิดขึ้น ผลผลิตวัดต่อชั่วโมง FOH ไม่ใช่ต่อระยะเวลาการจ่ายเงิน",
      "primaryCta": "จองการตรวจเยี่ยมแรงงาน",
      "secondaryCta": "ดูการสาธิตแรงงาน",
      "problemsEyebrow": "แรงงานรั่วไหลที่ไหน",
      "problemsTitle": "ในกรณีที่งบประมาณแรงงานรั่วไหล",
      "problemsDescription": "ตารางเวลาที่สร้างขึ้นจากสัญชาตญาณ ความแปรปรวนที่ค้นพบหลังการจ่ายเงินเดือน",
      "challenges": [
        {
          "title": "การสัมผัส OT มองไม่เห็นจนกระทั่งเงินเดือน",
          "description": "เมื่อคุณเห็นเส้นค่าล่วงเวลา แสดงว่าคุณชำระเงินแล้ว",
          "icon": "balance"
        },
        {
          "title": "กำหนดการที่สร้างจากจำนวนลูกค้าของสัปดาห์ที่แล้ว",
          "description": "ความต้องการเปลี่ยนไปแล้ว กำหนดการยังไม่เปลี่ยน % แรงงานก็เลื่อนตามไปด้วย",
          "icon": "operators"
        },
        {
          "title": "รายงานประสิทธิภาพการทำงานแล้ว ไม่สามารถดำเนินการได้",
          "description": "คุณรู้จำนวนลูกค้าต่อชั่วโมง FOH เป็นรายเดือน ไม่ใช่ตามกะ ไม่ใช่ตามพนักงานเสิร์ฟ ไม่ทันเวลาที่จะโค้ช",
          "icon": "performance"
        },
        {
          "title": "ไม่มีการเตือนล่วงหน้าเกี่ยวกับการขาดงาน",
          "description": "กะที่ไม่ดีจะเริ่มขึ้นเมื่อผู้ไม่มาปรากฏตัวเวลา 16:15 น. คุณจะได้ยินเรื่องนี้ในการสรุปของวันพรุ่งนี้",
          "icon": "support"
        }
      ],
      "howTitle": "Sundae ทำงานอย่างไรสำหรับ HR & People",
      "howDescription": "สัญญาณแรงงานแบบเรียลไทม์ การป้องกัน OT ประสิทธิภาพการทำงานตามรายละเอียดกะ",
      "howSundaeHelps": [
        {
          "title": "แรงงานสด % ตามสถานที่",
          "description": "ต้นทุนแรงงานรายชั่วโมงเทียบกับความต้องการ ไซต์ที่ใช้งบประมาณเกินจะถูกตั้งค่าสถานะในขณะที่คุณยังคงสามารถตัดแต่งได้",
          "product": "Pulse · Labor",
          "icon": "operators"
        },
        {
          "title": "การสัมผัส OT ก่อนที่จะโดน",
          "description": "Sundae คาดการณ์สาย OT ตามสถานที่ในช่วงที่เหลือของสัปดาห์ ดังนั้นคุณจึงสามารถกระจายชั่วโมงใหม่ได้ โดยไม่ดูดซับต้นทุน",
          "product": "Insights · Labor",
          "icon": "balance"
        },
        {
          "title": "ผลผลิตต่อชั่วโมง FOH ต่อพนักงานเสิร์ฟ",
          "description": "ลูกค้า/ชั่วโมง ยอดขาย/ชั่วโมง อัตราการขายต่อยอด - ต่อพนักงานเสิร์ฟ ต่อกะ โค้ชด้วยตัวเลข ไม่ใช่ความรู้สึก",
          "product": "Pulse · Server view",
          "icon": "performance"
        },
        {
          "title": "กำหนดเวลาคำแนะนำจากความต้องการ",
          "description": "โครงการ Foresight ครอบคลุม 14 วัน - Sundae แนะนำแผนแรงงานที่ตรงกัน",
          "product": "Foresight",
          "icon": "forecasting"
        }
      ],
      "outcomesTitle": "การเปลี่ยนแปลงอะไรบ้างสำหรับ HR",
      "outcomesDescription": "ตารางงานที่เข้มงวดมากขึ้น ความประหลาดใจน้อยลง ประสิทธิภาพการทำงานที่คุณสามารถแสดงได้",
      "outcomes": [
        {
          "title": "ตัดความเสี่ยง OT ก่อนที่จะลงจอด",
          "description": "จัดสรรชั่วโมงใหม่ในช่วงกลางสัปดาห์ ไม่ใช่หลังจากการรันบัญชีเงินเดือน",
          "icon": "speed"
        },
        {
          "title": "% แรงงานใกล้เคียงกับแผนทุกสัปดาห์",
          "description": "ความแปรปรวนแคบลงจากความโกลาหลรายเดือนไปจนถึงความแม่นยำรายสัปดาห์",
          "icon": "performance"
        },
        {
          "title": "ประสิทธิภาพการทำงานของพนักงานเสิร์ฟที่คุณสามารถเปรียบเทียบได้",
          "description": "ประสิทธิภาพการทำงานแบบสดต่อกะ ที่รองรับการโค้ช การประเมินผล และการตัดสินใจเลื่อนตำแหน่ง",
          "icon": "owners"
        },
        {
          "title": "ตารางที่ตรงกับหน้างานจริง",
          "description": "ตารางที่ขับเคลื่อนด้วยความต้องการ สร้างบนการคาดการณ์ของ Foresight - ไม่ใช่รูปแบบของสัปดาห์ที่แล้ว",
          "icon": "support"
        }
      ],
      "crewCallout": {
        "eyebrow": "ขับเคลื่อนโดย SUNDAE CREW",
        "title": "ฐานการปฏิบัติงานที่ป้อนข้อมูลให้กับความฉลาดนี้",
        "description": "Crew ดำเนินการจัดกำหนดการ เวลาและการเข้างาน บัญชีเงินเดือนหลายภูมิภาค (สหรัฐอเมริกา แคนาดา สหราชอาณาจักร สหภาพยุโรป GCC - เอ็นจินเดียว) และพื้นผิวด้านทรัพยากรบุคคลมากกว่า 60 แห่ง - ทุกกะ ชั่วโมง และรอบการจ่ายเงิน กลายเป็นสัญญาณที่ขับเคลื่อนความฉลาดด้านแรงงานในหน้านี้ ทางเลือก: นำ HR ของคุณมาเอง (Bayzat, Personio, Gusto) และ Sundae ยังคงอ่านสัญญาณเดียวกัน",
        "ctaText": "Explore Sundae Crew"
      },
      "ctaTitle": "ดูแรงงานของคุณเคลื่อนไหว",
      "ctaDescription": "30 นาที กำหนดการของคุณ OT ที่คุณจะจับได้ก่อนที่มันจะลงจอด",
      "ctaButton": "จองการตรวจเยี่ยมแรงงาน"
    },
    "ms": {
      "badge": "Untuk Pasukan HR & People",
      "titleLine1": "Jadual untuk syif.",
      "titleLine2": "Bukan untuk minggu lepas.",
      "description": "Varians buruh hidup mengikut lokasi. Risiko lebih masa dibenderakan sebelum ia berlaku. Produktiviti diukur setiap jam FOH, bukan setiap tempoh gaji.",
      "primaryCta": "Tempah Walk-through Buruh",
      "secondaryCta": "Lihat Demo Buruh",
      "problemsEyebrow": "DI MANA BURUH BOCOR",
      "problemsTitle": "Di mana bajet buruh bocor",
      "problemsDescription": "Jadual dibina berdasarkan naluri, varians ditemui selepas senarai gaji.",
      "challenges": [
        {
          "title": "Pendedahan OT tidak kelihatan sehingga gaji",
          "description": "Apabila anda melihat talian lebih masa, anda telah pun membayarnya.",
          "icon": "balance"
        },
        {
          "title": "Jadual yang dibina pada jumlah tetamu minggu lepas",
          "description": "Permintaan telah beralih. Jadual belum. % buruh hanyut dengannya.",
          "icon": "operators"
        },
        {
          "title": "Produktiviti dilaporkan, tidak boleh diambil tindakan",
          "description": "Anda tahu jumlah tetamu setiap jam FOH mengikut bulan. Bukan mengikut syif, bukan mengikut pelayan, tidak cukup masa untuk membimbing.",
          "icon": "performance"
        },
        {
          "title": "Tiada amaran awal tentang ketidakhadiran",
          "description": "Peralihan buruk bermula apabila ketidakhadiran mendarat pada 4:15 petang. Anda mendengar tentangnya dalam rekap esok.",
          "icon": "support"
        }
      ],
      "howTitle": "Cara Sundae berfungsi untuk HR & People",
      "howDescription": "Isyarat buruh hidup, pencegahan OT, produktiviti pada kebutiran syif.",
      "howSundaeHelps": [
        {
          "title": "Buruh hidup % mengikut lokasi",
          "description": "Kos buruh jam demi jam berbanding permintaan. Tapak yang melebihi belanjawan akan dibenderakan semasa anda masih boleh memangkas.",
          "product": "Pulse · Labor",
          "icon": "operators"
        },
        {
          "title": "Pendedahan OT sebelum terkena",
          "description": "Sundae meramalkan barisan OT mengikut lokasi untuk sepanjang minggu - jadi anda mengagihkan semula waktu kerja, bukan menyerap kos.",
          "product": "Insights · Labor",
          "icon": "balance"
        },
        {
          "title": "Produktiviti setiap jam FOH, setiap pelayan",
          "description": "Tetamu/jam, jualan/jam, kadar upsell - setiap pelayan, setiap syif. Bimbing dengan nombor, bukan tanggapan.",
          "product": "Pulse · Server view",
          "icon": "performance"
        },
        {
          "title": "Cadangan jadual daripada permintaan",
          "description": "Foresight meramalkan jumlah tetamu 14 hari lebih awal - Sundae mencadangkan pelan buruh yang sepadan.",
          "product": "Foresight",
          "icon": "forecasting"
        }
      ],
      "outcomesTitle": "Apakah perubahan untuk HR",
      "outcomesDescription": "Jadual yang lebih padat, lebih sedikit kejutan, produktiviti yang boleh anda tunjukkan.",
      "outcomes": [
        {
          "title": "Potong pendedahan OT sebelum ia mendarat",
          "description": "Agihkan semula waktu pada pertengahan minggu, bukan selepas pembayaran gaji.",
          "icon": "speed"
        },
        {
          "title": "Buruh % lebih dekat dengan rancangan, setiap minggu",
          "description": "Varians mengecil daripada huru-hara bulanan kepada ketepatan mingguan.",
          "icon": "performance"
        },
        {
          "title": "Produktiviti pelayan boleh anda bandingkan",
          "description": "Produktiviti setiap syif secara langsung yang menyokong panggilan bimbingan, ulasan dan promosi.",
          "icon": "owners"
        },
        {
          "title": "Jadual yang sepadan dengan lantai",
          "description": "Jadual dipacu permintaan dibina pada unjuran Foresight - bukan corak minggu lepas.",
          "icon": "support"
        }
      ],
      "crewCallout": {
        "eyebrow": "DISOKONG OLEH SUNDAE CREW",
        "title": "Substrat operasi yang membekalkan kecerdasan ini.",
        "description": "Crew menjalankan penjadualan, masa & kehadiran, senarai gaji berbilang wilayah (AS, Kanada, UK, EU, GCC - satu enjin) dan 60+ permukaan HR - setiap syif, jam dan kitaran gaji menjadi isyarat yang menguatkan kecerdasan buruh pada halaman ini. Pilihan: bawa HR anda sendiri (Bayzat, Personio, Gusto) dan Sundae masih membaca isyarat yang sama.",
        "ctaText": "Terokai Sundae Crew"
      },
      "ctaTitle": "Lihat kerja anda bergerak.",
      "ctaDescription": "30 minit. Jadual anda. OT yang anda akan tangkap sebelum ia mendarat.",
      "ctaButton": "Tempah Walk-through Buruh"
    }
  }
} as const
