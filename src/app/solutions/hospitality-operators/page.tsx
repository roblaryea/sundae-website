...
const localizedCopy: Record<"en" | "ar" | "fr" | "es", SolutionCopy> = {
  en: {
    ...
    description: "Hotel restaurants, banquet, IRD, lobby bar - every outlet's leak found, owned, and measured back to margin. Margin, mix, and labor in one frame.",
    ...
    outcomesDescription: "Outlet-level clarity, every leak owned, recovered margin measured back.",
    outcomes: [
      { title: "Recover F&B margin at outlet granularity", description: "Restaurant slipping? Lobby bar over plan? The fix gets an owner and the recovered margin is measured back by outlet.", icon: "speed" },
    ...
    ctaTitle: "Find the leak. Recover the F&B margin.",
    ctaDescription: "30 minutes. Your property. The outlet leak routed to an owner - and the recovered margin measured back against baseline.",
  ...
  ar: {
    ...
    outcomes: [
      { title: "استردّ هامش F&B بدقة المنفذ", description: "مطعم ينزلق؟ بار اللوبي متجاوز الخطة؟ يحصل الإصلاح على مسؤول ويُقاس الهامش المستردّ مجدداً لكل منفذ.", icon: "speed" },
  ...
  fr: {
    ...
    outcomes: [
      { title: "Récupérez la marge F&B à la granularité point", description: "Restaurant qui décroche ? Lobby au-dessus du plan ? La correction trouve un responsable et la marge récupérée est remesurée par point.", icon: "speed" },
  ...
  es: {
    ...
    outcomes: [
      { title: "Recupera el margen de A&B con granularidad de punto", description: "¿Restaurante deslizándose? ¿Lobby por encima del plan? La corrección recibe un responsable y el margen recuperado se vuelve a medir por punto.", icon: "speed" },
...
export default function HospitalityOperatorsPage() {
  ...
  return <SolutionPageLayout copy={copy} mockup={<ThemedShot framed priority width={1600} height={1000} dark="/images/product/2026-fresh/pulse-leaderboard-dark.png" light="/images/product/2026-fresh/pulse-leaderboard.png" alt="Pulse Portfolio Leaderboard - every F&B outlet ranked live by revenue versus target" />} gallery={<SectionProductGallery defaultPersona="c_suite" />} />;
}
