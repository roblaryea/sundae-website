...
const localizedCopy: Record<"en" | "ar" | "fr" | "es", SolutionCopy> = {
  en: {
    ...
    description: "Daily AI briefings across every brand. Find the margin leak the day it opens, route the fix to an accountable operator, and measure the recovery against baseline.",
    ...
    howSundaeHelps: [
      { title: "Portfolio command in real time", description: "Live revenue, margin, and labor across every outlet. The outlet leaking margin surfaces, the fix routed to its manager, and the recovery measured against baseline.", product: "Sundae Core + Pulse", icon: "chart" },
    ...
    outcomes: [
      ...
      { title: "Tighter portfolio, fewer surprises", description: "Underperforming locations surface in the morning brief, the fix routed to an owner and the recovery measured - not left for the quarterly review.", icon: "performance" },
    ...
    crewCallout: {
      ...
      title: "Workforce signal feeds the executive briefing.",
      description: "Scheduling, time, multi-region payroll readiness, and HR casework run through Crew - and every workforce event lands in your morning brief and quarterly review as signal, not as a separate workforce dashboard the C-suite has to remember to open. Optional: bring your own HR (Bayzat, Personio, Gusto) and the executive view still sees the same consolidated signal.",
      ctaText: "Explore Sundae Crew",
    },
    ctaTitle: "Recover margin across the whole portfolio.",
  ...
  ar: {
    ...
    description: "إحاطات يومية بالذكاء الاصطناعي عبر كل علامة تجارية. اعثر على تسرّب الهامش يوم حدوثه، ووجّه الإصلاح إلى مشغّل مسؤول، وقِس الهامش المستردّ مقابل خط الأساس.",
    ...
    howSundaeHelps: [
      { title: "قيادة المحفظة لحظياً", description: "إيرادات وهامش وعمالة حية عبر كل فرع. الفرع الذي يتسرّب هامشه يظهر مع توجيه الإصلاح إلى مديره، والهامش المستردّ يُقاس مقابل خط الأساس.", product: "Sundae Core + Pulse", icon: "chart" },
    ...
    outcomes: [
      ...
      { title: "محفظة أحكم، مفاجآت أقل", description: "المواقع ضعيفة الأداء تظهر في الإحاطة الصباحية، والإصلاح يُوجَّه إلى مسؤول والهامش المستردّ يُقاس - لا يُترك لمراجعة الربع.", icon: "performance" },
    ...
    crewCallout: {
      ...
      title: "إشارة القوى العاملة تغذي الإحاطة التنفيذية.",
      description: "الجدولة والوقت وجاهزية الرواتب متعددة المناطق وحالات HR تمر عبر Crew - وكل حدث قوى عاملة يصل إلى إحاطتك الصباحية ومراجعتك الربعية كإشارة، لا كلوحة قوى عاملة منفصلة على C-suite أن يتذكر فتحها. اختياري: أحضر HR الخاص بك (Bayzat، Personio، Gusto) ويرى العرض التنفيذي نفس الإشارة الموحدة.",
  ...
  fr: {
    ...
    description: "Briefings IA quotidiens sur chaque marque. Trouvez la fuite de marge le jour où elle apparaît, confiez la correction à un opérateur responsable, et mesurez la marge récupérée par rapport à la référence.",
    ...
    howSundaeHelps: [
      { title: "Pilotage portefeuille en temps réel", description: "Revenu, marge et main-d'œuvre sur chaque site. Le point de vente qui perd de la marge remonte, la correction confiée à son responsable, et la marge récupérée mesurée par rapport à la référence.", product: "Sundae Core + Pulse", icon: "chart" },
    ...
    outcomes: [
      ...
      { title: "Portefeuille plus net, moins de surprises", description: "Les sites en sous-performance apparaissent dans le briefing matinal, la correction confiée à un responsable et la marge récupérée mesurée - pas laissés à la revue trimestrielle.", icon: "performance" },
    ...
    crewCallout: {
      ...
      title: "Le signal main-d'œuvre alimente le briefing exécutif.",
      description: "Planning, temps, readiness paie multi-régions et casework RH passent par Crew - et chaque événement main-d'œuvre arrive dans votre briefing matinal et revue trimestrielle comme signal, pas comme un dashboard main-d'œuvre séparé que la direction doit penser à ouvrir. Optionnel : BYO-HR (Bayzat, Personio, Gusto) et la vue exécutive voit toujours le même signal consolidé.",
  ...
  es: {
    ...
    description: "Briefings de IA diarios sobre cada marca. Encuentra la fuga de margen el día que aparece, dirige la corrección a un operador responsable y mide el margen recuperado frente a la línea base.",
    ...
    howSundaeHelps: [
      { title: "Comando de portafolio en tiempo real", description: "Ingresos, margen y personal en vivo por local. El local que pierde margen aparece, la corrección dirigida a su responsable, y el margen recuperado medido frente a la línea base.", product: "Sundae Core + Pulse", icon: "chart" },
    ...
    outcomes: [
      ...
      { title: "Portafolio afinado, menos sorpresas", description: "Los locales débiles aparecen en el briefing matinal, la corrección dirigida a un responsable y el margen recuperado medido - no quedan para la revisión trimestral.", icon: "performance" },
    ...
    crewCallout: {
      ...
      title: "La señal de la fuerza laboral alimenta el briefing ejecutivo.",
      description: "Horarios, tiempo, readiness de nómina multi-región y casework de RR.HH. pasan por Crew - y cada evento de fuerza laboral llega a tu briefing matinal y revisión trimestral como señal, no como un panel separado que la C-suite tenga que recordar abrir. Opcional: BYO-HR (Bayzat, Personio, Gusto) y la vista ejecutiva sigue viendo la misma señal consolidada.",
      ctaText: "Explorar Sundae Crew",
    },
    ctaTitle: "Recupera margen en todo el portafolio.",
...
export default function CSuiteExecutivesPage() {
  ...
  return <SolutionPageLayout copy={copy} mockup={<ThemedShot framed priority width={1600} height={1000} dark="/images/product/2026-fresh/insights-exec-summary-dark.png" light="/images/product/2026-fresh/insights-exec-summary.png" alt="Executive Summary - portfolio health score, open alerts, and module scorecards across revenue, labor, and inventory" />} gallery={<SectionProductGallery defaultPersona="c_suite" />} />;
}
