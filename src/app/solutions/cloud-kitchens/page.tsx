...
const localizedCopy: Record<"en" | "ar" | "fr" | "es", SolutionCopy> = {
  en: {
    ...
    howSundaeHelps: [
      ...
      { title: "From live signal to recovered margin", description: "Order volume, prep time, and late-orders by hour surface the leak as it forms - Sundae routes one owner the fix and measures the recovered margin against baseline, not a week-later post-mortem.", product: "Pulse", icon: "chart" },
    ],
    ...
    outcomes: [
      ...
      { title: "Measure the margin you recover", description: "Every fix - a dropped channel, a rebalanced brand, a leaner shift - is measured back against baseline, so recovered margin is proven, not assumed.", icon: "support" },
    ],
    ctaTitle: "Recover the margin, platform by platform.",
    ctaDescription: "30 minutes. Your kitchens. The channel you would drop - and the margin you would recover.",
  ...
  ar: {
    ...
    howSundaeHelps: [
      ...
      { title: "من الإشارة الحية إلى الهامش المستعاد", description: "حجم الطلبات ووقت التحضير والطلبات المتأخرة بالساعة تكشف التسرّب فور تكوّنه - يوجّه Sundae الإصلاح إلى مالك واحد ويقيس الهامش المستعاد مقابل خط الأساس، لا تحليلاً لاحقاً بعد أسبوع.", product: "Pulse", icon: "chart" },
    ],
    ...
    outcomes: [
      ...
      { title: "قِس الهامش الذي تستعيده", description: "كل إصلاح - قناة مُسقَطة، علامة أُعيد توازنها، وردية أنحف - يُقاس مقابل خط الأساس، فيصبح الهامش المستعاد مُثبَتاً لا مفترضاً.", icon: "support" },
    ],
  ...
  fr: {
    ...
    howSundaeHelps: [
      ...
      { title: "Du signal en direct à la marge récupérée", description: "Le volume de commandes, le temps de préparation et les retards à l'heure font remonter la fuite dès qu'elle se forme - Sundae confie le correctif à un responsable unique et mesure la marge récupérée par rapport à la référence, pas un post-mortem une semaine plus tard.", product: "Pulse", icon: "chart" },
    ],
    ...
    outcomes: [
      ...
      { title: "Mesurez la marge que vous récupérez", description: "Chaque correction - un canal abandonné, une marque rééquilibrée, un service plus léger - est mesurée par rapport à la référence, de sorte que la marge récupérée est prouvée, pas supposée.", icon: "support" },
    ],
  ...
  es: {
    ...
    howSundaeHelps: [
      ...
      { title: "De la señal en vivo al margen recuperado", description: "El volumen de pedidos, el tiempo de preparación y los retrasos por hora hacen aflorar la fuga en cuanto se forma - Sundae asigna la corrección a un único responsable y mide el margen recuperado frente a la línea base, no un post-mortem una semana después.", product: "Pulse", icon: "chart" },
    ],
    ...
    outcomes: [
      ...
      { title: "Mide el margen que recuperas", description: "Cada corrección - un canal eliminado, una marca reequilibrada, un turno más ágil - se mide frente a la línea base, así el margen recuperado se prueba, no se supone.", icon: "support" },
    ],
