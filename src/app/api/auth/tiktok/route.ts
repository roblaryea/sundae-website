...
export async function GET(request: NextRequest) {
  ...
  if (clientKey) {
    const redirectUri = encodeURIComponent('http://localhost:3334/callback/')
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .card { background: #0f1923; border: 1px solid #2a3a4d; border-radius: 16px; padding: 48px; max-width: 520px; text-align: center; }
    .logo { margin-bottom: 32px; }
    .logo img { height: 40px; }
    h1 { font-size: 24px; font-weight: 700; margin-bottom: 8px; }
    .subtitle { font-size: 14px; color: #9ca3af; margin-bottom: 32px; }
    .tiktok-btn { display: inline-flex; align-items: center; gap: 10px; background: #000; color: #fff; border: none; border-radius: 12px; padding: 14px 32px; font-size: 16px; font-weight: 600; cursor: pointer; text-decoration: none; transition: transform 0.15s; }
    .tiktok-btn:hover { transform: scale(1.02); }
    .tiktok-icon { width: 24px; height: 24px; background: linear-gradient(135deg, #69C9D0, #EE1D52); border-radius: 4px; }
    .scopes { margin-top: 32px; text-align: left; background: #1a2332; border-radius: 8px; padding: 16px 20px; }
    .scopes h3 { font-size: 12px; text-transform: uppercase; letter-spacing: 0.1em; color: #6b7280; margin-bottom: 12px; }
    .scope-item { display: flex; align-items: center; gap: 8px; font-size: 14px; color: #d1d5db; margin-bottom: 8px; }
    ...
    .input-group input { width: 100%; padding: 10px 14px; background: #1a2332; border: 1px solid #2a3a4d; border-radius: 8px; color: #fff; font-size: 14px; outline: none; }
